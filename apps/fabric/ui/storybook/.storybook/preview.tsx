...
function installStorybookApiFixtures() {
  ...
  const currentWindow = window as typeof window & {
    __fabricStorybookFetchInstalled?: boolean;
  };
  if (currentWindow.__fabricStorybookFetchInstalled) return;

  const originalFetch = window.fetch.bind(window);
  currentWindow.__fabricStorybookFetchInstalled = true;
  ...
  window.fetch = async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (adapterSchemaMatch) {
      ...
      }).__fabricStorybookAdapterSchemas;
...
const preview: Preview = {
  ...
  globalTypes: {
    theme: {
      description: "HermesFabric color mode",
      defaultValue: "dark",
      toolbar: {
        title: "Theme",
        icon: "mirror",
