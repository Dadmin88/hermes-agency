...
export type { TranscriptEntry, StdoutLineParser, CreateConfigValues } from "@hermes-fabric/adapter-utils";

export interface StatefulStdoutParser {
  parseLine: (line: string, ts: string) => import("@hermes-fabric/adapter-utils").TranscriptEntry[];
  reset: () => void;
}
...
export interface TranscriptParserSource {
  parseStdoutLine: (line: string, ts: string) => import("@hermes-fabric/adapter-utils").TranscriptEntry[];
