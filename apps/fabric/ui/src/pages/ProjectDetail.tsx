...
import { PROJECT_COLORS, PROJECT_ICON_NAMES, isUuidLike, type BudgetPolicySummary } from "@hermes-fabric/shared";
...
function ProjectIssuesList({ projectId, companyId }: { projectId: string; companyId: string }) {
  ...
  return (
    <IssuesList
      ...
      projectId={projectId}
      viewStateKey="fabric:project-issues-view"
...
function ProjectPluginOperationsList({
  ...
  return (
    <IssuesList
      ...
      liveIssueIds={liveIssueIds}
      projectId={projectId}
      viewStateKey={`fabric:project-plugin-operations-view:${pluginKey}`}
      onUpdateIssue={(id, data) => updateIssue.mutate({ id, data })}
    />
  );
}
...
export function ProjectDetail() {
  ...
  if (routeProjectRef && activeTab === null) {
    ...
    if (project?.id) {
      try { cachedTab = localStorage.getItem(`fabric:project-tab:${project.id}`); } catch {}
    }
  ...
  const handleTabChange = (tab: ProjectTab) => {
    ...
    if (project?.id) {
      try { localStorage.setItem(`fabric:project-tab:${project.id}`, tab); } catch {}
    }
