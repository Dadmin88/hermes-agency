...
/**
 * Filter .fabric.yaml content so it only includes entries whose
 ...
 * renderYamlBlock.
 */
function filterHermesFabricYaml(yaml: string, checkedFiles: Set<string>): string {
...
function generateReadmeFromSelection(
  ...
  lines.push("");
  lines.push("This is an [Agent Company](https://fabric.ing) package.");
  lines.push("");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("pnpm hermes-fabric company import this-github-url-or-folder");
  lines.push("```");
  lines.push("");
  lines.push("See [HermesFabric](https://fabric.ing) for more information.");
  lines.push("");
  lines.push("---");
  lines.push(`Exported from [HermesFabric](https://fabric.ing) on ${new Date().toISOString().split("T")[0]}`);
  lines.push("");
...
export function CompanyExport() {
  ...
  // Recompute .fabric.yaml and README.md content whenever checked files
  ...
  const effectiveFiles = useMemo(() => {
    ...
    // Filter .fabric.yaml
    const yamlPath = exportData.fabricExtensionPath;
    if (yamlPath && typeof exportData.files[yamlPath] === "string") {
      filtered[yamlPath] = filterHermesFabricYaml(exportData.files[yamlPath], checkedFiles);
    }
