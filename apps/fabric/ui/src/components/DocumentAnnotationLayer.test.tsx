...
describe("DocumentAnnotationLayer", () => {
  ...
  it("uses solid yellow backgrounds for annotation highlights in light and dark themes", async () => {
    ...
    const highlights = Array.from(container.querySelectorAll(".fabric-doc-annotation-highlight"));
  ...
  it("does not render highlights for text clipped by folded document content", async () => {
    ...
    expect(container.querySelector(".fabric-doc-annotation-highlight")).toBeNull();
    expect(container.querySelector(".fabric-doc-annotation-hit-target")).toBeNull();
  });
  ...
  it("uses native CSS highlights for visual paint when the browser supports them", async () => {
    ...
    expect(container.querySelector(".fabric-doc-annotation-highlight")).toBeNull();
    expect(container.querySelector(".fabric-doc-annotation-hit-target")).not.toBeNull();
    const openHighlightCall = setHighlight.mock.calls.find(([name]) => name === "fabric-doc-annotation-open");
    expect(openHighlightCall).toBeTruthy();
    expect((openHighlightCall?.[1] as MockHighlight).ranges).toHaveLength(1);

    await act(async () => root?.unmount());
    root = null;
    expect(deleteHighlight).toHaveBeenCalledWith("fabric-doc-annotation-open");
