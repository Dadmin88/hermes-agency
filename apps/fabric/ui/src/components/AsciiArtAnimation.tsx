...
type HermesFabricSprite = (typeof HERMES_FABRIC_SPRITES)[number];
...
function spriteSize(sprite: HermesFabricSprite): { width: number; height: number } {
  let width = 0;
...
export function AsciiArtAnimation() {
  ...
  useEffect(() => {
    ...
    function drawStaticFrame() {
      ...
      for (let baseRow = 1; baseRow < rows - 9; baseRow += gapY) {
        ...
        for (let baseCol = startX; baseCol < cols - 10; baseCol += gapX) {
          const sprite = HERMES_FABRIC_SPRITES[(baseCol + baseRow) % HERMES_FABRIC_SPRITES.length]!;
    ...
    function spawnClip() {
      const sprite = HERMES_FABRIC_SPRITES[Math.floor(Math.random() * HERMES_FABRIC_SPRITES.length)]!;
