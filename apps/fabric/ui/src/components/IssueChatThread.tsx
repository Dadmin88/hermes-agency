...
import { useIssueRuntime, type IssueRuntimeReassignment } from "../hooks/useIssueRuntime";
...
import { buildAgentMentionHref } from "@hermes-fabric/shared";
...
import { AlertTriangle, ArrowRight, Brain, Check, ChevronDown, ClipboardList, Copy, Hammer, Loader2, MoreHorizontal, Link2, PauseCircle, Search, Square, ThumbsDown, ThumbsUp, Trash2 } from "lucide-react";
...
function parseReassignment(target: string): IssueRuntimeReassignment | null {
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Choose whether voted AI outputs can be shared with HermesFabric Labs. This
...
function SystemNoticeCommentRow({
  ...
  const source = (() => {
    ...
    if (authorType === "system") {
      const label = runAgentName ?? "Hermes Fabric";
      if (runAgentId && runId) return { label, href: `/agents/${runAgentId}/runs/${runId}` };
      return { label };
    }
    if (runAgentId && runId) {
      return { label: authorName ?? runAgentName ?? "Hermes Fabric", href: `/agents/${runAgentId}/runs/${runId}` };
    }
