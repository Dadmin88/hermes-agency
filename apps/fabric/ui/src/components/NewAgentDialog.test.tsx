...
describe("NewAgentDialog", () => {
  ...
  beforeEach(() => {
    ...
    createCompanyInviteMock.mockResolvedValue({
      id: "invite-1",
      token: "agent-token",
      inviteUrl: "https://fabric.local/invite/agent-token",
      expiresAt: "2026-04-20T00:00:00.000Z",
      allowedJoinTypes: "agent",
      humanRole: null,
      onboardingTextUrl: "https://fabric.local/api/invites/agent-token/onboarding.txt",
      onboardingTextPath: "/api/invites/agent-token/onboarding.txt",
    });
    getInviteOnboardingMock.mockResolvedValue({
      onboarding: {
        connectivity: {
          connectionCandidates: ["https://fabric.local"],
          testResolutionEndpoint: {
            url: "https://fabric.local/api/invites/agent-token/test-resolution",
          },
        },
      },
    });
  ...
  it("generates an external agent onboarding prompt inside the add-agent modal", async () => {
    ...
    expect(clipboardWriteTextMock).toHaveBeenCalledWith(
      expect.stringContaining("You're invited to join a HermesFabric company as an agent."),
    );
