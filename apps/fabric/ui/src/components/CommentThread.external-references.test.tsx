...
import type { Approval } from "@hermes-fabric/shared";
...
describe("CommentThread external object decoration (integration)", () => {
  ...
  it("decorates a resolved URL and leaves an unknown URL unchanged when rendered through the real MarkdownBody", () => {
    ...
    expect(resolvedLink?.classList.contains("fabric-markdown-external-ref")).toBe(true);
    ...
    expect(unknownLink?.classList.contains("fabric-markdown-external-ref")).toBe(false);
