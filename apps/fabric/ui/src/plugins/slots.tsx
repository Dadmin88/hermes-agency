 ...
 *
 * Plugin bundles are built with `external: ["@hermes-fabric/plugin-sdk/ui", "react", "react-dom"]`,
 ...
 *    host's global bridge registry (`globalThis.__fabricPluginBridge__`)
...
function createReactShimSource(reactModule: object): string {
  ...
  return `
        const R = globalThis.__fabricPluginBridge__?.react;
        if (!R) {
          throw new Error("HermesFabric plugin React runtime is not initialized.");
        }
        export default R;
${namedExports}
      `;
...
function getShimBlobUrl(specifier: "react" | "react-dom" | "react-dom/client" | "react/jsx-runtime" | "sdk-ui"): string {
  ...
  switch (specifier) {
    ...
    case "react-dom/client":
      source = `
        const RD = globalThis.__fabricPluginBridge__?.reactDom;
        export default RD;
    ...
    case "sdk-ui":
      source = `
        const SDK = globalThis.__fabricPluginBridge__?.sdkUi ?? {};
        function missing(name) {
          return function MissingHermesFabricSdkUiComponent() {
            throw new Error('HermesFabric plugin UI runtime is not initialized for "' + name + '". Ensure the host loaded the plugin bridge before rendering this UI module.');
          };
        }
 ...
 * - `import { ... } from "@hermes-fabric/plugin-sdk/ui";`
 *
...
function rewriteBareSpecifiers(source: string): string {
  ...
  const rewrites: Record<string, string> = {
    '"@hermes-fabric/plugin-sdk/ui"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@hermes-fabric/plugin-sdk/ui'": `'${getShimBlobUrl("sdk-ui")}'`,
    '"@hermes-fabric/plugin-sdk/ui/hooks"': `"${getShimBlobUrl("sdk-ui")}"`,
    "'@hermes-fabric/plugin-sdk/ui/hooks'": `'${getShimBlobUrl("sdk-ui")}'`,
...
async function importPluginModule(url: string): Promise<Record<string, unknown>> {
  ...
  if (!globalThis.__fabricPluginBridge__) {
 ...
 * self-register via `window.fabricPlugins.registerReactComponent()`.
 ...
 * of `@hermes-fabric/plugin-sdk/ui`, `react`, and `react-dom` resolve to the
