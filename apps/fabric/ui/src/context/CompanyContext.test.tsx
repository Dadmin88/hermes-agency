...
import type { Company } from "@hermes-fabric/shared";
...
describe("CompanyProvider", () => {
  ...
  it("does not expose a stale stored company id before companies load", async () => {
    localStorage.setItem("fabric.selectedCompanyId", "stale-company");
  ...
  it("replaces a stale stored company id with the first loaded company", async () => {
    ...
    expect(localStorage.getItem("fabric.selectedCompanyId")).toBe("company-1");
  });
});
