...
describe("issue-reference", () => {
  ...
  it("does not treat GitHub issue URLs as internal HermesFabric issue links", () => {
    expect(parseIssuePathIdFromPath("https://github.com/DeployFaith/Hermes_Agency/issues/1778")).toBeNull();
    expect(parseIssueReferenceFromHref("https://github.com/DeployFaith/Hermes_Agency/issues/1778")).toBeNull();
  });
  ...
  it("preserves absolute HermesFabric issue URLs so origin, port, and hash are not lost", () => {
