...
export function getWorktreeUiBranding(): WorktreeUiBranding | null {
  if (readMetaContent("fabric-worktree-enabled") !== "true") return null;

  const name = readMetaContent("fabric-worktree-name");
  const color = normalizeHexColor(readMetaContent("fabric-worktree-color"));
  ...
  return {
    ...
    name,
    color,
    textColor: normalizeHexColor(readMetaContent("fabric-worktree-text-color")) ?? pickReadableTextColor(color),
  };
}
