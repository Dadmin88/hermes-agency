...
import type { Agent, IssueComment } from "@hermes-fabric/shared";
...
function authorNameForComment(
  ...
  if (authorAgentId) {
    return agentMap?.get(authorAgentId)?.name ?? (options?.isSystemNotice ? "Hermes Fabric" : authorAgentId.slice(0, 8));
  }
