import type { ActivityEvent, Issue, SuccessfulRunHandoffState } from "@hermes-fabric/shared";
...
export const SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY =
  "HermesFabric could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.";
...
export function isSuccessfulRunHandoffEscalationComment(text: string) {
  ...
  return trimmed === SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY
    || /^HermesFabric exhausted the bounded successful-run handoff correction\b/i.test(trimmed);
}
