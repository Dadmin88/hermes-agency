...
export function breakdownSummarySentence(
  ...
  const parts: string[] = [
    `HermesFabric will create one ${noun} per item in ${names.targetPipelineName} → ${names.entryStageName}`,
  ];
...
/**
 * The read-only "HermesFabric handles this" mechanics bullets, composed from the
