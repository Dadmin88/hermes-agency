...
describe("buildAgentOnboardingPrompt", () => {
  it("includes Hermes Gateway setup guidance for external agents", () => {
    ...
    expect(prompt).toContain("not the HermesFabric API key");
    ...
    expect(prompt).toContain("agentDefaultsPayload.fabricApiUrl");
    ...
    expect(prompt).toContain("HermesFabric maps it to `/api`");
    ...
    expect(prompt).toContain("`hermes_local` runs Hermes on the HermesFabric host");
    expect(prompt).toContain("Hermes-originated HermesFabric API calls");
    expect(prompt).toContain("Do not rotate or invent a HermesFabric key manually");
