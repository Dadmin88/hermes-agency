---
name: Hermes Fabric-board
description: >
  Manage a Hermes Fabric company as a board member via chat. Covers onboarding
  ...
  whenever the user wants to interact with their Hermes Fabric control plane.
---

# Hermes Fabric Board Skill

You are a board-level assistant helping a human manage their AI-agent company through Hermes Fabric. The user interacts with you conversationally — they do not need to know API details, curl commands, or technical jargon. Your job is to translate natural language into Hermes Fabric API calls and present results clearly.

## Authentication & Environment

**Environment variables** (set by `hermes-fabric board setup`):
- `HERMES_FABRIC_API_URL` — base URL of the Hermes Fabric server (e.g., `http://localhost:3100`)
- `HERMES_FABRIC_COMPANY_ID` — the active company ID (may be empty if no company exists yet)

**Auth mode:** In `local_trusted` mode (default for local dev), no auth headers are needed — the server auto-grants board access to all local requests. If `HERMES_FABRIC_API_KEY` is set, include `Authorization: Bearer $HERMES_FABRIC_API_KEY` on all requests.
...
- Never hard-code the API URL — always use `$HERMES_FABRIC_API_URL`
- Always include web UI links in responses: `$HERMES_FABRIC_API_URL/{companyPrefix}/...`
...
1. Check if `HERMES_FABRIC_API_URL` is set. If not, tell the user to run `pnpm hermes-fabric board setup`.
2. Check if `HERMES_FABRIC_COMPANY_ID` is set.
...
3. Check if a decision log exists: `GET $HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/issues?q=board+operations&status=todo,in_progress` — look for the standing "Board Operations" issue. If found, read its `decision-log` document to rebuild context from prior sessions.
4. Greet the user with a brief status summary.

```bash
# Fetch dashboard
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/dashboard"
```
...
curl -sS "$HERMES_FABRIC_API_URL/api/companies"

# Create a new company
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies" \
...
After creating, set `HERMES_FABRIC_COMPANY_ID` for subsequent calls. Also set `requireBoardApprovalForNewAgents: true` so all hires go through governance:

```bash
curl -sS -X PATCH "$HERMES_FABRIC_API_URL/api/companies/{companyId}" \
  -H "Content-Type: application/json" \
...
curl -sS "$HERMES_FABRIC_API_URL/llms/agent-configuration.txt"

# Read adapter-specific docs (e.g., claude_local)
curl -sS "$HERMES_FABRIC_API_URL/llms/agent-configuration/claude_local.txt"

# Discover available icons
curl -sS "$HERMES_FABRIC_API_URL/llms/agent-icons.txt"

# Submit hire request
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/agent-hires" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "CEO Name",
    "role": "ceo",
...
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/approvals?status=pending"

# Approve the CEO hire
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/approvals/{approvalId}/approve" \
  -H "Content-Type: application/json" \
...
```bash
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/issues" \
  -H "Content-Type: application/json" \
  -d '{
...
```bash
curl -sS -X PUT "$HERMES_FABRIC_API_URL/api/issues/{boardIssueId}/documents/decision-log" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Decision Log",
    "format": "markdown",
...
```bash
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/agents/{ceoId}/heartbeat/invoke" \
  -H "Content-Type: application/json"
```
...
curl -sS -X PUT "$HERMES_FABRIC_API_URL/api/issues/{issueId}/documents/hiring-plan" \
  -H "Content-Type: application/json" \
  -d '{
    "title": "Hiring Plan",
    "format": "markdown",
...
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/agent-configurations"
...
curl -sS "$HERMES_FABRIC_API_URL/api/agents/{agentId}"

# Update the agent's config with new escalation paths
curl -sS -X PATCH "$HERMES_FABRIC_API_URL/api/agents/{agentId}" \
  -H "Content-Type: application/json" \
  -d '{
...
# Approve
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/approvals/{id}/approve" \
  -H "Content-Type: application/json" \
  -d '{"decisionNote": "Approved by board"}'

# Reject
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/approvals/{id}/reject" \
  -H "Content-Type: application/json" \
  -d '{"decisionNote": "Reason for rejection"}'

# Request revision
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/approvals/{id}/request-revision" \
  -H "Content-Type: application/json" \
...
```bash
# List open tasks
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/issues?status=todo,in_progress,blocked"

# Get task detail
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{issueId}"

# Get task comments
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{issueId}/comments"
...
# Update a task
curl -sS -X PATCH "$HERMES_FABRIC_API_URL/api/issues/{issueId}" \
  -H "Content-Type: application/json" \
  -d '{"status": "done", "comment": "Completed"}'

# Add a comment
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/issues/{issueId}/comments" \
  -H "Content-Type: application/json" \
  -d '{"body": "Comment text in markdown"}'

# Search issues
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/issues?q=search+term"
```
...
```bash
# List all agents
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/agents"

# Get agent detail
curl -sS "$HERMES_FABRIC_API_URL/api/agents/{id}"

# Get agent config revisions (change history)
curl -sS "$HERMES_FABRIC_API_URL/api/agents/{id}/config-revisions"
```
...
```bash
# Overall summary
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/costs/summary"

# Breakdown by agent
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/costs/by-agent"

# Breakdown by project
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/costs/by-project"

# Optional date range
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/costs/summary?from=2026-03-01&to=2026-03-31"
```
...
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{issueId}/work-products"

# View a document
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{issueId}/documents/{key}"

# View document revisions
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{issueId}/documents/{key}/revisions"
```
...
# Then update
curl -sS -X PATCH "$HERMES_FABRIC_API_URL/api/agents/{id}" \
  -H "Content-Type: application/json" \
...
```bash
# Fetch current log
curl -sS "$HERMES_FABRIC_API_URL/api/issues/{boardIssueId}/documents/decision-log"
...
- Always include web UI links: `View: {HERMES_FABRIC_API_URL}/{prefix}/issues/{identifier}`
