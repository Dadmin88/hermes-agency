...
usage() {
  cat <<'EOF'
Usage:
  fabric-upload-artifact.sh FILE [options]

Uploads a generated file from the current workspace to the current HermesFabric
issue, then creates an attachment-backed artifact work product by default.

Required environment for live uploads:
  HERMES_FABRIC_API_URL, HERMES_FABRIC_API_KEY, HERMES_FABRIC_COMPANY_ID, HERMES_FABRIC_TASK_ID, HERMES_FABRIC_RUN_ID

Options:
  --issue-id ID          Issue id to attach to (default: HERMES_FABRIC_TASK_ID)
  --company-id ID        Company id (default: HERMES_FABRIC_COMPANY_ID)
...
Examples:
  scripts/fabric-upload-artifact.sh dist/demo.mp4 \
    --title "Demo video render" \
    --summary "MP4 render for board review"

  scripts/fabric-upload-artifact.sh out/walkthrough.webm \
...
file_path=""
issue_id="${HERMES_FABRIC_TASK_ID:-}"
company_id="${HERMES_FABRIC_COMPANY_ID:-}"
title=""
summary=""
content_type=""
...
if [[ -z "${HERMES_FABRIC_API_URL:-}" || -z "${HERMES_FABRIC_API_KEY:-}" || -z "${HERMES_FABRIC_RUN_ID:-}" ]]; then
  printf 'Missing HERMES_FABRIC_API_URL, HERMES_FABRIC_API_KEY, or HERMES_FABRIC_RUN_ID.\n' >&2
  exit 1
fi

if [[ -z "$issue_id" || -z "$company_id" ]]; then
  printf 'Missing issue or company id. Pass --issue-id/--company-id or set HERMES_FABRIC_TASK_ID/HERMES_FABRIC_COMPANY_ID.\n' >&2
  exit 1
fi
