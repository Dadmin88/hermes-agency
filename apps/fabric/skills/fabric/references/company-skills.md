...
- App-shipped catalog: a curated set of company skills in `@hermes-fabric/skills-catalog`, browseable and installable without leaving Hermes Fabric.
...
`hermes-fabric skills` CLI documented in `doc/CLI.md` — it wraps every endpoint
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/skills/catalog?kind=bundled" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"

curl -sS "$HERMES_FABRIC_API_URL/api/skills/catalog/ref?ref=github-pr-workflow" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"

curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills/install-catalog" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "catalogSkillId": "hermes-fabric:bundled:software-development:github-pr-workflow"
  }'
```
...
```sh
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills/import" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
...
```sh
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills/scan-projects" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"
```
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills/<skill-id>" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"

curl -sS "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/skills/<skill-id>/files?path=SKILL.md" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"
```
...
```sh
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/agents/<agent-id>/skills/sync" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "desiredSkills": [
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/agents/<agent-id>/skills" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"
```
...
```sh
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/agents" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "QA Browser Agent",
...
- Built-in Hermes Fabric runtime skills are still added automatically when required by the adapter.
