...
Use the helper bundled with this skill. From an installed `fabric` skill directory, the helper lives at `scripts/fabric-upload-artifact.sh`:

```bash
scripts/fabric-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses `HERMES_FABRIC_API_URL`, `HERMES_FABRIC_API_KEY`, `HERMES_FABRIC_COMPANY_ID`, `HERMES_FABRIC_TASK_ID`, and `HERMES_FABRIC_RUN_ID`. It uploads the file as an issue attachment, creates an attachment-backed artifact work product by default, and prints issue-safe markdown links for your final comment.
...
If the helper is unavailable, use the Hermes Fabric API directly:
