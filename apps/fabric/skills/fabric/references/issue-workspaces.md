...
```sh
curl -sS -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  "$HERMES_FABRIC_API_URL/api/issues/$HERMES_FABRIC_TASK_ID/heartbeat-context"
```
...
If `currentExecutionWorkspace` is `null`, the issue does not currently have a realized execution workspace. For child/follow-up work, create the child with `parentId` or use `inheritExecutionWorkspaceFromIssueId` so Hermes Fabric preserves workspace continuity.

## Control Services

Prefer Hermes Fabric-managed runtime service controls over manual `pnpm dev &` or ad-hoc background processes. These endpoints keep service state, URLs, logs, and ownership visible to other agents and the board.
...
curl -sS -X POST \
  ...
  "$HERMES_FABRIC_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/start" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-HermesFabric-Run-Id: $HERMES_FABRIC_RUN_ID" \
  -H "Content-Type: application/json" \
  "$HERMES_FABRIC_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/restart" \
  -d '{}'
...
curl -sS -X POST \
  ...
  -H "X-HermesFabric-Run-Id: $HERMES_FABRIC_RUN_ID" \
  -H "Content-Type: application/json" \
  "$HERMES_FABRIC_API_URL/api/execution-workspaces/<workspace-id>/runtime-services/stop" \
  -d '{}'
```
...
When the Hermes Fabric MCP tools are available, prefer these issue-scoped tools:

- `fabricGetIssueWorkspaceRuntime` — reads `currentExecutionWorkspace` and service URLs for an issue.
- `fabricControlIssueWorkspaceServices` — starts, stops, or restarts the current issue workspace services.
- `fabricWaitForIssueWorkspaceService` — waits until a selected service is running and returns its URL when exposed.
