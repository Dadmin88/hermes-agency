---
name: Hermes Fabric-create-agent
description: >
  Create new agents in Hermes Fabric with governance-aware hiring. Use when you need
  to inspect adapter configuration options, compare existing agent configs,
  draft a new agent prompt/config, and submit a hire request.
---

# Hermes Fabric Create Agent Skill
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/agents/me" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"
```

### 2. Discover adapter configuration for this Hermes Fabric instance
...
`skills/fabric-create-agent/references/agent-instruction-templates.md`

Generic fallback for no-template hires:
`skills/fabric-create-agent/references/baseline-role-guide.md`
...
- for coding or execution agents, include the Hermes Fabric execution contract: start actionable work in the same heartbeat; do not stop at a plan unless planning was requested; leave durable progress with a clear next action; use child issues for long or parallel delegated work instead of polling; mark blocked work with owner/action; respect budget, pause/cancel, approval gates, and company boundaries
...
`skills/fabric-create-agent/references/draft-review-checklist.md`
...
- when the board approves, you will be woken with `HERMES_FABRIC_APPROVAL_ID`; read linked issues and close/comment follow-up

```sh
curl -sS "$HERMES_FABRIC_API_URL/api/approvals/<approval-id>" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"

curl -sS -X POST "$HERMES_FABRIC_API_URL/api/approvals/<approval-id>/comments" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS -X POST "$HERMES_FABRIC_API_URL/api/issues/<issue-id>/approvals" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "Content-Type: application/json" \
...
```sh
curl -sS "$HERMES_FABRIC_API_URL/api/approvals/$HERMES_FABRIC_APPROVAL_ID" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"

curl -sS "$HERMES_FABRIC_API_URL/api/approvals/$HERMES_FABRIC_APPROVAL_ID/issues" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY"
```
...
- Template index and how to apply a template: `skills/fabric-create-agent/references/agent-instruction-templates.md`
- Individual role templates: `skills/fabric-create-agent/references/agents/`
- Generic baseline role guide (no-template fallback): `skills/fabric-create-agent/references/baseline-role-guide.md`
- Pre-submit draft-review checklist: `skills/fabric-create-agent/references/draft-review-checklist.md`
- Endpoint payload shapes and full examples: `skills/fabric-create-agent/references/api-reference.md`
