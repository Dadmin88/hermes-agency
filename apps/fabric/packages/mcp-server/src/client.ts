import type { HermesFabricMcpConfig } from "./config.js";

export class HermesFabricApiError extends Error {
  ...
  constructor(input: {
    ...
    super(input.message);
    this.name = "HermesFabricApiError";
...
export class HermesFabricApiClient {
  constructor(private readonly config: HermesFabricMcpConfig) {}
  ...
  resolveCompanyId(companyId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("companyId is required because HERMES_FABRIC_COMPANY_ID is not set");
    }
    return resolved;
  }
  ...
  resolveAgentId(agentId?: string | null): string {
    ...
    if (!resolved) {
      throw new Error("agentId is required because HERMES_FABRIC_AGENT_ID is not set");
    }
    return resolved;
  }
  ...
  async requestJson<T>(method: string, path: string, options: JsonRequestOptions = {}): Promise<T> {
    ...
    if ((options.includeRunId ?? isWriteMethod(method)) && this.config.runId) {
      headers["X-HermesFabric-Run-Id"] = this.config.runId;
    }
    ...
    if (!response.ok) {
      throw new HermesFabricApiError({
