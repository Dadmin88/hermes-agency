...
function makeClient() {
  return new HermesFabricApiClient({
...
describe("fabric MCP tools", () => {
  ...
  it("adds auth headers and run id to mutating requests", async () => {
    ...
    const tool = getTool("fabricUpdateIssue");
    await tool.execute({
      issueId: "PAP-1135",
      status: "done",
    });
    ...
    expect((init.headers as Record<string, string>)["X-HermesFabric-Run-Id"]).toBe(
  ...
  it("uses default company id for company-scoped list tools", async () => {
    ...
    const tool = getTool("fabricListIssues");
  ...
  it("uses default agent id for checkout requests", async () => {
    ...
    const tool = getTool("fabricCheckoutIssue");
    await tool.execute({
      issueId: "PAP-1135",
    });
  ...
  it("allows create issue requests to omit status so the API applies assignee defaults", async () => {
    ...
    const tool = getTool("fabricCreateIssue");
    await tool.execute({
  ...
  it("defaults issue document format to markdown", async () => {
    ...
    const tool = getTool("fabricUpsertIssueDocument");
    await tool.execute({
      issueId: "PAP-1135",
      key: "plan",
      body: "# Updated",
  ...
  it("controls issue workspace services through the current execution workspace", async () => {
    ...
    const tool = getTool("fabricControlIssueWorkspaceServices");
    await tool.execute({
      issueId: "PAP-1135",
      action: "restart",
  ...
  it("waits for an issue workspace runtime service URL", async () => {
    ...
    const tool = getTool("fabricWaitForIssueWorkspaceService");
    const response = await tool.execute({
      issueId: "PAP-1135",
      serviceName: "web",
      timeoutSeconds: 1,
  ...
  it("creates suggest_tasks interactions with the expected issue-scoped payload", async () => {
    ...
    const tool = getTool("fabricSuggestTasks");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_confirmation interactions with plan target payloads", async () => {
    ...
    const tool = getTool("fabricRequestConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates request_checkbox_confirmation interactions with checkbox payloads", async () => {
    ...
    const tool = getTool("fabricRequestCheckboxConfirmation");
    await tool.execute({
      issueId: "PAP-1135",
  ...
  it("creates approvals with the expected company-scoped payload", async () => {
    ...
    const tool = getTool("fabricCreateApproval");
    await tool.execute({
      type: "hire_agent",
  ...
  it("rejects invalid generic request paths", async () => {
    ...
    const tool = getTool("fabricApiRequest");
    const response = await tool.execute({
      method: "GET",
      path: "issues",
    });
