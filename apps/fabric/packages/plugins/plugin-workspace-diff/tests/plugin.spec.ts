...
async function createGitWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-workspace-diff-plugin-"));
  tempRoots.push(root);
  await fs.mkdir(path.join(root, "src"), { recursive: true });
  await git(root, ["init"]);
  await git(root, ["config", "user.email", "fabric@example.com"]);
  await git(root, ["config", "user.name", "HermesFabric Test"]);
