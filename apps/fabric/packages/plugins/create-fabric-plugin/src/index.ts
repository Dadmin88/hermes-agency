...
function packLocalPackage(packagePath: string, outputDir: string): string {
  ...
  const sdkBundleDir = path.join(outputDir, ".fabric-sdk");
...
/**
 * Generate a complete HermesFabric plugin starter project.
 *
...
export function scaffoldPluginProject(options: ScaffoldPluginOptions): string {
  ...
  const description = options.description ?? "A HermesFabric plugin";
  ...
  const packageJson = {
    ...
    scripts: {
      ...
      "dev:ui": "fabric-plugin-dev-server --root . --ui-dir dist/ui --port 4177",
    ...
    keywords: ["fabric", "plugin", category],
    author,
    license: "MIT",
    ...(packedSharedTarball
      ? {
        pnpm: {
          overrides: {
            "@hermes-fabric/shared": `file:${toPosixPath(path.relative(outputDir, packedSharedTarball))}`,
          },
        },
      }
      : {}),
    devDependencies: {
      ...(packedSharedTarball
        ...
        : {}),
      "@hermes-fabric/plugin-sdk": sdkDependency,
...
`,
    ...
    writeFile(
      ...
      `import { usePluginData, type PluginWidgetProps } from "@hermes-fabric/plugin-sdk/ui";
...
} from "@hermes-fabric/plugin-sdk/testing";
...
`,
    ...
    writeFile(
      path.join(outputDir, "src", "ui", "index.tsx"),
      `import { usePluginAction, usePluginData, type PluginWidgetProps } from "@hermes-fabric/plugin-sdk/ui";
...
When this package is installed from a local path, HermesFabric watches that rebuilt
output and reloads the plugin worker. Local installs run trusted code from this
folder on your machine.

${sdkDependency.startsWith("file:")
  ? `This scaffold snapshots \`@hermes-fabric/plugin-sdk\` and \`@hermes-fabric/shared\` from a local HermesFabric checkout at:\n\n\`${toPosixPath(localSdkPath)}\`\n\nThe packed tarballs live in \`.fabric-sdk/\` for local development. Before publishing this plugin, switch those dependencies to published package versions once they are available on npm.\n\n`
  : ""}

## Install Into HermesFabric

\`\`\`bash
hermes-fabric plugin install ${shellQuote(toPosixPath(outputDir))}
\`\`\`
