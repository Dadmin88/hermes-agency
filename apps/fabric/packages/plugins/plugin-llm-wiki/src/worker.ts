...
import {
  bootstrapWikiRoot,
  bootstrapSpace,
  assembleHermesFabricSourceBundle,
  archiveSpace,
  captureWikiSource,
  createSpace,
  createHermesFabricDistillationRun,
  createHermesFabricDistillationWorkItem,
  createOperationIssue,
  distillHermesFabricProjectPage,
  ...
  listHermesFabricIngestionCandidates,
  getHermesFabricIngestionProfile,
  getOverview,
  listSpaces,
  handleHermesFabricEventIngestion,
  ...
  readTemplate,
  readWikiPage,
  recordHermesFabricDistillationOutcome,
  ...
  updateHermesFabricIngestionProfile,
  updateSpace,
  writeTemplate,
  writeWikiPage,
} from "./wiki.js";
...
const HERMES_FABRIC_EVENT_INGESTION_EVENTS = [
  "issue.created",
  "issue.updated",
...
function buildManualDistillPrompt(input: { companyId: string; projectId?: string | null; rootIssueId?: string | null }) {
  ...
  return [
    ...
    "",
    "Prompt source: LLM Wiki plugin action `distill-fabric-now` (`packages/plugins/plugin-llm-wiki/src/worker.ts`).",
    `Required skill: use the installed \`${HERMES_FABRIC_DISTILL_SKILL_KEY}\` skill before changing wiki files.`,
    "",
    "Scope:",
    ...
    !input.projectId && !input.rootIssueId
      ? "- Do not hardcode a single project. Find non-plugin HermesFabric issues/comments/documents that changed in any project after the last processed cursor and are old enough for the stale/debounce threshold."
      : null,
    "",
    "Process:",
    "1. Read the wiki root AGENTS.md, wiki/index.md, and recent wiki/log.md entries.",
    "2. Assemble bounded HermesFabric source bundles for every eligible project or root issue, excluding LLM Wiki plugin-operation issues.",
    "3. Turn durable signal into project standups, wiki-insightful project pages, decisions, history, index, and log updates per the fabric-distill skill.",
...
const plugin = definePlugin({
  async setup(ctx) {
    ...
    for (const eventName of HERMES_FABRIC_EVENT_INGESTION_EVENTS) {
      ctx.events.on(eventName, async (event) => {
        const result = await handleHermesFabricEventIngestion(ctx, event);
        if (result.status === "recorded") {
          ctx.logger.info("LLM Wiki recorded HermesFabric event for cursor discovery", {
    ...
    ctx.data.register("fabric-ingestion-profile", async (params) => {
      return getHermesFabricIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
      });
    ...
    ctx.data.register("fabric-ingestion-candidates", async (params) => {
      return listHermesFabricIngestionCandidates(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("update-fabric-ingestion-profile", async (params) => {
      return updateHermesFabricIngestionProfile(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("queue-fabric-ingestion-backfill", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      ...
      for (const scope of scopes) {
        ...
        const workItem = await createHermesFabricDistillationWorkItem(ctx, {
          companyId,
          wikiId,
          spaceSlug,
          kind: "backfill",
          ...
          metadata: { backfillStartAt, backfillEndAt, requestedFrom: "queue-fabric-ingestion-backfill" },
        });
        const operation = await createOperationIssue(ctx, {
          ...
          spaceSlug,
          operationType: "backfill",
          title: scope.rootIssueId ? "Backfill HermesFabric root issue wiki history" : "Backfill HermesFabric project wiki history",
          useCheapModelProfile: params.useCheapModelProfile === true,
          prompt: [
            "Backfill LLM Wiki distillation was queued from a per-space HermesFabric ingestion profile.",
    ...
    ctx.actions.register("assemble-fabric-source-bundle", async (params) => {
      return assembleHermesFabricSourceBundle(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
    ...
    ctx.actions.register("create-fabric-distillation-run", async (params) => {
      return createHermesFabricDistillationRun(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("record-fabric-distillation-outcome", async (params) => {
      ...
      return recordHermesFabricDistillationOutcome(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        runId,
    ...
    ctx.actions.register("distill-fabric-project-page", async (params) => {
      return distillHermesFabricProjectPage(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        projectId: stringField(params.projectId),
    ...
    ctx.actions.register("distill-fabric-now", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      ...
      const workItem = await createHermesFabricDistillationWorkItem(ctx, {
        ...
        metadata: { requestedFrom: "distill-fabric-now" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        ...
        operationType: "distill",
        title: rootIssueId
          ? "Distill HermesFabric root issue into wiki"
          : projectId
            ? "Distill HermesFabric project into wiki"
            : "Distill HermesFabric changes into wiki",
        useCheapModelProfile: params.useCheapModelProfile === true,
    ...
    ctx.actions.register("enable-fabric-distillation-active-projects", async (params) => {
    ...
    ctx.actions.register("backfill-fabric-distillation", async (params) => {
      const companyId = readCompanyIdFromParams(params);
      const spaceSlug = stringField(params.spaceSlug);
      const projectId = stringField(params.projectId);
      const rootIssueId = stringField(params.rootIssueId);
      ...
      const workItem = await createHermesFabricDistillationWorkItem(ctx, {
        ...
        metadata: { backfillStartAt, backfillEndAt, requestedFrom: "backfill-fabric-distillation" },
      });
      const operation = await createOperationIssue(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        operationType: "backfill",
        title: rootIssueId ? "Backfill HermesFabric root issue wiki history" : "Backfill HermesFabric project wiki history",
        useCheapModelProfile: params.useCheapModelProfile === true,
        prompt: [
          "Backfill LLM Wiki distillation requested for a bounded HermesFabric source window.",
      ...
      const result = await distillHermesFabricProjectPage(ctx, {
        companyId,
        wikiId: stringField(params.wikiId),
        spaceSlug,
        projectId,
    ...
    ctx.actions.register("create-fabric-distillation-work-item", async (params) => {
      ...
      return createHermesFabricDistillationWorkItem(ctx, {
        companyId: readCompanyIdFromParams(params),
        wikiId: stringField(params.wikiId),
        spaceSlug: stringField(params.spaceSlug),
        kind,
