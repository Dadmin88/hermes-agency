...
export const PLUGIN_ID = "hermes-fabric.plugin-llm-wiki";
...
export const HERMES_FABRIC_DISTILL_SKILL_KEY = "fabric-distill";
...
export const DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS = 12000;
export const DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS = 60000;
export const DEFAULT_MAX_HERMES_FABRIC_ROUTINE_RUN_CHARS = 120000;
export const DEFAULT_HERMES_FABRIC_COST_CENTS_PER_1K_CHARS = 1;
...
function canonicalSkillKey(skillKey: string) {
  return `plugin/hermes-fabric-plugin-llm-wiki/${skillKey}`;
}
...
const CURSOR_WINDOW_ROUTINE_DESCRIPTION = `Process bounded HermesFabric issue-history windows into the LLM Wiki.

Run procedure:
Target space: default (slug: default). HermesFabric-derived indexing currently writes only into the default space, so this routine never sweeps other spaces. Per-space HermesFabric ingestion profiles are a later phase; until they ship, treat any prompt to operate on a non-default space here as a bug and stop.
...
5. For ambiguous contradictions or major rewrites, leave the pages unchanged and create or comment a follow-up HermesFabric issue with the exact files and evidence.
...
const manifest: HermesFabricPluginManifestV1 = {
  ...
  localFolders: [
    {
      ...
      description: "Company-scoped local folder that stores raw sources, wiki pages, HermesFabric project standups under wiki/projects/, AGENTS.md, IDEA.md, wiki/index.md, and wiki/log.md.",
      access: "readWrite",
      requiredDirectories: [
        "raw",
        "wiki",
  ...
  skills: [
    ...
    {
      skillKey: HERMES_FABRIC_DISTILL_SKILL_KEY,
      displayName: "HermesFabric Distill",
      slug: HERMES_FABRIC_DISTILL_SKILL_KEY,
      description: "Turn HermesFabric cursor-window, distill, or backfill source bundles into wiki-insightful project knowledge.",
      markdown: skillMarkdown(HERMES_FABRIC_DISTILL_SKILL_KEY)
    },
    {
