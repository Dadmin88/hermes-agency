...
import type { Agent, AgentSessionEvent, Issue, IssueComment, PluginContext, PluginEvent, PluginLocalFolderEntry, Project, ToolResult } from "@hermes-fabric/plugin-sdk";
import type { IssueDocument, PluginIssueOriginKind, PluginManagedRoutineResolution, PluginManagedSkillResolution } from "@hermes-fabric/plugin-sdk/types";
import {
  DEFAULT_MAX_SOURCE_BYTES,
  DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS,
  DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS,
  DEFAULT_MAX_HERMES_FABRIC_ROUTINE_RUN_CHARS,
  DEFAULT_HERMES_FABRIC_COST_CENTS_PER_1K_CHARS,
  PLUGIN_ID,
...
const MAX_HERMES_FABRIC_INGESTION_PROFILE_SOURCE_COUNT = 3;
const MAX_HERMES_FABRIC_DISTILLATION_FAN_OUT = 25;
const MAX_HERMES_FABRIC_PROFILE_SELECTED_PROJECTS = 25;
const MAX_HERMES_FABRIC_PROFILE_ROOT_ISSUES = 25;
...
export type HermesFabricDistillationScope = "company" | "project" | "root_issue";
export type HermesFabricDistillationWorkItemKind = "manual" | "retry" | "backfill" | "priority_override" | "review_patch";
...
export type HermesFabricIngestionSourceScope =
...
export type HermesFabricIngestionProfileV1 = {
  version: 1;
  enabled: boolean;
  sourceScopes: HermesFabricIngestionSourceScope[];
...
export type HermesFabricIngestionProfileEffectiveState =
  | "enabled"
  | "disabled"
  | "policy_blocked"
  | "pending_approval"
...
export type HermesFabricIngestionProfileRead = {
  wikiId: string;
  space: Pick<WikiSpace, "id" | "slug" | "displayName" | "accessScope" | "status">;
  profile: HermesFabricIngestionProfileV1;
  effectiveState: HermesFabricIngestionProfileEffectiveState;
  policyBlocks: string[];
...
type HermesFabricIngestionPolicyPurpose =
  | "profile_read"
  | "profile_update"
  | "candidate_search"
  | "queue"
...
type HermesFabricIngestionPolicyDecision =
...
type HermesFabricIngestionCandidatesInput = SpaceInput & {
  query?: string | null;
};
...
type HermesFabricSourceBundleInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  projectId?: string | null;
...
type HermesFabricSourceBundle = {
  markdown: string;
  sourceRefs: HermesFabricSourceRef[];
  sourceHash: string;
...
type HermesFabricDistillationRunInput = HermesFabricSourceBundleInput;

type HermesFabricDistillationOutcomeInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  runId: string;
...
type HermesFabricDistillationWorkItemInput = {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  kind: HermesFabricDistillationWorkItemKind;
  projectId?: string | null;
  rootIssueId?: string | null;
...
type HermesFabricProjectPageDistillationInput = HermesFabricSourceBundleInput & {
  autoApply?: boolean;
...
type HermesFabricDistillationPatchOperation =
  | "standup_update"
  ...
  | "index_refresh"
  | "log_append";
type HermesFabricDistillationPatch = {
  pagePath: string;
  operationType: HermesFabricDistillationPatchOperation;
...
type HermesFabricEventIngestResult =
...
async function requireHermesFabricIngestionPolicy(
  ctx: PluginContext,
  input: { companyId: string; wikiId: string; spaceSlug?: string | null },
  purpose: HermesFabricIngestionPolicyPurpose,
  ...
  const decision = evaluateHermesFabricProfilePolicy({
    space,
    profile,
    purpose,
...
function assertHermesFabricSourceScopePayload(input: { projectId?: string | null; rootIssueId?: string | null }) {
  if (input.projectId && input.rootIssueId) {
    throw new Error("HermesFabric source scope must specify either projectId or rootIssueId, not both.");
  }
}
...
function assertRequestedCharacterLimit(name: string, value: unknown, max: number) {
  ...
  if (Math.floor(value) > max) {
    throw new Error(`${name} exceeds the hard HermesFabric ingestion cap of ${max} characters.`);
  }
}
...
function normalizeCostRate(value: unknown): number {
  if (typeof value !== "number" || !Number.isFinite(value)) return DEFAULT_HERMES_FABRIC_COST_CENTS_PER_1K_CHARS;
  return Math.max(0, value);
}

type HermesFabricDistillationLimits = {
  maxCharacters: number;
  maxCharactersPerSource: number;
...
type DistillationSourceProtectionResult = {
  body: string;
  warning: string | null;
  refPatch: Pick<HermesFabricSourceRef, "redactionStatus" | "redactionReasons">;
};
...
function protectDistillationSourceBody(input: {
  ...
  return {
    body: [
      ...
      "- Review the original HermesFabric source directly if a human needs the unredacted material.",
    ].join("\n"),
...
async function resolveHermesFabricDistillationLimits(
  ctx: PluginContext,
  input: Pick<HermesFabricSourceBundleInput, "maxCharacters" | "maxCharactersPerSource" | "routineRun">,
): Promise<HermesFabricDistillationLimits> {
  assertRequestedCharacterLimit("maxCharacters", input.maxCharacters, DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS);
  assertRequestedCharacterLimit("maxCharactersPerSource", input.maxCharactersPerSource, DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS);
  const config = await ctx.config.get() as Record<string, unknown>;
  const maxCharactersPerSource = Math.min(
    normalizeBundleLimit(input.maxCharactersPerSource, DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS),
    normalizeBundleLimit(config.maxHermesFabricIssueSourceCharacters, DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS),
  );
  const cursorWindowCap = normalizeBundleLimit(
    config.maxHermesFabricCursorWindowCharacters,
    DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS,
  );
  const routineRunCap = normalizeBundleLimit(
    config.maxHermesFabricRoutineRunCharacters,
    DEFAULT_MAX_HERMES_FABRIC_ROUTINE_RUN_CHARS,
  );
  ...
  return {
    ...
    costCentsPerThousandSourceCharacters: normalizeCostRate(config.fabricCostCentsPerThousandSourceCharacters),
  };
}

async function resolveHermesFabricDistillationLimitsForSpace(
  ctx: PluginContext,
  input: Pick<HermesFabricSourceBundleInput, "companyId" | "maxCharacters" | "maxCharactersPerSource" | "routineRun"> & { space: WikiSpace },
): Promise<HermesFabricDistillationLimits> {
  const [base, profile] = await Promise.all([
    resolveHermesFabricDistillationLimits(ctx, input),
...
function defaultHermesFabricIngestionProfile(input: {
  space: Pick<WikiSpace, "slug">;
  legacySettings?: WikiEventIngestionSettings | null;
}): HermesFabricIngestionProfileV1 {
  ...
  return {
    ...
    cursor: {
      maxWindowCharacters: DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS,
      maxCharactersPerSource: DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS,
...
function normalizeHermesFabricIngestionSourceScope(value: unknown): HermesFabricIngestionSourceScope | null {
  ...
  if (kind === "active_projects") {
    ...
    return {
      kind,
      limit: normalizeLimit(record.limit, 3, MAX_HERMES_FABRIC_PROFILE_SELECTED_PROJECTS),
      ...(statuses && statuses.length > 0 ? { statuses: [...new Set(statuses)] } : {}),
    };
  }
  if (kind === "selected_projects") {
    return { kind, projectIds: stringArray(record.projectIds).slice(0, MAX_HERMES_FABRIC_PROFILE_SELECTED_PROJECTS) };
  }
  if (kind === "root_issues") {
    return { kind, issueIds: stringArray(record.issueIds).slice(0, MAX_HERMES_FABRIC_PROFILE_ROOT_ISSUES) };
  }
  if (kind === "company_all") {
...
function normalizeHermesFabricIngestionProfile(
  value: unknown,
  input: { space: Pick<WikiSpace, "slug">; legacySettings?: WikiEventIngestionSettings | null },
): HermesFabricIngestionProfileV1 {
  const fallback = defaultHermesFabricIngestionProfile(input);
  ...
  return {
    ...
    sourceScopes: Array.isArray(record.sourceScopes)
      ? record.sourceScopes.map(normalizeHermesFabricIngestionSourceScope).filter((scope): scope is HermesFabricIngestionSourceScope => Boolean(scope))
    ...
    cursor: {
      maxWindowCharacters: normalizeLimit(cursor.maxWindowCharacters, fallback.cursor.maxWindowCharacters, DEFAULT_MAX_HERMES_FABRIC_CURSOR_WINDOW_CHARS),
      maxCharactersPerSource: normalizeLimit(cursor.maxCharactersPerSource, fallback.cursor.maxCharactersPerSource, DEFAULT_MAX_HERMES_FABRIC_ISSUE_SOURCE_CHARS),
...
async function profileForSpace(ctx: PluginContext, companyId: string, space: WikiSpace): Promise<HermesFabricIngestionProfileV1> {
  const legacySettings = space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, companyId) : null;
  return normalizeHermesFabricIngestionProfile(space.settings.fabricIngestion, { space, legacySettings });
}
...
function evaluateHermesFabricProfilePolicy(input: {
  space: WikiSpace;
  profile?: HermesFabricIngestionProfileV1 | null;
  purpose: HermesFabricIngestionPolicyPurpose;
  requireEnabledProfile?: boolean;
}): HermesFabricIngestionPolicyDecision {
  ...
  if (space.status !== "active") {
    return {
      ...
      message: `HermesFabric ingestion policy denied ${purpose}: space "${space.slug}" is ${space.status}.`,
    };
  }
  if (space.accessScope !== "shared") {
    return {
      ...
      message: `HermesFabric ingestion policy denied ${purpose}: ${space.accessScope} spaces cannot ingest HermesFabric sources until host permissions are enforced.`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && !profile?.enabled) {
    return {
      ...
      message: `HermesFabric ingestion policy denied ${purpose}: HermesFabric ingestion is not enabled for space "${space.slug}".`,
    };
  }
  if (input.requireEnabledProfile && space.slug !== DEFAULT_SPACE_SLUG && profile?.enabled && profile.sourceScopes.length === 0) {
    return {
      ...
      message: `HermesFabric ingestion policy denied ${purpose}: space "${space.slug}" has no source scopes configured.`,
    };
  }
  return { allowed: true, space };
}

export async function getHermesFabricIngestionProfile(
  ctx: PluginContext,
  input: { companyId: string; wikiId?: string | null; spaceSlug?: string | null },
): Promise<HermesFabricIngestionProfileRead> {
  ...
  const profile = await profileForSpace(ctx, input.companyId, space);
  const policy = evaluateHermesFabricProfilePolicy({ space, profile, purpose: "profile_read" });
  const historicalPageCount = await countHermesFabricHistoricalPages(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceId: space.id,
  });
  const overlapCount = await countHermesFabricProfileOverlaps(ctx, {
    companyId: input.companyId,
    wikiId,
    space,
    profile,
  });
  const effectiveState: HermesFabricIngestionProfileEffectiveState = !policy.allowed
    ? "policy_blocked"
...
async function countHermesFabricHistoricalPages(ctx: PluginContext, input: { companyId: string; wikiId: string; spaceId: string }): Promise<number> {
...
function scopeIdentity(scope: HermesFabricIngestionSourceScope): string[] {
...
async function countHermesFabricProfileOverlaps(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  profile: HermesFabricIngestionProfileV1;
...
async function validateHermesFabricIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  space: WikiSpace;
  profile: HermesFabricIngestionProfileV1;
}) {
  const policy = evaluateHermesFabricProfilePolicy({
    space: input.space,
  ...
  if (input.profile.enabled && input.profile.sourceScopes.length === 0) {
    throw new Error("HermesFabric ingestion profile must include at least one source scope before it can be enabled.");
  }
  if (input.profile.sourceScopes.length > MAX_HERMES_FABRIC_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`HermesFabric ingestion profile sources exceed the hard cap of ${MAX_HERMES_FABRIC_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  for (const scope of input.profile.sourceScopes) {
    ...
    if (scope.kind === "selected_projects") {
      if (scope.projectIds.length > MAX_HERMES_FABRIC_PROFILE_SELECTED_PROJECTS) {
        throw new Error(`selected_projects exceeds the hard cap of ${MAX_HERMES_FABRIC_PROFILE_SELECTED_PROJECTS}.`);
      }
    ...
    if (scope.kind === "root_issues") {
      if (scope.issueIds.length > MAX_HERMES_FABRIC_PROFILE_ROOT_ISSUES) {
        throw new Error(`root_issues exceeds the hard cap of ${MAX_HERMES_FABRIC_PROFILE_ROOT_ISSUES}.`);
      }
...
export async function updateHermesFabricIngestionProfile(ctx: PluginContext, input: {
  companyId: string;
  wikiId?: string | null;
  spaceSlug?: string | null;
  profile: unknown;
  ...
  const profile = normalizeHermesFabricIngestionProfile(input.profile, { space, legacySettings: space.slug === DEFAULT_SPACE_SLUG ? await getEventIngestionSettings(ctx, input.companyId) : null });
  await validateHermesFabricIngestionProfile(ctx, { companyId: input.companyId, space, profile });
  await updateSpace(ctx, {
    companyId: input.companyId,
    wikiId,
    spaceSlug: space.slug,
    settings: { fabricIngestion: profile },
  });
  ...
  await ctx.activity.log({
    companyId: input.companyId,
    message: `Updated HermesFabric ingestion profile for ${space.displayName}`,
    entityType: "llm_wiki_space",
    entityId: space.id,
    metadata: {
      type: "plugin.llm_wiki.fabric_ingestion_profile_updated",
      wikiId,
      spaceSlug: space.slug,
  ...
  return getHermesFabricIngestionProfile(ctx, { companyId: input.companyId, wikiId, spaceSlug: space.slug });
}

export async function listHermesFabricIngestionCandidates(ctx: PluginContext, input: HermesFabricIngestionCandidatesInput): Promise<{
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search");
  ...
  await requireHermesFabricIngestionPolicy(ctx, {
    companyId: input.companyId,
  ...
  if (sourceKeys.length > MAX_HERMES_FABRIC_INGESTION_PROFILE_SOURCE_COUNT) {
    throw new Error(`HermesFabric ingestion profile sources exceed the hard cap of ${MAX_HERMES_FABRIC_INGESTION_PROFILE_SOURCE_COUNT}.`);
  }
  ...
  const profile = normalizeHermesFabricIngestionProfile(
    {
      ...defaultHermesFabricIngestionProfile({ space: defaultSpace, legacySettings: next }),
      ...
      cursor: {
        ...defaultHermesFabricIngestionProfile({ space: defaultSpace, legacySettings: next }).cursor,
...
function assertWikiPath(path: string, options: { allowMetadata?: boolean } = {}): string {
  ...
  if (
    ...
    !(options.allowMetadata && trimmed.startsWith(".fabric/"))
  ) {
...
function distillationCursorTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "fabric_distillation_cursors");
}

function distillationRunTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "fabric_distillation_runs");
}

function sourceSnapshotTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "fabric_source_snapshots");
}

function distillationWorkItemTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "fabric_distillation_work_items");
}

function pageBindingTable(ctx: PluginContext): string {
  return tableName(ctx.db.namespace, "fabric_page_bindings");
}
...
function operationPromptWithSpaceContext(input: OperationSpaceContext): string {
  const fabricDerived = input.operationType === "distill" || input.operationType === "backfill";
  return [
    ...
    fabricDerived
      ? "- HermesFabric-derived distill/backfill operations are default-space-only in Phase 1. Stop and comment if asked to write HermesFabric-derived pages into a non-default space."
...
export async function createOperationIssue(ctx: PluginContext, input: OperationInput) {
  ...
  const space = input.operationType === "distill" || input.operationType === "backfill"
    ? await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true })
...
function fabricDistillationScope(input: { projectId?: string | null; rootIssueId?: string | null }): HermesFabricDistillationScope {
  if (input.rootIssueId) return "root_issue";
  if (input.projectId) return "project";
  return "company";
}

function fabricCursorScopeMetadata(input: { projectId?: string | null; rootIssueId?: string | null }) {
  const sourceScope = fabricDistillationScope(input);
...
async function upsertHermesFabricDistillationCursor(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  spaceId: string;
  projectId?: string | null;
  ...
  const scope = fabricCursorScopeMetadata(input);
  await ctx.db.execute(
    `INSERT INTO ${distillationCursorTable(ctx)} AS fabric_distillation_cursors
     ...
     VALUES ($1, $2, $3, $11, $4, $5, $6, $7, 'fabric_issue_history', $8::timestamptz, $9, $10::jsonb)
     ON CONFLICT (company_id, wiki_id, space_id, source_scope, scope_key, source_kind)
     DO UPDATE SET last_observed_at = GREATEST(
                       COALESCE(fabric_distillation_cursors.last_observed_at, EXCLUDED.last_observed_at),
                       COALESCE(EXCLUDED.last_observed_at, fabric_distillation_cursors.last_observed_at)
                     ),
                   pending_event_count = fabric_distillation_cursors.pending_event_count + EXCLUDED.pending_event_count,
                   metadata = fabric_distillation_cursors.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      cursorId,
      input.companyId,
  ...
  const rows = await ctx.db.query<{ id: string }>(
    `SELECT id
      ...
      WHERE company_id = $1
        ...
        AND source_scope = $4
        AND scope_key = $5
        AND source_kind = 'fabric_issue_history'
      LIMIT 1`,
...
export async function enableActiveProjectDistillation(ctx: PluginContext, input: {
  ...
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "candidate_search", { requireEnabledProfile: true });
  if (typeof input.limit === "number" && Number.isFinite(input.limit) && Math.floor(input.limit) > MAX_HERMES_FABRIC_DISTILLATION_FAN_OUT) {
    throw new Error(`HermesFabric ingestion fan-out exceeds the hard cap of ${MAX_HERMES_FABRIC_DISTILLATION_FAN_OUT} enabled profiles.`);
  }
  ...
  for (const project of activeProjects) {
    ...
    const cursorId = await upsertHermesFabricDistillationCursor(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceId: space.id,
...
function appendBoundedSection(input: {
  ...
  title: string;
  body: string;
  refs: HermesFabricSourceRef[];
  ref: HermesFabricSourceRef;
...
function sourceRefUpdatedAt(ref: HermesFabricSourceRef): string | null {
  return ref.updatedAt ?? ref.createdAt ?? null;
}

function issueInBackfillWindow(issue: Issue, input: Pick<HermesFabricSourceBundleInput, "backfillStartAt" | "backfillEndAt">): boolean {
...
async function listHermesFabricBundleIssues(ctx: PluginContext, input: HermesFabricSourceBundleInput): Promise<Issue[]> {
...
export async function assembleHermesFabricSourceBundle(ctx: PluginContext, input: HermesFabricSourceBundleInput): Promise<HermesFabricSourceBundle> {
  const wikiId = normalizeWikiId(input.wikiId);
  assertHermesFabricSourceScopePayload(input);
  const space = await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const limits = await resolveHermesFabricDistillationLimitsForSpace(ctx, { ...input, space });
  ...
  const issues = await listHermesFabricBundleIssues(ctx, input);
  const scope = fabricCursorScopeMetadata(input);
  const sourceRefs: HermesFabricSourceRef[] = [];
  const warnings: string[] = [];
  const lines = [
    `# HermesFabric source bundle`,
    "",
    "## Bundle Metadata",
    "",
...
export async function createHermesFabricDistillationRun(ctx: PluginContext, input: HermesFabricDistillationRunInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertHermesFabricSourceScopePayload(input);
  const space = await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
  const scope = fabricCursorScopeMetadata(input);
  ...
  const bundle = await assembleHermesFabricSourceBundle(ctx, input);
...
export async function recordHermesFabricDistillationOutcome(ctx: PluginContext, input: HermesFabricDistillationOutcomeInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  const space = await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "execute", { requireEnabledProfile: true });
...
export async function createHermesFabricDistillationWorkItem(ctx: PluginContext, input: HermesFabricDistillationWorkItemInput) {
  const wikiId = normalizeWikiId(input.wikiId);
  assertHermesFabricSourceScopePayload(input);
  const space = await requireHermesFabricIngestionPolicy(ctx, { companyId: input.companyId, wikiId, spaceSlug: input.spaceSlug }, "queue", { requireEnabledProfile: true });
  ...
  await ctx.db.execute(
    `INSERT INTO ${distillationWorkItemTable(ctx)} AS fabric_distillation_work_items
     ...
     DO UPDATE SET priority = EXCLUDED.priority,
                   metadata = fabric_distillation_work_items.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      itemId,
      input.companyId,
...
function sourceRefLabel(ref: HermesFabricSourceRef): string {
  const issue = ref.issueIdentifier ? issueReference(ref.issueIdentifier) : (ref.title ?? "HermesFabric source");
  if (ref.kind === "document") return `${issue} document:${ref.documentKey ?? "unknown"}`;
  if (ref.kind === "comment") return `${issue} comment`;
  return issue;
}

function sourceRefMarkdown(ref: HermesFabricSourceRef): string {
  const metadata = [
...
function issueSourceRef(issue: Issue): HermesFabricSourceRef {
  return {
    kind: "issue",
    issueId: issue.id,
    issueIdentifier: issue.identifier ?? null,
...
function projectPageSlug(input: { project: Project | null; rootIssue: Issue | null }): string {
  return slugify(input.project?.name ?? input.rootIssue?.title ?? "fabric-project");
}
...
function hasDurableSignal(bundle: HermesFabricSourceBundle, issues: Issue[]): boolean {
...
function standupPageContents(input: {
  ...
  issues: Issue[];
  bundle: HermesFabricSourceBundle;
  pagePath: string;
  durablePagePath: string;
}): string {
  const currentAsOf = input.bundle.sourceWindowEnd ?? new Date().toISOString();
  const title = input.project?.name ?? input.rootIssue?.title ?? "HermesFabric Project";
  ...
  return [
    ...
    "",
    lead
      ? `The current center of gravity is **${issueConcept(lead)}** (${issueReferenceFor(lead)}). ${input.bundle.clipped ? "The source window was clipped, so treat this as a bounded readout rather than the full live state." : "This is a high-level readout of the meaningful HermesFabric work in the current source window."}`
...
function projectPageContents(input: {
  ...
  return [
    ...
    "## Overview",
    "",
    description ? excerpt(description, 700) : `This page synthesizes HermesFabric issue history into a stable project brief for ${title}.`,
    "",
    "## Current Direction",
    "",
    activeIssues.length
    ...
    input.bundle.clipped ? "\nThe source window was clipped, so verify HermesFabric before treating this as complete state." : null,
    "",
    "## Workstreams",
    "",
    ...(recentIssues.length
...
function decisionsPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: HermesFabricSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "HermesFabric Project";
...
function historyPageContents(input: { project: Project | null; rootIssue: Issue | null; issues: Issue[]; bundle: HermesFabricSourceBundle }): string {
  const title = input.project?.name ?? input.rootIssue?.title ?? "HermesFabric Project";
...
function appendProjectLogContents(current: string | null, input: { standupPath: string; pagePath: string; runId: string; sourceHash: string; status: string; warnings: string[] }): string {
  ...
  const warningLines = input.warnings.length
    ...
    : ["- warnings: none"];
  const entry = [
    `## [${new Date().toISOString().slice(0, 10)}] fabric-distill | ${input.status}`,
...
async function upsertPageBinding(ctx: PluginContext, input: {
  ...
  await ctx.db.execute(
    `INSERT INTO ${pageBindingTable(ctx)} AS fabric_page_bindings
     ...
     DO UPDATE SET last_applied_source_hash = EXCLUDED.last_applied_source_hash,
                   ...
                   metadata = fabric_page_bindings.metadata || EXCLUDED.metadata,
                   updated_at = now()`,
    [
      randomUUID(),
      input.companyId,
...
export function getDistillationAutoApplyRestriction(): DistillationAutoApplyRestriction {
  const rawMode = process.env.HERMES_FABRIC_DEPLOYMENT_MODE;
  const rawExposure = process.env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE;
...
export async function distillHermesFabricProjectPage(ctx: PluginContext, input: HermesFabricProjectPageDistillationInput) {
  if (!input.projectId && !input.rootIssueId) {
  ...
  const run = await createHermesFabricDistillationRun(ctx, input);
  ...
  if (!hasDurableSignal(bundle, issues)) {
    await recordHermesFabricDistillationOutcome(ctx, {
      companyId: input.companyId,
      wikiId,
      spaceSlug: space.slug,
      runId: run.runId,
      ...
      warning: "Skipped low-signal HermesFabric source window.",
    });
    return {
      status: "skipped",
      reason: "low_signal",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped low-signal HermesFabric source window."],
      patches: [] as HermesFabricDistillationPatch[],
    };
  }
  ...
  if (existingBinding?.last_applied_source_hash === bundle.sourceHash) {
    await recordHermesFabricDistillationOutcome(ctx, {
      ...
      sourceHash: bundle.sourceHash,
      sourceWindowEnd: bundle.sourceWindowEnd,
      warning: "Skipped unchanged HermesFabric source hash.",
    });
    return {
      status: "skipped",
      ...
      cursorId: run.cursorId,
      sourceHash: bundle.sourceHash,
      warnings: ["Skipped unchanged HermesFabric source hash."],
      patches: [] as HermesFabricDistillationPatch[],
    };
  }
  ...
  const title = project?.name ?? rootIssue?.title ?? "HermesFabric Project";
  ...
  const patches: HermesFabricDistillationPatch[] = [
  ...
  for (const patch of patches) {
    await writeWikiPage(ctx, {
      ...
      summary: `HermesFabric distillation ${patch.operationType} from ${bundle.sourceHash}`,
...
function rawPathForHermesFabricEvent(input: {
  ...
  return assertRawPath(`raw/fabric/${input.sourceKind}/${eventDate}-${slugify(identifier)}-${slugify(input.label)}-${contentHash(input.contents).slice(0, 8)}.md`);
}

function formatIssueEventSource(issue: Issue, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# HermesFabric issue: ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
...
function formatCommentEventSource(issue: Issue, comment: IssueComment, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# HermesFabric comment on ${sourceTitleForIssue(issue)}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
function formatDocumentEventSource(issue: Issue, document: IssueDocument, event: PluginEvent, maxCharacters: number): string {
  return truncateEventSource([
    `# HermesFabric document: ${document.title ?? document.key}`,
    "",
    "## Provenance",
    "",
    `- Company ID: ${issue.companyId}`,
...
async function recordHermesFabricCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  wikiId: string;
  space: WikiSpace;
  sourceKind: WikiEventIngestionSource;
  ...
  issue: Issue;
  event: PluginEvent;
}): Promise<Extract<HermesFabricEventIngestResult, { status: "recorded" }>> {
  const cursorId = await upsertHermesFabricDistillationCursor(ctx, {
    companyId: input.companyId,
    wikiId: input.wikiId,
    spaceId: input.space.id,
...
async function fabricProfileIncludesIssue(ctx: PluginContext, input: {
  companyId: string;
  issue: Issue;
  profile: HermesFabricIngestionProfileV1;
}): Promise<boolean> {
...
async function routeHermesFabricCursorObservation(ctx: PluginContext, input: {
  companyId: string;
  sourceKind: WikiEventIngestionSource;
  sourceId: string;
  issue: Issue;
  event: PluginEvent;
}): Promise<HermesFabricEventIngestResult> {
  const { spaces } = await listSpaces(ctx, { companyId: input.companyId, wikiId: DEFAULT_WIKI_ID });
  const recorded: Array<Extract<HermesFabricEventIngestResult, { status: "recorded" }>> = [];
  let eligibleProfileCount = 0;
  for (const space of spaces) {
    const profile = await profileForSpace(ctx, input.companyId, space);
    if (!profile.enabled) continue;
    const policy = evaluateHermesFabricProfilePolicy({ space, profile, purpose: "event_routing", requireEnabledProfile: true });
    if (!policy.allowed) continue;
    if (!profile.sourceKinds[input.sourceKind]) continue;
    if (!(await fabricProfileIncludesIssue(ctx, { companyId: input.companyId, issue: input.issue, profile }))) continue;
    eligibleProfileCount += 1;
    if (eligibleProfileCount > MAX_HERMES_FABRIC_DISTILLATION_FAN_OUT) {
      throw new Error(`HermesFabric ingestion fan-out exceeds the hard cap of ${MAX_HERMES_FABRIC_DISTILLATION_FAN_OUT} enabled profiles.`);
    }
    if (await ctx.state.get(eventIngestionDedupKey(input.companyId, space.wikiId, space.id, input.sourceKind, input.sourceId))) {
      continue;
    }
    recorded.push(await recordHermesFabricCursorObservation(ctx, {
      ...input,
      wikiId: space.wikiId,
      space,
    }));
...
export async function handleHermesFabricEventIngestion(ctx: PluginContext, event: PluginEvent): Promise<HermesFabricEventIngestResult> {
  ...
  if (event.eventType === "issue.created" || event.eventType === "issue.updated") {
    ...
    return routeHermesFabricCursorObservation(ctx, {
      companyId,
      sourceKind: "issues",
      sourceId,
      issue,
