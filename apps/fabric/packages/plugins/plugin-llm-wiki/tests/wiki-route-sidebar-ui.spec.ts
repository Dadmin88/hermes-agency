...
const EXPANDED_STORAGE_KEY = `hermes-fabric.plugin-llm-wiki:route-sidebar-expanded:v2:${COMPANY_ID}`;
...
describe("WikiRouteSidebar", () => {
  ...
  beforeEach(() => {
    ...
    (globalThis as BridgeGlobal).__fabricPluginBridge__ = {
      sdkUi: {
  ...
  afterEach(() => {
    ...
    container.remove();
    window.localStorage.clear();
    delete (globalThis as BridgeGlobal).__fabricPluginBridge__;
  });
  ...
  it("keeps sidebar tree selection scoped to sidebar navigation", () => {
    ...
    expect(navigatedTo).toEqual({
      ...
      options: { state: { fabricWikiSidebarTreePath: "wiki/concepts/sidebar-navigation.md" } },
    });
    ...
    hostLocation = {
      pathname: "/PAP/wiki/page/wiki/entities/fabric.md",
      search: "",
      hash: "",
    };
    act(() => {
...
describe("WikiPage", () => {
  ...
  it("renders structured HermesFabric source refs as text", () => {
    act(() => {
