...
here rather than in Hermes Fabric core.
...
- opt-in company-scoped Hermes Fabric event ingestion controls for issues, comments, and documents; event ingestion is disabled by default and routes captured raw provenance into the default space only
- manual Hermes Fabric project/root issue distillation and bounded backfill actions with explicit work items, operation issues, source caps, and estimated cost recording
- Hermes Fabric-derived distillation (cursor windows, manual `distill-now`, backfill) always writes into the default wiki space in Phase 1; non-default spaces remain on manual / raw-file ingest until per-space Hermes Fabric ingestion profiles ship
- Hermes Fabric-derived distillation maintains `wiki/projects/<slug>/standup.md` as the executive current-state view for each represented project, alongside durable `wiki/projects/<slug>/index.md` knowledge pages
...
Hermes Fabric-derived text ingestion stays limited to issue titles/descriptions, issue comments, and issue documents.
...
The accepted policy lives in [doc/plans/2026-05-06-llm-wiki-Hermes Fabric-asset-security-gate.md](../../../doc/plans/2026-05-06-llm-wiki-Hermes Fabric-asset-security-gate.md).
...
From the Hermes Fabric repo root:

```bash
pnpm --filter @hermes-fabric/plugin-llm-wiki typecheck
pnpm --filter @hermes-fabric/plugin-llm-wiki test
pnpm --filter @hermes-fabric/plugin-llm-wiki build
```

## Alpha Verification

Run these commands from the Hermes Fabric repo root before handing off alpha plugin
changes:
...
- disabled and enabled Hermes Fabric event ingestion paths
...
- Host-level plugin install and live agent invocation still need Hermes Fabric
...
curl -X POST http://127.0.0.1:3100/api/plugins/install \
  ...
  -d '{"packageName":"/Users/dotta/fabric/.fabric/worktrees/PAP-3179-design-a-llm-wiki-plugin/packages/plugins/plugin-llm-wiki","isLocalPath":true}'
```
...
The package-local `dist/` directory is ignored by git, but local Hermes Fabric
...
restart the Hermes Fabric dev server so the host imports the fresh bundle.
...
write the starter files. The plugin uses Hermes Fabric's local folder API for path
...
`CLAUDE.md` renamed to `AGENTS.md` and Hermes Fabric project overviews, standups,
