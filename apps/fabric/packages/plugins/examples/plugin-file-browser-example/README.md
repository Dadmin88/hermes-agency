...
Example Hermes Fabric plugin that demonstrates:
...
```bash
pnpm --filter @hermes-fabric/plugin-file-browser-example build
pnpm hermes-fabric plugin install ./packages/plugins/examples/plugin-file-browser-example
```

To uninstall:

```bash
pnpm hermes-fabric plugin uninstall fabric-file-browser-example --force
```
...
- Optional: use `fabric-plugin-dev-server` for UI hot-reload with `devUiUrl` in plugin config.
