...
describe("buildSandboxCrManifest", () => {
  ...
  it("applies provided labels to CR metadata and pod template labels (with role=agent added)", () => {
    const cr = buildSandboxCrManifest(baseInput);
    expect(cr.metadata.labels["fabric.io/run-id"]).toBe("r1");
    expect(
      cr.spec.podTemplate.metadata.labels["fabric.io/run-id"],
    ).toBe("r1");
    expect(cr.spec.podTemplate.metadata.labels["fabric.io/role"]).toBe(
      "agent",
    );
  });
