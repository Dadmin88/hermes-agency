...
describe("Novita sandbox provider plugin", () => {
  ...
  it("parses defaults", () => {
    expect(parseNovitaDriverConfig({})).toMatchObject({
      ...
      domain: null,
      template: null,
      requestedCwd: "/home/user/fabric-workspace",
      timeoutMs: 300_000,
  ...
  it("builds a quoted shell command with cwd, env, args, and stdin", () => {
    ...
    expect(command).toContain("printf '%s' 'input body' > '/tmp/.fabric-stdin-");
    expect(command).toMatch(/< '\/tmp\/\.fabric-stdin-[^']+'/);
    expect(command).toMatch(/rm -f '\/tmp\/\.fabric-stdin-[^']+'/);
  ...
  it("uses a unique stdin path for each command", () => {
    ...
    const firstPath = first.match(/\/tmp\/\.fabric-stdin-[^']+/)?.[0];
    const secondPath = second.match(/\/tmp\/\.fabric-stdin-[^']+/)?.[0];

    expect(firstPath).toBeTruthy();
    expect(secondPath).toBeTruthy();
    expect(firstPath).not.toBe(secondPath);
    expect(first).not.toContain("/tmp/.fabric-stdin <<");
    expect(second).not.toContain("/tmp/.fabric-stdin <<");
  });
