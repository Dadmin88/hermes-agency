...
const PLUGIN_ID = "fabric.daytona-sandbox-provider";
...
const manifest: HermesFabricPluginManifestV1 = {
  ...
  description:
    "First-party sandbox provider plugin that provisions Daytona sandboxes as HermesFabric execution environments.",
      ...
      configSchema: {
        ...
        properties: {
          apiKey: {
            ...
            format: "secret-ref",
            description:
              "Environment-specific Daytona API key. Paste a key or an existing HermesFabric secret reference; saved environments store pasted values as company secrets. Falls back to DAYTONA_API_KEY if omitted.",
          },
          apiUrl: {
            type: "string",
            description:
