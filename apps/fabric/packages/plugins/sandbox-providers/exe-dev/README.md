# `@hermes-fabric/plugin-exe-dev`

Published exe.dev sandbox provider plugin for Hermes Fabric.

This package lives in the Hermes Fabric monorepo, but it is intentionally excluded from the root `pnpm` workspace and shaped to publish and install like a standalone npm package. That lets operators install it from the Plugins page by package name without introducing root lockfile churn.
...
- When you save an environment, Hermes Fabric stores pasted API keys and pasted SSH private keys as company secrets.
...
- SSH access from the Hermes Fabric host to the resulting `*.exe.xyz` VMs.
