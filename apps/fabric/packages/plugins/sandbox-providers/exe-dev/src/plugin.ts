...
// has passwordless sudo. The HermesFabric sandbox callback bridge is a Node script, so
// every HermesFabric workload on this provider needs node on PATH before the bridge can
// start. When the operator hasn't supplied their own setup script, install Node 20 via
// nodesource so the VM comes up ready for HermesFabric out of the box.
...
function normalizeNamePrefix(value: string | null): string {
  const normalized = (value ?? "fabric")
    .toLowerCase()
    .replace(/[^a-z0-9-]+/g, "-")
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
  return normalized.length > 0 ? normalized.slice(0, 24) : "fabric";
}
...
async function prepareSshIdentity(config: ExeDevDriverConfig): Promise<{
  ...
  const tempDir = await mkdtemp(path.join(tmpdir(), "fabric-exe-dev-ssh-"));
...
function formatSshFailure(
  ...
  if (
    ...
    return [
      `Failed to ${action} exe.dev VM ${vmName}: the HermesFabric host SSH key is not registered with exe.dev.`,
...
async function buildLease(
  ...
  const remoteCwd = requestedCwd?.trim() || path.posix.join(remote.homeDir, "fabric-workspace");
