...
function buildHeaders(config: CloudflareDriverConfig, extra: BridgeClientHeaders = {}): Headers {
  ...
  if (extra.environmentId) headers.set("X-HermesFabric-Environment-Id", extra.environmentId);
  if (extra.runId) headers.set("X-HermesFabric-Run-Id", extra.runId);
  if (extra.issueId) headers.set("X-HermesFabric-Issue-Id", extra.issueId);
  return headers;
}
...
export function createCloudflareBridgeClient(options: BridgeClientOptions) {
  ...
  const apiPrefix = "/api/fabric-sandbox/v1";
