...
const DEFAULT_WORKDIR = "/workspace/fabric";
...
function resolveAuth(config: ModalDriverConfig): { tokenId: string; tokenSecret: string } | null {
  ...
  // environment config, which HermesFabric stores as company secrets.
  ...
  async onEnvironmentProbe(
    ...
    try {
      ...
      try {
        ...
        const proc = await sandbox.exec(["sh", "-lc", "printf fabric-probe"]);
        const { stdout, exitCode } = await readProcessStreams(proc);
        if (exitCode !== 0 || stdout.trim() !== "fabric-probe") {
          return {
            ok: false,
  ...
  async onEnvironmentExecute(
    ...
    try {
      ...
      const stdinPath = params.stdin != null
        ? `/tmp/fabric-stdin-${Date.now()}-${Math.random().toString(36).slice(2, 10)}`
        : null;
      try {
