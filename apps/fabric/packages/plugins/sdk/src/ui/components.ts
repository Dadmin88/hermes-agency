 ...
 *
 * These components are exported from `@hermes-fabric/plugin-sdk/ui` and are
 ...
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components In `@hermes-fabric/plugin-sdk/ui`
 */
...
function createSdkUiComponent<TProps>(name: string): React.ComponentType<TProps> {
  return function HermesFabricSdkUiComponent(props: TProps) {
...
/**
 * Renders HermesFabric's shared Markdown editor.
 *
 * @see PLUGIN_SPEC.md §19.6 — Shared Components
 */
...
/**
 * Renders HermesFabric's native issue list component for company-scoped plugin
...
/**
 * Renders HermesFabric's native managed routines list for plugin settings pages.
 */
