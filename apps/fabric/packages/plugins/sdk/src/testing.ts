...
function requireCapability(manifest: HermesFabricPluginManifestV1, allowed: Set<PluginCapability>, capability: PluginCapability) {
 ...
 * can validate plugin behavior without spinning up the HermesFabric server runtime.
 */
export function createTestHarness(options: TestHarnessOptions): TestHarness {
  ...
  function isManagedAgent(agent: Agent, agentKey: string) {
    const marker = agent.metadata?.fabricManagedResource;
    return Boolean(
      marker
