...
describe("isWorkerEntrypoint", () => {
  ...
  function createTempRoot(): string {
    const tempRoot = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-sdk-worker-"));
  ...
  it("keeps overlapping company scopes local to each getData invocation", async () => {
    ...
    function callWorker(method: string, params: unknown, invocation?: PluginInvocationContext) {
      ...
      const request = {
        ...
        ...(invocation ? { fabricInvocation: invocation } : {}),
      };
      const result = new Promise<unknown>((resolve, reject) => {
        pending.set(id, (response) => {
          if ("error" in response && response.error) {
    ...
    hostReadline.on("line", (line) => {
      ...
      const invocationId = (message as { fabricInvocationId?: string }).fabricInvocationId ?? "";
