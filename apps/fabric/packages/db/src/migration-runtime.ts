...
async function ensureEmbeddedPostgresConnection(
  ...
  const preferredAdminConnectionString = `postgres://fabric:fabric@127.0.0.1:${preferredPort}/postgres`;
  ...
  if (!runningPid && existsSync(pgVersionFile)) {
    try {
      ...
      await ensurePostgresDatabase(preferredAdminConnectionString, "fabric");
      process.emitWarning(
      ...
      return {
        connectionString: `postgres://fabric:fabric@127.0.0.1:${preferredPort}/fabric`,
  ...
  if (runningPid) {
    ...
    const adminConnectionString = `postgres://fabric:fabric@127.0.0.1:${port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fabric");
    return {
      connectionString: `postgres://fabric:fabric@127.0.0.1:${port}/fabric`,
  ...
  const adminConnectionString = `postgres://fabric:fabric@127.0.0.1:${selectedPort}/postgres`;
  await ensurePostgresDatabase(adminConnectionString, "fabric");

  return {
    connectionString: `postgres://fabric:fabric@127.0.0.1:${selectedPort}/fabric`,
