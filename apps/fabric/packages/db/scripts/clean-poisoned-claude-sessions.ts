 ...
 *     --config /path/to/fabric/config.json \
 *     [--claude-config-dir ~/.claude] \
 *     [--dry-run] [--json]
 *
 * Or in a HermesFabric checkout shell:
 *   pnpm --filter @hermes-fabric/db exec tsx scripts/clean-poisoned-claude-sessions.ts --dry-run
 *
...
Flags:
  --config <path>            Path to fabric config.json (defaults to
                             $HERMES_FABRIC_HOME/instances/default/config.json or
...
function defaultConfigPath(): string | null {
  ...
  if (process.env.HERMES_FABRIC_HOME) {
    candidates.push(path.join(process.env.HERMES_FABRIC_HOME, "config.json"));
  }
  candidates.push(path.join(home, ".fabric", "instances", "default", "config.json"));
...
async function main(argv: string[]): Promise<void> {
  ...
  if (!databaseUrl) {
    throw new Error(
      "Unable to resolve database URL. Pass --database-url or --config <fabric config.json>.",
    );
  }
