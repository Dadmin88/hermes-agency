...
Use when:
- You want HermesFabric to run Cursor Cloud Agents through the official Cursor SDK
- You want durable remote Cursor agent sessions across HermesFabric heartbeats
- You want HermesFabric to keep task state while Cursor handles remote code execution
...
Notes:
- HermesFabric reuses the durable Cursor agent across heartbeats when the repo/runtime identity still matches.
- Each HermesFabric heartbeat maps to a Cursor run on that durable agent.
- HermesFabric injects HERMES_FABRIC_* runtime env vars into the cloud agent shell through Cursor SDK cloud envVars.
- HermesFabric remains the source of truth for issue/task state; Cursor provides the remote execution surface.
`;
