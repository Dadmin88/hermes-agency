...
describe("cursor_cloud execute", () => {
  ...
  it("creates a fresh Cursor agent and injects HermesFabric env without CURSOR_API_KEY", async () => {
    ...
    expect(createMock.mock.calls[0]?.[0]).toMatchObject({
      ...
      name: "HermesFabric Cursor Cloud Agent",
      model: { id: "gpt-5.4" },
      cloud: {
        env: { type: "cloud" },
        repos: [{ url: "https://github.com/DeployFaith/Hermes_Agency.git", startingRef: "main" }],
      },
    });
    expect(createMock.mock.calls[0]?.[0]?.cloud?.envVars).toMatchObject({
      EXTRA_FLAG: "1",
      HERMES_FABRIC_RUN_ID: "run-heartbeat-1",
      HERMES_FABRIC_TASK_ID: "issue-1",
      HERMES_FABRIC_WAKE_REASON: "issue_commented",
      HERMES_FABRIC_API_KEY: "fabric-run-jwt",
    });
  ...
  it("maps non-finished Cursor results to failing HermesFabric runs", async () => {
