...
import { readAdapterExecutionTarget, adapterExecutionTargetSessionIdentity } from "@hermes-fabric/adapter-utils/execution-target";
import {
  ...
  type HermesFabricSkillEntry,
} from "@hermes-fabric/adapter-utils/server-utils";
import { shellQuote } from "@hermes-fabric/adapter-utils/ssh";
import {
  createAcpRuntime,
  createAgentRegistry,
  createRuntimeStore,
...
const HERMES_FABRIC_MANAGED_CODEX_SKILLS_MANIFEST = ".fabric-managed-skills.json";
...
interface AcpxPreparedRuntime {
  ...
  fabricClaudeSettings: HermesFabricClaudeSettingsResult | null;
}
...
function defaultHermesFabricInstanceDir(): string {
  const home = process.env.HERMES_FABRIC_HOME?.trim() || path.join(os.homedir(), ".fabric");
  const instanceId = process.env.HERMES_FABRIC_INSTANCE_ID?.trim() || "default";
  return resolveHermesFabricInstanceRootForAdapter({
    homeDir: home,
    instanceId,
  });
}
...
function defaultStateDir(companyId: string, agentId: string): string {
  return path.join(defaultHermesFabricInstanceDir(), "companies", companyId, "acpx-local", "agents", agentId);
}

function resolveManagedCodexHomeDir(companyId: string): string {
  return path.join(defaultHermesFabricInstanceDir(), "companies", companyId, "codex-home");
}
...
async function prepareManagedCodexHome(input: {
  ...
  await onLog(
    "stdout",
    `[fabric] Using HermesFabric-managed ACPX Codex home "${targetHome}" (seeded from "${sourceHome}").\n`,
  );
  return targetHome;
}
...
async function buildSkillSetKey(input: {
  skills: HermesFabricSkillEntry[];
  label: string;
}): Promise<string> {
  const hash = createHash("sha256");
  hash.update(`fabric-acpx-${input.label}-skills:v1\n`);
...
): Promise<{ allSkills: HermesFabricSkillEntry[]; selectedSkills: HermesFabricSkillEntry[]; desiredSkillNames: string[] }> {
  const allSkills = await readHermesFabricRuntimeSkillEntries(config, __moduleDir);
  const desiredSkillNames = resolveHermesFabricDesiredSkillNames(config, allSkills);
...
async function prepareClaudeSkillRuntime(input: {
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      const result = await materializeHermesFabricSkillCopy(entry.source, target);
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[fabric] Materialized ACPX Claude skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[fabric] Failed to materialize ACPX Claude skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
  ...
  const promptInstructions = selectedSkills.length > 0
    ? [
        "HermesFabric has materialized selected runtime skills for this ACPX Claude session.",
  ...
  return {
    ...
    commandNotes: selectedSkills.length > 0
      ? [`Materialized ${selectedSkills.length} HermesFabric skill(s) for ACPX Claude at ${skillsHome}.`]
      : [],
  };
}

async function readManagedCodexSkillsManifest(skillsHome: string): Promise<Set<string>> {
  const manifestPath = path.join(skillsHome, HERMES_FABRIC_MANAGED_CODEX_SKILLS_MANIFEST);
  try {
...
async function writeManagedCodexSkillsManifest(skillsHome: string, skillNames: Iterable<string>): Promise<void> {
  ...
  await fs.writeFile(
    path.join(skillsHome, HERMES_FABRIC_MANAGED_CODEX_SKILLS_MANIFEST),
...
async function reconcileManagedCodexSkills(input: {
  skillsHome: string;
  allSkills: HermesFabricSkillEntry[];
  selectedSkills: HermesFabricSkillEntry[];
  onLog: AdapterExecutionContext["onLog"];
}): Promise<void> {
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[fabric] Revoked ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const entry of input.allSkills) {
    ...
    if (await removeSkillTarget(target)) {
      await input.onLog("stdout", `[fabric] Revoked legacy ACPX Codex skill "${entry.runtimeName}" from ${input.skillsHome}\n`);
    }
  }
  ...
  for (const name of managed) {
    ...
    if (await removeSkillTarget(path.join(input.skillsHome, name))) {
      await input.onLog("stdout", `[fabric] Revoked unavailable ACPX Codex skill "${name}" from ${input.skillsHome}\n`);
    }
  }
}
  ...
  for (const entry of selectedSkills) {
    ...
    try {
      ...
      if (result.skippedSymlinks.length > 0) {
        await input.onLog(
          "stdout",
          `[fabric] Materialized ACPX Codex skill "${entry.runtimeName}" into ${skillsHome} and skipped ${result.skippedSymlinks.length} symlink(s).\n`,
        );
      }
    } catch (err) {
      await input.onLog(
        "stderr",
        `[fabric] Failed to inject ACPX Codex skill "${entry.key}" into ${skillsHome}: ${err instanceof Error ? err.message : String(err)}\n`,
      );
    }
  }
...
interface HermesFabricClaudeSettingsResult {
  filePath: string;
  allow: string[];
...
// widen the SDK's Read sandbox to include the HermesFabric state dirs the agent
// needs to talk to its own control plane.
async function writeHermesFabricClaudeSettings(input: {
  cwd: string;
  stateDir: string;
  agentHome: string;
  companyId: string;
}): Promise<HermesFabricClaudeSettingsResult> {
  const filePath = path.join(input.cwd, ".claude", "settings.local.json");
  const instanceRoot = defaultHermesFabricInstanceDir();
  const companyRoot = path.join(instanceRoot, "companies", input.companyId);
  const fabricAdditionalDirectories = uniqueSorted([
    input.stateDir,
    input.agentHome,
    companyRoot,
  ]);
  const fabricAllow = uniqueSorted([
    "Bash(curl:*)",
    "Bash(env:*)",
    "Bash(env)",
    `Bash(${input.cwd}/scripts/fabric-issue-update.sh:*)`,
    `Bash(${input.cwd}/scripts/fabric:*)`,
  ]);
  ...
  const existingAdditionalDirectories = Array.isArray(existingPerms.additionalDirectories)
    ...
    : [];
  const mergedAllow = uniqueSorted([...existingAllow, ...fabricAllow]);
  const mergedAdditionalDirectories = uniqueSorted([
    ...existingAdditionalDirectories,
    ...fabricAdditionalDirectories,
  ]);
...
async function writeAgentWrapper(input: {
  ...
  const script = [
    ...
    "if [[ -n \"${HERMES_FABRIC_RUN_ID:-}\" ]]; then",
    "  mkdir -p \"$stderr_dir\"",
    "  exec 2> >(tee -a \"$stderr_dir/$HERMES_FABRIC_RUN_ID.log\" >&2)",
    "fi",
...
async function buildRuntime(input: {
  ...
  const env: Record<string, string> = { ...buildHermesFabricEnv(agent), HERMES_FABRIC_RUN_ID: runId };
  const wakeTaskId =
  ...
  applyHermesFabricWorkspaceEnv(env, {
  ...
  if (!hasExplicitApiKey && authToken) env.HERMES_FABRIC_API_KEY = authToken;
  ...
  let fabricClaudeSettings: HermesFabricClaudeSettingsResult | null = null;
  if (acpxAgent === "claude") {
    ...
    fabricClaudeSettings = await writeHermesFabricClaudeSettings({
      cwd,
      stateDir,
      agentHome,
    ...
    skillCommandNotes.push(
      `Wrote HermesFabric-managed Claude settings to ${fabricClaudeSettings.filePath} (defaultMode=${fabricClaudeSettings.defaultMode}${
        fabricClaudeSettings.overrodeDontAsk ? "; overrode user dontAsk" : ""
      }, +${fabricClaudeSettings.additionalDirectories.length} read root(s), +${fabricClaudeSettings.allow.length} allow rule(s)).`,
    );
    ...
    const desired = resolveHermesFabricDesiredSkillNames(config, await readHermesFabricRuntimeSkillEntries(config, __moduleDir));
    skillsIdentity = { mode: "custom_unsupported", desiredSkillNames: desired };
    if (desired.length > 0) {
      skillCommandNotes.push("Selected HermesFabric skills are tracked only; ACPX custom commands do not expose a runtime skill contract yet.");
    }
  }
  ...
  const fingerprint = shortHash({
    ...
    fabricClaudeSettings: fabricClaudeSettings
      ? {
          allow: fabricClaudeSettings.allow,
          additionalDirectories: fabricClaudeSettings.additionalDirectories,
          defaultMode: fabricClaudeSettings.defaultMode,
        }
      : null,
  });
  const taskKey = asString(input.ctx.runtime.taskKey, "") || wakeTaskId || workspaceId || "default";
  const sessionKey = `fabric:${agent.companyId}:${agent.id}:${taskKey}:${fingerprint}`;
...
async function applySessionConfigOptions(input: {
  ...
  if (!input.runtime.setConfigOption) {
    ...
    await input.onLog("stderr", `[fabric] ${message}\n`);
  ...
  for (const option of options) {
    ...
    await input.onLog(
      "stdout",
      `[fabric] Applied ACPX ${input.prepared.acpxAgent} config ${option.key}=${option.value}\n`,
    );
  }
}
...
async function buildPrompt(ctx: AdapterExecutionContext, resumedSession: boolean): Promise<{
  ...
  const promptTemplate = asString(config.promptTemplate, DEFAULT_HERMES_FABRIC_AGENT_PROMPT_TEMPLATE);
  ...
  const renderedBootstrapPrompt =
    !resumedSession && bootstrapPromptTemplate.trim().length > 0
      ...
      : "";
  const wakePrompt = renderHermesFabricWakePrompt(context.fabricWake, { resumedSession });
  ...
  const taskContextNote = asString(context.fabricTaskMarkdown, "").trim();
...
async function emitAcpxFailure(input: {
  ...
  if (childStderrTail) {
    await ctx.onLog(
      "stderr",
      `[fabric] ACPX child stderr tail (${phase}):\n${childStderrTail}\n`,
    );
  }
  await emitAcpxLog(ctx, {
    type: "acpx.error",
...
export function createAcpxLocalExecutor(deps: ExecuteDeps = {}) {
  ...
  return async function executeAcpxLocal(ctx: AdapterExecutionContext): Promise<AdapterExecutionResult> {
    ...
    if (!canResume && asString(previousParams.runtimeSessionName, "")) {
      await ctx.onLog(
        "stdout",
        `[fabric] ACPX session "${asString(previousParams.runtimeSessionName, "")}" does not match the current agent/cwd/mode/runtime identity; starting fresh in "${prepared.cwd}".\n`,
      );
    }
    ...
    try {
      if (!handle) {
        try {
          ...
          await ctx.onLog(
            "stdout",
            `[fabric] ACPX resume session "${resumeSessionId}" is unavailable; retrying with a fresh session.\n`,
          );
          handle = await runtime.ensureSession({
            sessionKey: prepared.sessionKey,
            agent: prepared.acpxAgent,
    ...
    if (ctx.onMeta) {
      await ctx.onMeta({
        ...
        cwd: prepared.cwd,
        commandNotes: [
          `ACPX runtime embedded in HermesFabric with ${prepared.mode} session mode.`,
    ...
    try {
      ...
      if (terminal.status === "failed" || terminal.status === "cancelled" || timedOut) {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            entry: existing,
            reason: timedOut ? "fabric timeout cleanup" : `fabric turn ${terminal.status}`,
        ...
        if (existing && !warmHandleMatches(existing, runtime, sessionHandle)) {
          await runtime.close({
            handle: sessionHandle,
            reason: "fabric duplicate warm handle cleanup",
            discardPersistentState: false,
          }).catch(() => {});
        } else {
        ...
        if (warmHandleMatches(existing, runtime, sessionHandle) && existing) {
          await closeWarmHandle({
            ...
            key: prepared.sessionKey,
            entry: existing,
            reason: "fabric completed turn cleanup",
          });
        } else {
          await runtime.close({
            handle: sessionHandle,
      ...
      await runtime.close({
        handle: sessionHandle,
        reason: timedOut ? "fabric timeout cleanup" : "fabric error cleanup",
