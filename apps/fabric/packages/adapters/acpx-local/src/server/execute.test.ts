...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-acpx-skills-"));
  tempRoots.push(root);
  return root;
}
...
describe("acpx_local runtime skill isolation", () => {
  it.skipIf(process.platform === "win32")("materializes ACPX Claude skills without symlinked descendants", async () => {
    ...
    const { meta } = await runExecutor({
      agent: "claude",
      stateDir,
      fabricRuntimeSkills: [skill],
      fabricSkillSync: { desiredSkills: [skill.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("revokes removed ACPX Codex skills and skips symlinked descendants", async () => {
    ...
    const baseConfig = {
      ...
      fabricRuntimeSkills: [keep, remove],
    };

    await runExecutor({
      ...baseConfig,
      fabricSkillSync: { desiredSkills: [keep.key, remove.key] },
    });
    expect(await pathExists(path.join(codexHome, "skills", remove.runtimeName, "SKILL.md"))).toBe(true);

    await runExecutor({
      ...baseConfig,
      fabricSkillSync: { desiredSkills: [keep.key] },
    });
  ...
  it.skipIf(process.platform === "win32")("removes legacy ACPX Codex skill symlinks when a skill is no longer desired", async () => {
    ...
    await runExecutor({
      ...
      stateDir: path.join(root, "state"),
      env: { CODEX_HOME: codexHome },
      fabricRuntimeSkills: [legacy],
      fabricSkillSync: { desiredSkills: [] },
    });
  ...
  it.skipIf(process.platform === "win32")("replaces stale managed Codex auth files with source symlinks", async () => {
    ...
    const fabricInstanceId = "test-instance";
    ...
    try {
      ...
      process.env.HERMES_FABRIC_INSTANCE_ID = fabricInstanceId;
      await runExecutor({
        agent: "codex",
        stateDir: path.join(root, "state"),
        fabricRuntimeSkills: [],
  ...
  it("keeps fresh credential wrapper scripts across ACPX agent changes", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      agent: "custom-a",
      env: { HERMES_FABRIC_API_KEY: "old-key" },
    });
    await runExecutor({
      ...baseConfig,
      agent: "custom-b",
      env: { HERMES_FABRIC_API_KEY: "new-key" },
    });
    ...
    expect(wrapper).not.toContain("HERMES_FABRIC_API_KEY");
    expect(wrapper).not.toContain("new-key");
    expect(wrapper).not.toContain("old-key");
    expect(env).toContain("HERMES_FABRIC_API_KEY='new-key'");
  ...
  it("shapes ACPX wrapper workspace env for remote execution identities", async () => {
    ...
    expect(env).toContain("HERMES_FABRIC_WORKSPACE_CWD='/remote/workspace'");
    expect(env).not.toContain("HERMES_FABRIC_WORKSPACE_WORKTREE_PATH=");
  });
  ...
  it("keeps distinct wrapper env files for concurrent runs with different credentials", async () => {
    ...
    await runExecutor({
      ...baseConfig,
      env: { HERMES_FABRIC_API_KEY: "first-key" },
    });
    await runExecutor({
      ...baseConfig,
      env: { HERMES_FABRIC_API_KEY: "second-key" },
    });
    ...
    expect(envFiles.filter((contents) => contents.includes("HERMES_FABRIC_API_KEY='first-key'"))).toHaveLength(1);
    expect(envFiles.filter((contents) => contents.includes("HERMES_FABRIC_API_KEY='second-key'"))).toHaveLength(1);
  });
  ...
  it("writes wrapper that redirects child stderr to a per-run log file", async () => {
    ...
    expect(wrapper).toContain("HERMES_FABRIC_RUN_ID");
    expect(wrapper).toContain("tee -a");
    expect(wrapper).toContain("exec node ./fake-acp.js");
  });

  it("passes HermesFabric env through the ACP agent wrapper instead of process.env", async () => {
      ...
      createRuntime: () => ({
        ...
        startTurn: () => ({
          events: (async function* () {
            ...
            observedApiKeyDuringStream = process.env.HERMES_FABRIC_API_KEY;
            yield { type: "done", stopReason: "end_turn" };
          })(),
          result: Promise.resolve({ status: "completed", stopReason: "end_turn" }),
          cancel: async () => {},
    ...
    const previousApiKey = process.env.HERMES_FABRIC_API_KEY;
    try {
      ...
      if (previousApiKey === undefined) delete process.env.HERMES_FABRIC_API_KEY;
      else process.env.HERMES_FABRIC_API_KEY = previousApiKey;
    }
  });

  it("writes a HermesFabric-managed .claude/settings.local.json for the claude agent so it can reach the HermesFabric API", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    ...
    const { meta } = await runExecutor(
      ...
      { context: { fabricWorkspace: { cwd, agentHome: path.join(root, "agent-home") } } },
    );
    ...
    expect(allow).toContain(`Bash(${cwd}/scripts/fabric-issue-update.sh:*)`);
    ...
    const note = (meta[0]?.commandNotes as string[] | undefined)?.find((entry) =>
      entry.includes("HermesFabric-managed Claude settings"),
    );
    expect(note).toBeTruthy();
  });

  it("merges HermesFabric allowlist into an existing .claude/settings.local.json without losing user entries", async () => {
    const root = await makeTempRoot();
    const stateDir = path.join(root, "state");
    const cwd = path.join(root, "worktree");
