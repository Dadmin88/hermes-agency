...
function resolveOpenCodeCommand(input: unknown): string {
  const envOverride =
    typeof process.env.HERMES_FABRIC_OPENCODE_COMMAND === "string" &&
    process.env.HERMES_FABRIC_OPENCODE_COMMAND.trim().length > 0
      ? process.env.HERMES_FABRIC_OPENCODE_COMMAND.trim()
      : "opencode";
