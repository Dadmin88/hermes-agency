...
Use when:
- You want HermesFabric to run Cursor Agent CLI locally as the agent runtime
...
- HermesFabric auto-injects local skills into "~/.cursor/skills" when missing, so Cursor can discover "$fabric" and related skills on local runs.
- HermesFabric auto-adds --yolo unless one of --trust/--yolo/-f is already present in extraArgs.
