...
vi.mock("@hermes-fabric/adapter-utils/execution-target", () => ({
...
async function makeTempRoot() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-grok-local-"));
  tempRoots.push(root);
  return root;
}
...
describe("grok_local execute", () => {
  ...
  it("stages Grok-native instructions and skills into the workspace for the run and cleans them up afterward", async () => {
    ...
    const skillSource = path.join(root, "runtime-skills", "fabric");
    ...
    runProcessMock.mockImplementation(async (_runId, _target, _command, args, options) => {
      ...
      expect(await pathExists(path.join(root, ".claude", "skills", "fabric", "SKILL.md"))).toBe(true);
    ...
    const ctx: AdapterExecutionContext = {
      ...
      config: {
        ...
        fabricSkillSync: { desiredSkills: ["fabric"] },
      },
      context: {},
      authToken: "run-token",
    ...
    expect(await pathExists(path.join(root, ".claude", "skills", "fabric"))).toBe(false);
