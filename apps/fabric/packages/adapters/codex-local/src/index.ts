...
- If instructionsFilePath is configured, HermesFabric prepends that file's contents to the stdin prompt on every run.
- Codex exec automatically applies repo-scoped AGENTS.md instructions from the active workspace. HermesFabric cannot suppress that discovery in exec mode, so repo AGENTS.md files may still apply even when you only configured an explicit instructionsFilePath.
- HermesFabric injects desired local skills into the effective CODEX_HOME/skills/ directory at execution time so Codex can discover "$fabric" and related skills without polluting the project working directory. For new and updated agents, HermesFabric assigns an isolated managed home at ~/.hermes-fabric/instances/<id>/companies/<companyId>/agents/<agentId>/codex-home/skills/; when CODEX_HOME is explicitly overridden in adapter config, that override is used instead.
...
- Fast mode is supported on GPT-5.5, GPT-5.4 and manual model IDs. When enabled for those models, HermesFabric applies \`service_tier="fast"\` and \`features.fast_mode=true\`.
