import { asBoolean, asString, asStringArray } from "@hermes-fabric/adapter-utils/server-utils";
import {
  ...
  return {
    ...
    fastModeIgnoredReason:
      fastModeRequested && !fastModeApplied
        ? `Configured fast mode is currently only supported on ${formatFastModeSupportedModels()}; HermesFabric will ignore it for model ${model || "(default)"}.`
        : null,
  };
}
