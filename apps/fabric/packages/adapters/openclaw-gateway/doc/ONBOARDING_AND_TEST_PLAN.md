...
## Scope
This plan is now **gateway-only**. Hermes Fabric supports OpenClaw through `openclaw_gateway` only.
...
6. OpenClaw claims API key and installs/uses Hermes Fabric skill.
...
```json
{
  "fabricApiUrl": "http://host.docker.internal:3100",
...
3. In `/new` OpenClaw session, OpenClaw can still create a Hermes Fabric task.
...
```bash
pnpm --filter @hermes-fabric/server test:run -- openclaw-gateway
pnpm --filter @hermes-fabric/server typecheck
pnpm --filter @hermes-fabric/ui typecheck
pnpm --filter hermes-fabric typecheck
```
