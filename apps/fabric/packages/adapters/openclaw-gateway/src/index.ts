...
Use when:
- You want HermesFabric to invoke OpenClaw over the Gateway WebSocket protocol.
...
- Your deployment does not permit outbound WebSocket access from the HermesFabric server.
...
- fabricApiUrl (string, optional): absolute HermesFabric base URL advertised in wake text
- claimedApiKeyPath (string, optional): path to the claimed API key JSON file read by the agent at wake time (default ~/.openclaw/workspace/fabric-claimed-api-key.json)

Session routing fields:
- sessionKeyStrategy (string, optional): issue (default), fixed, or run
- sessionKey (string, optional): fixed session key when strategy=fixed (default fabric)

Wake payload notes:
- HermesFabric wake context is embedded into the generated message text
- No top-level fabric field is sent; the gateway agent schema rejects unknown root params
