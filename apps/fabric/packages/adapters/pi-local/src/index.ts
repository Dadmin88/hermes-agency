...
Use when:
- You want HermesFabric to run Pi (the AI coding agent) locally as the agent runtime
...
- HermesFabric requires an explicit \`model\` value for \`pi_local\` agents.
- Sessions are stored in ~/.pi/fabrics/ and resumed with --session.
