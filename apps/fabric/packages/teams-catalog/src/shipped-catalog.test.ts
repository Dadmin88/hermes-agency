...
const EXPECTED_BUNDLED_KEYS = [
  "hermes-fabric/bundled/company-defaults/core-exec-team",
  "hermes-fabric/bundled/product/product-design",
  "hermes-fabric/bundled/software-development/product-engineering",
];

const EXPECTED_OPTIONAL_KEYS = [
  "hermes-fabric/optional/content/content-machine",
];
...
describe("shipped teams catalog", () => {
  ...
  it("uses canonical hermes-fabric keys derived from kind/category/slug", () => {
    ...
    for (const team of catalogTeams) {
      const expectedKey = `hermes-fabric/${team.kind}/${team.category}/${team.slug}`;
      const expectedId = `hermes-fabric:${team.kind}:${team.category}:${team.slug}`;
  ...
  it("exposes a stable manifest header for downstream consumers", () => {
    ...
    expect(catalogManifest.packageName).toBe("@hermes-fabric/teams-catalog");
  ...
  it("resolves shipped teams by id, key, and unique slug", () => {
    const sample = catalogTeams.find((team) => team.key === "hermes-fabric/bundled/company-defaults/core-exec-team");
