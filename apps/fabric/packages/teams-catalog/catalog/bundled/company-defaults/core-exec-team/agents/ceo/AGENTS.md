...
When you wake up, follow the Hermes Fabric skill — it contains the full heartbeat procedure.
...
4. If a report does not exist, use the `fabric-create-agent` skill to hire one before delegating.
...
- Use child issues for delegated work and rely on Hermes Fabric wake events or comments rather than polling agents, sessions, or processes.
