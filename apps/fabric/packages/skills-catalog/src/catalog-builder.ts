...
const CATALOG_PACKAGE_NAME = "@hermes-fabric/skills-catalog";
...
export async function validateCatalog(packageDir: string): Promise<BuildCatalogManifestResult> {
  ...
  if (generatedText !== null) {
    ...
    if (generatedText !== expectedText) {
      errors.push("generated/catalog.json is stale. Run pnpm --filter @hermes-fabric/skills-catalog build:manifest.");
    }
  }
...
function skillIdForCandidate(candidate: BaseSkillCandidate) {
  return `hermes-fabric:${candidate.kind}:${candidate.category}:${candidate.slug}`;
}
