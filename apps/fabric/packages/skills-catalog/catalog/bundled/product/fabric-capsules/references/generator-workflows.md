...
Use this reference when the request is to generate Hermes Fabric capsule graphics, capsule identicons, or reusable visual assets.
...
2. Deterministic identicon/profile-pill prototype:
   - `fabric-content/design/PAP-11825/fabric-capsule-identicon-prototyper/README.md`
   - `src/identicon.ts`
   - `src/App.tsx`
3. Website embedded generator:
   - `fabric-website/public/brand/generator.js`
   - `fabric-website/src/components/brand/sections/08-imagery.html`
...
Use this when the task needs reproducible generated capsule art beyond the small embedded website tool. The contract here is intentionally mirrored into Hermes Fabric so agents can proceed if the original prototype repository is renamed, deleted, or private.
...
If an implementation needs source code, prefer the Hermes Fabric website embedded generator or a Hermes Fabric-owned tool. Use any external prototype link only as optional historical context, not as required task input.

Palette caution:

- The `duotones` palette matches the website 12-preset capsule palette.
- Experimental palettes such as vaporwave, cyberpunk, ocean, and jewel are generator options, not canonical Hermes Fabric brand palettes.
- Do not use experimental palettes when the request asks for strict Hermes Fabric brand work.
...
- Divergence from canonical HermesFabric rendering:
```
