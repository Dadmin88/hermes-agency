...
const SANDBOX_CALLBACK_BRIDGE_ENTRYPOINT = "fabric-bridge-server.mjs";
const SANDBOX_EXEC_CHANNEL_ENV = "HERMES_FABRIC_SANDBOX_EXEC_CHANNEL";
...
// reverse bridge. Keep this in sync with the HermesFabric skill in
// `skills/fabric/SKILL.md` and `references/api-reference.md`.
...
export function buildSandboxCallbackBridgeEnv(input: {
  ...
  return {
    ...
    HERMES_FABRIC_BRIDGE_QUEUE_DIR: input.queueDir,
    HERMES_FABRIC_BRIDGE_TOKEN: input.bridgeToken,
    HERMES_FABRIC_BRIDGE_HOST: input.host?.trim() || "127.0.0.1",
    HERMES_FABRIC_BRIDGE_PORT: String(input.port && input.port > 0 ? Math.trunc(input.port) : 0),
    HERMES_FABRIC_BRIDGE_POLL_INTERVAL_MS: String(
      normalizeTimeoutMs(input.pollIntervalMs, DEFAULT_BRIDGE_POLL_INTERVAL_MS),
    ),
    HERMES_FABRIC_BRIDGE_RESPONSE_TIMEOUT_MS: String(
      normalizeTimeoutMs(input.responseTimeoutMs, DEFAULT_BRIDGE_RESPONSE_TIMEOUT_MS),
    ),
    HERMES_FABRIC_BRIDGE_MAX_QUEUE_DEPTH: String(
      normalizeTimeoutMs(input.maxQueueDepth, DEFAULT_BRIDGE_MAX_QUEUE_DEPTH),
    ),
    HERMES_FABRIC_BRIDGE_MAX_BODY_BYTES: String(
...
export async function createSandboxCallbackBridgeAsset(): Promise<SandboxCallbackBridgeAsset> {
  const localDir = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-bridge-asset-"));
...
export function createFileSystemSandboxCallbackBridgeQueueClient(): SandboxCallbackBridgeQueueClient {
  return {
    ...
    writeResponseFile: async (responsePath, body, options = {}) => {
      ...
      const lockDir = `${responsePath}.fabric-write.lock`;
...
export function createCommandManagedSandboxCallbackBridgeQueueClient(input: {
  ...
  return {
    ...
    writeTextFile: async (remotePath, body) => {
      ...
      const tempPath = `${remotePath}.fabric-upload.b64`;
      await runChecked(
...
export async function startSandboxCallbackBridgeWorker(input: {
  ...
  const processRequestFile = async (fileName: string) => {
    ...
    try {
      ...
      console.warn(
        `[fabric] sandbox callback bridge handler failed for ${request.id}: ${error instanceof Error ? error.message : String(error)}`,
      );
      await writeBridgeResponse(input.client, requestPath, responsePath, {
        id: request.id,
        status: 502,
  ...
  const failPendingRequests = async (message: string) => {
    ...
    for (const fileName of fileNames) {
      ...
      try {
        ...
        console.warn(
          `[fabric] sandbox callback bridge failed to abort pending request ${requestId}: ${error instanceof Error ? error.message : String(error)}`,
        );
      } finally {
        await input.client.remove(requestPath).catch(() => undefined);
      }
  ...
  const loop = (async () => {
    try {
      ...
      console.warn(`[fabric] ${message}`);
      try {
        await failPendingRequests(message);
      } catch (failPendingError) {
        console.warn(
          `[fabric] sandbox callback bridge failed to abort queued requests after worker failure: ${failPendingError instanceof Error ? failPendingError.message : String(failPendingError)}`,
        );
      }
    } finally {
      settled = true;
...
export async function syncSandboxCallbackBridgeEntrypoint(input: {
  ...
  const remoteUploadPath = `${remoteEntrypoint}.fabric-upload.b64`;
  const remoteLockDir = path.posix.join(input.assetRemoteDir, ".fabric-bridge-upload.lock");
...
const queueDir = process.env.HERMES_FABRIC_BRIDGE_QUEUE_DIR;
const bridgeToken = process.env.HERMES_FABRIC_BRIDGE_TOKEN;
const host = process.env.HERMES_FABRIC_BRIDGE_HOST || "127.0.0.1";
const port = Number(process.env.HERMES_FABRIC_BRIDGE_PORT || "0");
const pollIntervalMs = Number(process.env.HERMES_FABRIC_BRIDGE_POLL_INTERVAL_MS || "100");
const responseTimeoutMs = Number(process.env.HERMES_FABRIC_BRIDGE_RESPONSE_TIMEOUT_MS || "30000");
const maxQueueDepth = Number(process.env.HERMES_FABRIC_BRIDGE_MAX_QUEUE_DEPTH || "${DEFAULT_BRIDGE_MAX_QUEUE_DEPTH}");
const maxBodyBytes = Number(process.env.HERMES_FABRIC_BRIDGE_MAX_BODY_BYTES || "${DEFAULT_BRIDGE_MAX_BODY_BYTES}");
const allowedHeaders = new Set(${JSON.stringify([...DEFAULT_SANDBOX_CALLBACK_BRIDGE_HEADER_ALLOWLIST])});

if (!queueDir || !bridgeToken) {
  throw new Error("HERMES_FABRIC_BRIDGE_QUEUE_DIR and HERMES_FABRIC_BRIDGE_TOKEN are required.");
}
