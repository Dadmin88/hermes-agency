...
// @google/gemini-cli. The bootstrap also sets HERMES_FABRIC_NPM_BOOTSTRAPPED=1
...
const ENSURE_NPM_PREAMBLE =
  "HERMES_FABRIC_NPM_BOOTSTRAPPED=; " +
  ...
  "HERMES_FABRIC_NPM_BOOTSTRAPPED=1; " +
  "fi;";
...
export function buildSandboxNpmInstallCommand(packageName: string): string {
  ...
  return [
    ENSURE_NPM_PREAMBLE,
    'if [ -n "$HERMES_FABRIC_NPM_BOOTSTRAPPED" ]; then',
