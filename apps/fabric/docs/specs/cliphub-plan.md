# ClipHub: Marketplace for Hermes Fabric Team Configurations
...
> The "app store" for whole-company AI teams — pre-built Hermes Fabric configurations, agent blueprints, skills, and governance templates that ship real work from day one.

## 1. Vision & Positioning

**ClipHub** sells **entire team configurations** — org charts, agent roles, inter-agent workflows, governance rules, and project templates — for Hermes Fabric-managed companies.

| Dimension | ClipHub |
|---|---|
| Unit of sale | Team blueprint (multi-agent org) |
| Buyer | Founder / team lead spinning up an AI company |
| Install target | Hermes Fabric company (agents, projects, governance) |
...
A complete Hermes Fabric company configuration:
...
Single-agent configurations designed to plug into a Hermes Fabric org:
...
Portable skill files that any Hermes Fabric agent can use:

- Markdown skill files with instructions
- Tool configurations and shell scripts
- Compatible with Hermes Fabric's skill loading system
...
interface Listing {
  ...
  fabricVersionMin: string;     // Minimum HermesFabric version
...
interface Purchase {
  ...
  listingId: string;
  buyerUserId: string;
  buyerCompanyId: string | null;    // Target HermesFabric company
...
| `POST` | `/api/listings/:id/install` | Install to Hermes Fabric company |
...
Homepage → Browse marketplace → Filter by type/category
  ...
  → Select target HermesFabric company (or create new)
  → ClipHub API calls HermesFabric API to:
  ...
  → Redirect to HermesFabric dashboard with new team running
```
...
### 5.3 Creator: Export from Hermes Fabric → Publish

```
Running HermesFabric company → "Export as Blueprint" (CLI or UI)
  → HermesFabric exports:
...
- **Color palette**: Dark ink primary, warm sand backgrounds, accent color for CTAs (Hermes Fabric brand blue/purple)
...
| Install flow | One-click deploy to Hermes Fabric company |
...
{
  "targetCompanyId": "uuid",         // Existing HermesFabric company
...
3. For each agent in blueprint:
   - `POST /api/companies/:id/agents` (if `fabric-create-agent` supports it, or via approval flow)
...
- **Frontend**: Next.js (React), Tailwind CSS, same UI framework as Hermes Fabric
- **Backend**: Node.js API (or extend Hermes Fabric server)
- **Database**: Postgres (can share Hermes Fabric's DB or separate)
- **Payments**: Stripe Connect (marketplace mode)
- **Storage**: S3/R2 for listing bundles and images
- **Auth**: Shared with Hermes Fabric auth (or OAuth2)

### 9.2 Integration with Hermes Fabric

ClipHub can be:
- **Option A**: A separate app that calls Hermes Fabric's API to install blueprints
- **Option B**: A built-in section of the Hermes Fabric UI (`/marketplace` route)

Option B is simpler for MVP — adds routes to the existing Hermes Fabric UI and API.
...
- [ ] Install flow: blueprint → Hermes Fabric company
...
- [ ] "Export from Hermes Fabric" CLI command
