...
Build a custom adapter to connect Hermes Fabric to any agent runtime.
...
|---|---|---|
| Source | Inside `fabric-fork` | Separate npm package |
| Distribution | Ships with Hermes Fabric | Independent npm publish |
...
For most cases, **build an external adapter plugin**. It's cleaner, independently versioned, and doesn't require modifying Hermes Fabric's source. See [External Adapters](/adapters/external-adapters) for the full guide.
...
1. Read config using safe helpers (`asString`, `asNumber`, etc.) from `@hermes-fabric/adapter-utils/server-utils`
2. Build environment with `buildHermesFabricEnv(agent)` plus context vars
...
|--------|--------|---------|
| `runChildProcess(cmd, opts)` | `@hermes-fabric/adapter-utils/server-utils` | Spawn with timeout, grace, streaming |
| `buildHermesFabricEnv(agent)` | `@hermes-fabric/adapter-utils/server-utils` | Inject `HERMES_FABRIC_*` env vars |
| `renderTemplate(tpl, data)` | `@hermes-fabric/adapter-utils/server-utils` | `{{variable}}` substitution |
| `asString(v)` | `@hermes-fabric/adapter-utils` | Safe config value extraction |
| `asNumber(v)` | `@hermes-fabric/adapter-utils` | Safe number extraction |
...
For built-in adapters registered in Hermes Fabric's source:
...
`format-event.ts` — pretty-prints stdout for `hermes-fabric run --watch` using `picocolors`.
...
With these flags set, the Hermes Fabric UI will automatically show the instructions bundle editor, skills management tab, and working directory field for agents using this adapter — no Hermes Fabric source changes required.
...
Make Hermes Fabric skills discoverable to your agent runtime without writing to the agent's working directory:
