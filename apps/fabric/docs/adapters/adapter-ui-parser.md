...
│  HermesFabric Server  │◄────────────────────────────────│  uiParserCache    │
│  (in-memory)      │                                 └──────────────────┘
└────────┬─────────┘
         │ serves JS to browser
         ▼
┌──────────────────┐   fetch() + eval   ┌──────────────────┐
│  HermesFabric UI     │─────────────────────→│  parseStdoutLine │
...
### 1. `fabric.adapterUiParser` — contract version
...
The Hermes Fabric host checks this field. If the major version is unsupported, the host logs a warning and falls back to the generic parser instead of executing potentially incompatible code.
