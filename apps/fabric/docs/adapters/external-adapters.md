...
summary: Build, package, and distribute adapters as plugins without modifying Hermes Fabric source
---

Hermes Fabric supports external adapter plugins that can be installed from npm packages or local directories. External adapters work exactly like built-in adapters — they execute agents, parse output, and render transcripts — but they live in their own package and don't require changes to Hermes Fabric's source code.

## Built-in vs External

| | Built-in | External |
|---|---|---|
| Source location | Inside `fabric-fork/packages/adapters/` | Separate npm package or local directory |
...
| Distribution | Ships with Hermes Fabric | Published to npm or linked via `file:` |
| Updates | Requires Hermes Fabric release | Independent versioning |
...
- `hermes_local` starts the local Hermes CLI from
  `@hermes-fabric/hermes-fabric-adapter`.
- `hermes_gateway` calls an already-running Hermes API server through
  `@hermes-fabric/hermes-fabric-adapter/gateway`.

The legacy `@hermes-fabric/adapter-hermes-gateway` package is a deprecated
...
depend on or link `@hermes-fabric/hermes-fabric-adapter` and declare the type
...
```json
{
  ...
  "dependencies": {
    "@hermes-fabric/adapter-utils": "^2026.325.0",
    "picocolors": "^1.1.0"
  },
  "devDependencies": {
    "@types/node": "^22.0.0",
...
| `fabric.adapterUiParser` | Contract version for the UI parser (`"1.0.0"`) |
...
```ts
import type { ServerAdapterModule } from "@hermes-fabric/adapter-utils";
...
export async function execute(
  ...
  // 2. Build environment with HermesFabric vars injected
  const env = buildHermesFabricEnv(agent);
...
#### Available Helpers from `@hermes-fabric/adapter-utils`

| Helper | Purpose |
|--------|---------|
| `runChildProcess(command, opts)` | Spawn a child process with timeout, grace period, and streaming callbacks |
| `buildHermesFabricEnv(agent)` | Inject `HERMES_FABRIC_*` environment variables |
...
```sh
# Via the HermesFabric UI
# Settings → Adapters → Install from npm → "my-fabric-adapter"

# Or via API
curl -X POST http://localhost:3102/api/adapters \
  -H "Authorization: Bearer <token>" \
  -H "Content-Type: application/json" \
  -d '{"packageName": "my-fabric-adapter"}'
```
...
Local adapters are symlinked into Hermes Fabric's adapter directory. Changes to the source are picked up on server restart.

### Via adapter-plugins.json

For development, you can also edit `~/.hermes-fabric/adapter-plugins.json` directly:

```json
[
  {
    "packageName": "my-fabric-adapter",
...
Other Hermes Fabric users can then install your adapter by package name from the UI or API.
