...
summary: Deploy Hermes Fabric to AWS using ECS Fargate, RDS Postgres, and EFS
---

Deploy Hermes Fabric to AWS with ECS Fargate (compute), RDS Postgres 17 (database), and EFS (persistent storage). This guide uses the AWS CLI and produces a single-task ECS service behind an ALB with HTTPS.
...
- The Hermes Fabric repo cloned locally
...
export HERMES_FABRIC_DOMAIN=fabric.example.com   # your domain
...
# Tag and push
docker tag fabric-server:latest \
  $AWS_ACCOUNT_ID.dkr.ecr.$AWS_REGION.amazonaws.com/fabric-server:latest
...
ALB_SG=$(aws ec2 create-security-group \
  --group-name fabric-alb \
  --description "HermesFabric ALB" \
  --vpc-id $VPC_ID \
...
ECS_SG=$(aws ec2 create-security-group \
  --group-name fabric-ecs \
  --description "HermesFabric ECS tasks" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
RDS_SG=$(aws ec2 create-security-group \
  --group-name fabric-rds \
  --description "HermesFabric RDS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
EFS_SG=$(aws ec2 create-security-group \
  --group-name fabric-efs \
  --description "HermesFabric EFS" \
  --vpc-id $VPC_ID \
  --query 'GroupId' --output text)
...
aws rds create-db-subnet-group \
  --db-subnet-group-name fabric-db-subnet \
  --db-subnet-group-description "HermesFabric RDS subnets" \
  --subnet-ids $SUBNET_1 $SUBNET_2

aws rds create-db-instance \
  --db-instance-identifier fabric-db \
  --db-instance-class db.t4g.micro \
  --engine postgres \
  --engine-version 17 \
  --master-username fabric \
...
DATABASE_URL="postgresql://fabric:${DB_PASSWORD}@${RDS_ENDPOINT}:5432/fabric"
```
...
EFS_ID=$(aws efs create-file-system \
  ...
  --encrypted \
  --tags Key=Name,Value=fabric-data \
...
aws secretsmanager create-secret \
  --name fabric/database-url \
  --secret-string "$DATABASE_URL"

aws secretsmanager create-secret \
  --name fabric/anthropic-api-key \
  --secret-string "YOUR_ANTHROPIC_KEY"

aws secretsmanager create-secret \
  --name fabric/better-auth-secret \
  --secret-string "$AUTH_SECRET"

aws secretsmanager create-secret \
  --name fabric/openai-api-key \
  --secret-string "YOUR_OPENAI_KEY"

aws secretsmanager create-secret \
  --name fabric/github-token \
...
# Task execution role
aws iam create-role \
  --role-name fabric-ecs-execution \
...
aws iam put-role-policy \
  ...
  --policy-document '{
    ...
    "Statement": [{
      ...
      "Resource": "arn:aws:secretsmanager:'$AWS_REGION':'$AWS_ACCOUNT_ID':secret:fabric/*"
    }]
  }'

# Task role (application — add permissions as needed)
aws iam create-role \
  --role-name fabric-ecs-task \
  --assume-role-policy-document '{
    "Version": "2012-10-17",
    "Statement": [{
      "Effect": "Allow",
...
```bash
aws ecs create-cluster --cluster-name fabric

aws logs create-log-group --log-group-name /ecs/fabric
```
...
sed -e "s|<ACCOUNT_ID>|$AWS_ACCOUNT_ID|g" \
    ...
    -e "s|<EFS_ID>|$EFS_ID|g" \
    -e "s|<DOMAIN>|$HERMES_FABRIC_DOMAIN|g" \
    docker/ecs-task-definition.json > /tmp/fabric-task-def.json

aws ecs register-task-definition \
  --cli-input-json file:///tmp/fabric-task-def.json
```
...
CERT_ARN=$(aws acm request-certificate \
  --domain-name $HERMES_FABRIC_DOMAIN \
...
- Create a CNAME or ALIAS record for `$HERMES_FABRIC_DOMAIN` -> `$ALB_DNS`
...
aws ecs create-service \
  --cluster fabric \
  --service-name fabric-server \
  --task-definition fabric-server \
  --desired-count 1 \
  --launch-type FARGATE \
...
# Check task health
aws ecs list-tasks --cluster fabric --service-name fabric-server
TASK_ARN=$(aws ecs list-tasks --cluster fabric --service-name fabric-server --query 'taskArns[0]' --output text)
aws ecs describe-tasks --cluster fabric --tasks $TASK_ARN \
  --query 'tasks[0].{status:lastStatus,health:healthStatus}'

# Check logs
aws logs tail /ecs/fabric --since 10m --follow

# Hit the health endpoint
curl -sf https://$HERMES_FABRIC_DOMAIN/api/health
```
...
#   { "name": "HERMES_FABRIC_AUTH_DISABLE_SIGN_UP", "value": "true" }
...
aws ecs update-service \
  --cluster fabric \
  --service fabric-server \
  --task-definition fabric-server:<PREVIOUS_REVISION>
```
...
```bash
aws rds stop-db-instance --db-instance-identifier fabric-db
aws rds start-db-instance --db-instance-identifier fabric-db
```
...
aws ecs update-service --cluster fabric --service fabric-server --desired-count 0
aws ecs delete-service --cluster fabric --service fabric-server --force
aws ecs delete-cluster --cluster fabric
...
aws rds delete-db-instance \
  --db-instance-identifier fabric-db \
  --final-db-snapshot-identifier fabric-db-final
aws rds wait db-instance-deleted --db-instance-identifier fabric-db
aws rds delete-db-subnet-group --db-subnet-group-name fabric-db-subnet
...
for s in database-url anthropic-api-key better-auth-secret openai-api-key github-token; do
  aws secretsmanager delete-secret --secret-id fabric/$s --force-delete-without-recovery
done
...
# 7. ECR
aws ecr delete-repository --repository-name fabric-server --force

# 8. IAM roles
aws iam delete-role-policy --role-name fabric-ecs-execution --policy-name SecretsAccess
aws iam detach-role-policy --role-name fabric-ecs-execution \
  --policy-arn arn:aws:iam::aws:policy/service-role/AmazonECSTaskExecutionRolePolicy
aws iam delete-role --role-name fabric-ecs-execution
aws iam delete-role --role-name fabric-ecs-task

# 9. Log group
aws logs delete-log-group --log-group-name /ecs/fabric
```
