...
Hermes Fabric encrypts secrets at rest using a local master key. Agent environment variables that contain sensitive values (API keys, tokens) are stored as encrypted secret references.

## Custody Boundaries

Hermes Fabric protects secret values up to the moment they are handed to an agent
or workload:
...
- Transport: values are decrypted server-side and injected into the agent
  ...
  immediately before the call. Hermes Fabric does not return decrypted values to
  the board UI.
...
Once a value reaches the consuming process, Hermes Fabric can no longer guarantee
...
At runtime, Hermes Fabric resolves the selected secret server-side and injects the
...
matches an agent env key, the project value wins before Hermes Fabric injects its
own `HERMES_FABRIC_*` runtime variables.
...
```
~/.hermes-fabric/instances/default/secrets/master.key
```

This key is auto-created during onboarding. The key never leaves your machine.
Hermes Fabric best-effort enforces `0600` permissions when it creates or loads the
key file. `hermes-fabric doctor` and the provider health API warn when the file is
...
|----------|-------------|
| `HERMES_FABRIC_SECRETS_MASTER_KEY` | 32-byte key as base64, hex, or raw string |
| `HERMES_FABRIC_SECRETS_MASTER_KEY_FILE` | Custom key file path |
| `HERMES_FABRIC_SECRETS_STRICT_MODE` | Set to `true` to enforce secret refs |
...
configuration or `HERMES_FABRIC_SECRETS_STRICT_MODE=false`.

## External References

Provider-owned secrets can be linked without copying values into Hermes Fabric by
using `managedMode: "external_reference"` plus a provider `externalRef`.
Hermes Fabric stores metadata and a non-sensitive fingerprint, never the value.
...
For hosted Hermes Fabric Cloud on AWS, see the AWS Secrets Manager operational
...
identity, not in Hermes Fabric company secrets**. Allowed credential sources are
workload identity attached to the Hermes Fabric server (instance profile, IRSA, ECS
...
can override) the deployment-level `HERMES_FABRIC_SECRETS_AWS_*` env. Bootstrap
...
Hermes Fabric `external_reference` secrets. This is a metadata-only link: Hermes Fabric
...
4. Select an AWS vault, search the remote inventory, and load more pages as
   needed.
5. Check the rows to import, review/edit the Hermes Fabric name and key, then
   submit.
...
cursors. Do not expect Hermes Fabric to crawl a whole account in the background;
load pages deliberately and retry throttled requests with backoff.

Remote import exposes AWS secret metadata visible to the Hermes Fabric runtime
...
- Keep managed create/rotate/delete permissions scoped to the Hermes Fabric
...
dedicated Hermes Fabric runtime roles per environment/account, AWS vaults pointed at
...
- `local_encrypted`: back up the local master key file and the Hermes Fabric
  ...
  key bytes.
- `aws_secrets_manager`: back up Hermes Fabric's database for vault metadata
  ...
  configured prefix; restore by pointing the same Hermes Fabric company at the
...
- `gcp_secret_manager` and `vault`: while these are coming soon, only the
  draft vault config exists in Hermes Fabric. Database backups capture it. There
  is nothing to restore on the provider side until runtime support lands.

### AWS Provider Bootstrap Boundary

The AWS Secrets Manager provider cannot bootstrap itself from Hermes Fabric
...
For Hermes Fabric Cloud, provision the server runtime IAM role/workload identity,
KMS key, deployment prefix, and non-secret `HERMES_FABRIC_SECRETS_AWS_*` environment
...
Hermes Fabric secrets. Bootstrap material belongs in infrastructure IAM/workload
...
```sh
pnpm hermes-fabric secrets migrate-inline-env --company-id <company-id>
...
Use the CLI command for normal operations because it goes through the Hermes Fabric
...
the value custodian; Hermes Fabric stores metadata and provider version references,
