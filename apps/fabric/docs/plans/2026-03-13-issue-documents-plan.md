...
Add first-class **documents** to Hermes Fabric as editable, revisioned, company-scoped text artifacts that can be linked to issues.
...
The `plan` key is conventional and reserved by Hermes Fabric workflow/docs.
...
Update the Hermes Fabric agent workflow so planning no longer edits the issue description.

Required changes:

- update `skills/fabric/SKILL.md`
