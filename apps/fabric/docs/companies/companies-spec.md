...
This specification is vendor-neutral. It is intended to be usable by any agent-company runtime, not only Hermes Fabric.
...
- be useful outside Hermes Fabric
...
- vendors may attach the actual schedule / trigger / runtime fidelity in a vendor extension such as `.fabric.yaml`
...
Example Hermes Fabric extension:
...
- Hermes Fabric must not require extra top-level fields for skill validity
- Hermes Fabric-specific extensions must live under `metadata.fabric` or `metadata.sources`
...
For Hermes Fabric, the preferred fidelity extension is:
...
- issue/task Hermes Fabric-only metadata
...
- Hermes Fabric tools may emit the vendor extension by default as a sidecar while keeping the base markdown clean

Suggested Hermes Fabric shape:
...
Additional rules for Hermes Fabric exporters:
...
- omit empty and default-valued Hermes Fabric fields when possible
...
- Hermes Fabric exporters should emit `.fabric.yaml` as a sidecar by default
...
## 19. Hermes Fabric Mapping

Hermes Fabric can map this spec to its runtime model like this:
...
- Hermes Fabric extension:
  - `.fabric.yaml` -> adapter config, runtime config, env input declarations, permissions, budgets, routine triggers, and other Hermes Fabric-specific fidelity

Inline Hermes Fabric-only metadata that must live inside a shared markdown file should use:

- `metadata.fabric`

That keeps the base format broader than Hermes Fabric.

This specification itself remains vendor-neutral and intended for any agent-company runtime, not only Hermes Fabric.

## 20. Cutover

Hermes Fabric should cut over to this markdown-first package model as the primary portability format.

`fabric.manifest.json` does not need to be preserved as a compatibility requirement for the future package system.

For Hermes Fabric, this should be treated as a hard cutover in product direction rather than a long-lived dual-format strategy.
