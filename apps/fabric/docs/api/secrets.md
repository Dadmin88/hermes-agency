...
To link a provider-owned secret without copying the value into Hermes Fabric, create
...
```json
{
  ...
  "externalRef": "arn:aws:secretsmanager:us-east-1:123456789012:secret:fabric/prod/stripe",
  "providerVersionRef": "version-id-or-label"
}
```

Hermes Fabric stores the provider reference and a non-sensitive fingerprint only.
...
credentials must not be stored in Hermes Fabric `company_secrets`.

The equivalent CLI check is:

```sh
pnpm hermes-fabric secrets doctor --company-id {companyId}
```
...
Remote import links existing AWS Secrets Manager entries into Hermes Fabric as
`external_reference` secrets. Import stores provider reference metadata only; it
does not copy the remote secret plaintext into Hermes Fabric.
...
- `duplicate`: a Hermes Fabric secret already links the same canonical provider
...
`provider_guardrail`. AWS refs under Hermes Fabric's own managed namespace are
blocked as external references; use the Hermes Fabric-managed secret flow for those
resources instead.
...
Hermes Fabric `name`, `key`, optional Hermes Fabric `description`,
`providerVersionRef`, and sanitized `providerMetadata`. Blank descriptions are
stored as `null`; AWS provider descriptions are not copied into Hermes Fabric
...
- `imported`: Hermes Fabric created an active `external_reference` secret and one
...
real value into the agent process environment. Hermes Fabric's custody guarantees
...
```sh
pnpm hermes-fabric secrets declarations --company-id {companyId}
```
