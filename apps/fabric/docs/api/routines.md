...
Fires a webhook trigger from an external system. Requires a valid `Authorization` or `X-HermesFabric-Signature` + `X-HermesFabric-Timestamp` header pair matching the trigger's signing mode.
