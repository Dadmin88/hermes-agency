...
Hermes Fabric `external_reference` secrets without copying plaintext into
Hermes Fabric.
...
`ListSecrets` only and import stores a Hermes Fabric external reference plus
...
remote metadata, or copies secret plaintext into Hermes Fabric.
...
- `conflict`: the suggested Hermes Fabric `name` or `key` is already in use.
...
Hermes Fabric-managed AWS secret through a broad runtime role.
...
Each row may include an optional Hermes Fabric `description` entered during review;
...
the Hermes Fabric runtime role can list the AWS secret but lacks
