...
Hermes Fabric exposes a RESTful JSON API for all control plane operations.
...
- **Agent run JWTs** — short-lived tokens injected during heartbeats (`HERMES_FABRIC_API_KEY`)
...
- Run audit trail: include `X-HermesFabric-Run-Id` header on all mutating requests during heartbeats
