...
This guide documents the intended runtime model for projects, execution workspaces, and issue runs in Hermes Fabric.

Hermes Fabric now presents this as a workspace-command model:
...
- Hermes Fabric does not automatically start or stop these workspace services as part of issue execution.
- Hermes Fabric also does not automatically restart workspace services on server boot.
...
2. Hermes Fabric realizes the effective execution workspace, including creating or reusing a worktree when needed.
3. Hermes Fabric persists execution-workspace metadata such as paths, refs, and provisioning settings.
