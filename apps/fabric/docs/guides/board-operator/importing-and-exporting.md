...
Hermes Fabric companies can be exported to portable markdown packages and imported from local directories or GitHub repositories. This lets you share company configurations, duplicate setups, and version-control your agent teams.
...
├── tasks/
│   └── onboarding/TASK.md
└── .fabric.yaml     # Adapter config, env inputs, routines
```
...
- **.fabric.yaml** holds Hermes Fabric-specific config (adapter types, env inputs, budgets) as an optional sidecar.
...
```sh
hermes-fabric company export <company-id> --out ./my-export
```
...
hermes-fabric company export abc123 --out ./backup --include company,agents,projects

# Export everything including tasks and skills
hermes-fabric company export abc123 --out ./full-export --include company,agents,projects,tasks,skills

# Export only specific skills
hermes-fabric company export abc123 --out ./skills-only --include skills --skills review,deploy
```
...
- Adapter type and env input declarations in `.fabric.yaml`
...
```sh
# From a local folder
hermes-fabric company import ./my-export

# From a GitHub URL
hermes-fabric company import https://github.com/org/repo

# From a GitHub subfolder
hermes-fabric company import https://github.com/org/repo/tree/main/companies/acme

# From GitHub shorthand
hermes-fabric company import org/repo
hermes-fabric company import org/repo/companies/acme
```
...
If `--target` is not specified, Hermes Fabric infers it: if a `--company-id` is provided (or one exists in context), it defaults to `existing`; otherwise `new`.
...
```sh
hermes-fabric company import org/repo --target existing --company-id abc123 --dry-run
```
...
```sh
hermes-fabric company import org/company-templates/engineering-team \
  --target new \
...
```sh
hermes-fabric company import ./shared-agents \
  --target existing \
  --company-id abc123 \
  --include agents \
  --collision rename
...
```sh
hermes-fabric company import org/repo --ref v2.0.0 --dry-run
```

**Non-interactive import (CI/scripts):**

```sh
hermes-fabric company import ./package \
  --target new \
  --yes \
  --json
```
...
Hermes Fabric supports several GitHub URL formats:
