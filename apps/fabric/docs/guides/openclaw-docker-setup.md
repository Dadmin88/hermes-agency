...
How to get OpenClaw running in a Docker container for local development and testing the Hermes Fabric OpenClaw adapter integration.

## Automated Join Smoke Test (Recommended First)

Hermes Fabric includes an end-to-end join smoke harness:
...
- writes isolated smoke config under `~/.openclaw-fabric-smoke/openclaw.json` and Docker `.env`
...
- probes and prints a Hermes Fabric host URL that is reachable from inside OpenClaw Docker
...
- `OPENCLAW_CONFIG_DIR` (default `~/.openclaw-fabric-smoke`)
- `OPENCLAW_RESET_STATE=1` (default) resets smoke agent state on each run to avoid stale auth/session drift
- `HERMES_FABRIC_HOST_PORT` (default `3100`)
- `HERMES_FABRIC_HOST_FROM_CONTAINER` (default `host.docker.internal`)

### Authenticated mode

If your Hermes Fabric deployment is `authenticated`, provide auth context:

```bash
HERMES_FABRIC_AUTH_HEADER="Bearer <token>" pnpm smoke:openclaw-join
# or
HERMES_FABRIC_COOKIE="your_session_cookie=..." pnpm smoke:openclaw-join
```

### Network topology tips

- Local same-host smoke: default callback uses `http://127.0.0.1:<port>/webhook`.
- Inside OpenClaw Docker, `127.0.0.1` points to the container itself, not your host Hermes Fabric server.
- For invite/onboarding URLs consumed by OpenClaw in Docker, use the script-printed Hermes Fabric URL (typically `http://host.docker.internal:3100`).
- If Hermes Fabric rejects the container-visible host with a hostname error, allow it from host:

```bash
pnpm hermes-fabric allowed-hostname host.docker.internal
```

Then restart Hermes Fabric and rerun the smoke script.
