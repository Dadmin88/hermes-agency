...
Run this against a dev Hermes Fabric instance with a board token or an agent token that can manage pipelines, routines, and issues.

```sh
export HERMES_FABRIC_API_URL=http://localhost:3100
export HERMES_FABRIC_COMPANY_ID=<company-id>
export HERMES_FABRIC_API_KEY=<token>
...
hermes-fabric pipelines create \
  -C "$HERMES_FABRIC_COMPANY_ID" \
...
hermes-fabric pipelines set-transitions \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$RELEASE_PIPELINE" \
...
hermes-fabric pipelines guidance put \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
...
export DRAFTING_ROUTINE_ID="$(
  hermes-fabric routine create \
    -C "$HERMES_FABRIC_COMPANY_ID" \
    --payload-json "$(jq -c . /tmp/drafting-routine.json)" \
    --json | jq -r '.id'
)"

hermes-fabric pipelines set-automation \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$CONTENT_PIPELINE" \
  --stage drafting \
...
export RELEASE_CASE_ID="$(
  hermes-fabric pipelines ingest \
    -C "$HERMES_FABRIC_COMPANY_ID" \
    "$RELEASE_PIPELINE" \
...
hermes-fabric pipelines ingest-batch \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$FEATURE_PIPELINE" \
...
hermes-fabric pipelines review-bulk \
  -C "$HERMES_FABRIC_COMPANY_ID" \
...
export SUGGESTION_ID="$(
  hermes-fabric pipelines case suggest \
    -C "$HERMES_FABRIC_COMPANY_ID" \
    "$BLOG_CASE" \
    --to assets \
...
hermes-fabric pipelines case resolve-suggestion \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$BLOG_CASE" \
...
curl -sS -X POST \
  ...
  "$HERMES_FABRIC_API_URL/api/cases/$TWEET_CASE/issue-links" >/dev/null

hermes-fabric pipelines case edit \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$BLOG_CASE" \
  --expected-version 2 \
...
# Recovery:
hermes-fabric pipelines case get -C "$HERMES_FABRIC_COMPANY_ID" "$BLOG_CASE" --json
```
...
```sh
export BLOG_VERSION="$(hermes-fabric pipelines case get -C "$HERMES_FABRIC_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"
...
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$HERO_CASE" --to published --expected-version 1 --reason "Hero image done."
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$CARD_CASE" --to dropped --expected-version 1 --reason "Social card not needed."
```

When both asset cases are terminal, move the blog case to `assembly`.

```sh
export BLOG_ASSETS_VERSION="$(hermes-fabric pipelines case get -C "$HERMES_FABRIC_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

hermes-fabric pipelines case transition \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$BLOG_CASE" \
  --to assembly \
...
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$ASSEMBLY_CASE" --to published --expected-version 1 --reason "Assembly complete."
```
...
```sh
export BLOG_REVIEW_VERSION="$(hermes-fabric pipelines case get -C "$HERMES_FABRIC_COMPANY_ID" "$BLOG_CASE" --json | jq -r '.case.version')"

hermes-fabric pipelines case review \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$BLOG_CASE" \
  --approve \
...
```sh
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 1 --reason "Draft ready for final review."
...
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$CHANGELOG_CASE" --to final_review --expected-version 4 --reason "Revised draft ready."
hermes-fabric pipelines case review -C "$HERMES_FABRIC_COMPANY_ID" "$CHANGELOG_CASE" --approve --expected-version 5
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$CHANGELOG_CASE" --to published --expected-version 6 --reason "Published after request-changes loop."
```
...
```sh
hermes-fabric pipelines case transition -C "$HERMES_FABRIC_COMPANY_ID" "$TWEET_CASE" --to final_review --expected-version 1 --reason "Blog blocker is now done."
...
```sh
hermes-fabric pipelines case rollup \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$RELEASE_CASE_ID" \
  --json
```
...
```sh
hermes-fabric pipelines case events \
  -C "$HERMES_FABRIC_COMPANY_ID" \
  "$CHANGELOG_CASE" \
  --json
```
...
```sh
HERMES_FABRIC_API_URL=http://localhost:3100 \
HERMES_FABRIC_COMPANY_ID=<company-id> \
HERMES_FABRIC_API_KEY=<token> \
