...
const PORT = Number(process.env.HERMES_FABRIC_E2E_PORT ?? 3199);
const BASE_URL = `http://127.0.0.1:${PORT}`;
const HERMES_FABRIC_HOME = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-e2e-home-"));
const PLAYWRIGHT_CHANNEL = process.env.HERMES_FABRIC_PLAYWRIGHT_CHANNEL;
...
export default defineConfig({
  ...
  // `onboard --yes --run` works in a non-interactive temp HERMES_FABRIC_HOME.
  webServer: {
    command: `pnpm hermes-fabric onboard --yes --run`,
    url: `${BASE_URL}/api/health`,
    // Always boot a dedicated throwaway instance for e2e so browser tests
    // never attach to the developer's active HermesFabric home/server.
    ...
    env: {
      ...
      PORT: String(PORT),
      HERMES_FABRIC_HOME,
      HERMES_FABRIC_INSTANCE_ID: "playwright-e2e",
      HERMES_FABRIC_BIND: "loopback",
      HERMES_FABRIC_DEPLOYMENT_MODE: "local_trusted",
