...
function detectTailnetBindHost(): string | undefined {
  const explicit = fabricEnv("TAILNET_BIND_HOST")?.trim();
...
export function loadConfig(): Config {
  ...
  const providerFromEnvRaw = fabricEnv("SECRETS_PROVIDER");
  ...
  const storageProviderFromEnvRaw = fabricEnv("STORAGE_PROVIDER");
  ...
  const storageS3ForcePathStyle =
    fabricEnvDefined("STORAGE_S3_FORCE_PATH_STYLE")
      ? fabricEnv("STORAGE_S3_FORCE_PATH_STYLE") === "true"
      : (fileStorage?.s3?.forcePathStyle ?? false);
  const feedbackExportBackendUrl =
    fabricEnv("FEEDBACK_EXPORT_BACKEND_URL")?.trim() ||
    fabricEnv("TELEMETRY_BACKEND_URL")?.trim() ||
    undefined;
  const feedbackExportBackendToken =
    fabricEnv("FEEDBACK_EXPORT_BACKEND_TOKEN")?.trim() ||
    fabricEnv("TELEMETRY_BACKEND_TOKEN")?.trim() ||
    undefined;

  const deploymentModeFromEnvRaw = fabricEnv("DEPLOYMENT_MODE");
  const deploymentModeFromEnv =
    deploymentModeFromEnvRaw && DEPLOYMENT_MODES.includes(deploymentModeFromEnvRaw as DeploymentMode)
      ? (deploymentModeFromEnvRaw as DeploymentMode)
      : null;
  const deploymentMode: DeploymentMode = deploymentModeFromEnv ?? fileConfig?.server.deploymentMode ?? "local_trusted";
  const strictModeFromEnv = fabricEnv("SECRETS_STRICT_MODE");
  const secretsStrictMode =
    strictModeFromEnv !== undefined
      ? strictModeFromEnv === "true"
      : (fileSecrets?.strictMode ?? deploymentMode === "authenticated");
  const deploymentExposureFromEnvRaw = fabricEnv("DEPLOYMENT_EXPOSURE");
  ...
  const bindFromEnvRaw = fabricEnv("BIND");
  const bindFromEnv =
  ...
  const customBindHost = fabricEnv("BIND_HOST") ?? fileConfig?.server.customBindHost;
  const authBaseUrlModeFromEnvRaw = fabricEnv("AUTH_BASE_URL_MODE");
  const authBaseUrlModeFromEnv =
    authBaseUrlModeFromEnvRaw &&
    AUTH_BASE_URL_MODES.includes(authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      ? (authBaseUrlModeFromEnvRaw as AuthBaseUrlMode)
      : null;
  const publicUrlFromEnv = fabricEnv("PUBLIC_URL");
  const authPublicBaseUrlRaw =
    fabricEnv("AUTH_PUBLIC_BASE_URL") ??
  ...
  const disableSignUpFromEnv = fabricEnv("AUTH_DISABLE_SIGN_UP");
  const authDisableSignUp: boolean =
    disableSignUpFromEnv !== undefined
      ? disableSignUpFromEnv === "true"
      : (fileConfig?.auth?.disableSignUp ?? false);
  const allowedHostnamesFromEnvRaw = fabricEnv("ALLOWED_HOSTNAMES");
  ...
  const companyDeletionEnvRaw = fabricEnv("ENABLE_COMPANY_DELETION");
  const companyDeletionEnabled =
    companyDeletionEnvRaw !== undefined
      ? companyDeletionEnvRaw === "true"
      : deploymentMode === "local_trusted";
  const databaseBackupEnabled =
    fabricEnvDefined("DB_BACKUP_ENABLED")
      ? fabricEnv("DB_BACKUP_ENABLED") === "true"
      : (fileDatabaseBackup?.enabled ?? true);
  const databaseBackupIntervalMinutes = Math.max(
    1,
    Number(fabricEnv("DB_BACKUP_INTERVAL_MINUTES")) ||
      fileDatabaseBackup?.intervalMinutes ||
      60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    Number(fabricEnv("DB_BACKUP_RETENTION_DAYS")) ||
      fileDatabaseBackup?.retentionDays ||
      7,
  );
  const databaseBackupDir = resolveHomeAwarePath(
    fabricEnv("DB_BACKUP_DIR") ??
  ...
  return {
    ...
    uiDevMiddleware: fabricEnv("UI_DEV_MIDDLEWARE") === "true",
    secretsProvider,
    secretsStrictMode,
    secretsMasterKeyFilePath:
      resolveHomeAwarePath(
        fabricEnv("SECRETS_MASTER_KEY_FILE") ??
