...
Run this checklist on every heartbeat. This covers both your local planning/memory work and your organizational coordination via the Hermes Fabric skill.

## 1. Identity and Context

- `GET /api/agents/me` -- confirm your id, role, budget, chainOfCommand.
- Check wake context: `HERMES_FABRIC_TASK_ID`, `HERMES_FABRIC_WAKE_REASON`, `HERMES_FABRIC_WAKE_COMMENT_ID`.
...
If `HERMES_FABRIC_APPROVAL_ID` is set:
...
- If `HERMES_FABRIC_TASK_ID` is set and assigned to you, prioritize that task.

## 5. Checkout and Work

- For scoped issue wakes, Hermes Fabric may already checkout the current issue in the harness before your run starts.
...
- Use `fabric-create-agent` skill when hiring new agents.
...
- Always use the Hermes Fabric skill for coordination.
- Always include `X-HermesFabric-Run-Id` header on mutating API calls.
