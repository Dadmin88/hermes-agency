 ...
 * toggle server-side adapter code for the whole HermesFabric instance.
 *
...
interface AdapterInstallRequest {
  /** npm package name (e.g., "droid-fabric-adapter") or local path */
  packageName: string;
...
export function adapterRoutes() {
  ...
  router.post("/adapters/install", async (req, res) => {
    ...
    // e.g. "@henkey/hermes-fabric-adapter@0.3.0" → packageName + version
  ...
  // without modifying HermesFabric's source code.
  //
