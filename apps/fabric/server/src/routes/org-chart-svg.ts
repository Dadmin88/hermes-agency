/**
 * Server-side SVG renderer for Hermes Agency org charts.
...
// Hermes Agency logo: scaled icon (~16px) + wordmark (13px), vertically centered
const HERMES_AGENCY_LOGO_SVG = `<g>
  ...
  <text x="22" y="11.5" font-family="system-ui, -apple-system, sans-serif" font-size="13" font-weight="600" fill="currentColor">Hermes Agency</text>
</g>`;
...
export function renderOrgChartSvg(orgTree: OrgNode[], style: OrgChartStyle = "warmth", overlay?: OrgChartOverlay): string {
  ...
  const logoX = TARGET_W - 156 - LOGO_PADDING;
