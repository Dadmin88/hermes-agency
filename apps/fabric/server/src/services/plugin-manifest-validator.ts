 ...
 */
import { pluginManifestV1Schema } from "@hermes-fabric/shared";
import type { HermesFabricPluginManifestV1 } from "@hermes-fabric/shared";
import { PLUGIN_API_VERSION } from "@hermes-fabric/shared";
...
export interface PluginManifestValidator {
   ...
   */
  parseOrThrow(input: unknown): HermesFabricPluginManifestV1;
  ...
  return {
    parse(input: unknown): ManifestParseResult {
      ...
      if (result.success) {
        return {
          success: true,
          manifest: result.data as HermesFabricPluginManifestV1,
        };
      }
