 ...
 * ~/.hermes-fabric/adapter-plugins.json. This is the source of truth for which
...
import { resolveHermesFabricHomeDir } from "../home-paths.js";
...
export interface AdapterPluginRecord {
  /** npm package name (e.g., "droid-fabric-adapter") */
  packageName: string;
...
function adapterPluginPaths() {
  const fabricDir = resolveHermesFabricHomeDir();
  return {
    adapterPluginsDir: path.join(fabricDir, "adapter-plugins"),
    adapterPluginsStorePath: path.join(fabricDir, "adapter-plugins.json"),
    adapterSettingsPath: path.join(fabricDir, "adapter-settings.json"),
  };
}
...
function ensureDirs(): string {
  ...
  if (!fs.existsSync(pkgJsonPath)) {
    fs.writeFileSync(pkgJsonPath, JSON.stringify({
      name: "fabric-adapter-plugins",
      version: "0.0.0",
      private: true,
      description: "Managed directory for HermesFabric external adapter plugins. Do not edit manually.",
    }, null, 2) + "\n");
  }
