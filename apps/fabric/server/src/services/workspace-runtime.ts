...
import type { AdapterRuntimeServiceReport } from "@hermes-fabric/adapter-utils";
import type { Db } from "@hermes-fabric/db";
import { executionWorkspaces, projectWorkspaces, workspaceRuntimeServices } from "@hermes-fabric/db";
import {
...
function discoverWorkspacePackagePaths(rootDir: string): Map<string, string> {
  ...
  const ignoredDirNames = new Set([".git", ".fabric", "dist", "node_modules"]);
...
export function sanitizeRuntimeServiceBaseEnv(baseEnv: NodeJS.ProcessEnv): NodeJS.ProcessEnv {
  ...
  for (const key of Object.keys(env)) {
    if (key.startsWith("HERMES_FABRIC_") || key.startsWith("FABRIC_")) {
      delete env[key];
    }
  }
...
function buildWorkspaceCommandEnv(input: {
  ...
  env.FABRIC_WORKSPACE_CWD = input.worktreePath;
  env.HERMES_FABRIC_WORKSPACE_CWD = input.worktreePath;
  env.FABRIC_WORKSPACE_PATH = input.worktreePath;
  env.HERMES_FABRIC_WORKSPACE_PATH = input.worktreePath;
  env.FABRIC_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.HERMES_FABRIC_WORKSPACE_WORKTREE_PATH = input.worktreePath;
  env.FABRIC_WORKSPACE_BRANCH = input.branchName;
  env.HERMES_FABRIC_WORKSPACE_BRANCH = input.branchName;
  env.FABRIC_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.HERMES_FABRIC_WORKSPACE_BASE_CWD = input.base.baseCwd;
  env.FABRIC_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.HERMES_FABRIC_WORKSPACE_REPO_ROOT = input.repoRoot;
  env.FABRIC_WORKSPACE_SOURCE = input.base.source;
  env.HERMES_FABRIC_WORKSPACE_SOURCE = input.base.source;
  env.FABRIC_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.HERMES_FABRIC_WORKSPACE_REPO_REF = input.base.repoRef ?? "";
  env.FABRIC_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.HERMES_FABRIC_WORKSPACE_REPO_URL = input.base.repoUrl ?? "";
  env.FABRIC_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.HERMES_FABRIC_WORKSPACE_CREATED = input.created ? "true" : "false";
  env.FABRIC_PROJECT_ID = input.base.projectId ?? "";
  env.HERMES_FABRIC_PROJECT_ID = input.base.projectId ?? "";
  env.FABRIC_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.HERMES_FABRIC_PROJECT_WORKSPACE_ID = input.base.workspaceId ?? "";
  env.FABRIC_AGENT_ID = input.agent.id ?? "";
  env.HERMES_FABRIC_AGENT_ID = input.agent.id ?? "";
  env.FABRIC_AGENT_NAME = input.agent.name;
  env.HERMES_FABRIC_AGENT_NAME = input.agent.name;
  env.FABRIC_COMPANY_ID = input.agent.companyId;
  env.HERMES_FABRIC_COMPANY_ID = input.agent.companyId;
  env.FABRIC_ISSUE_ID = input.issue?.id ?? "";
  env.HERMES_FABRIC_ISSUE_ID = input.issue?.id ?? "";
  env.FABRIC_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.HERMES_FABRIC_ISSUE_IDENTIFIER = input.issue?.identifier ?? "";
  env.FABRIC_ISSUE_TITLE = input.issue?.title ?? "";
  env.HERMES_FABRIC_ISSUE_TITLE = input.issue?.title ?? "";
  env.FABRIC_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  env.HERMES_FABRIC_ISSUE_WORK_MODE = input.issue?.workMode ?? "";
  return env;
}
...
function buildExecutionWorkspaceCleanupEnv(input: {
  ...
  env.FABRIC_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.HERMES_FABRIC_WORKSPACE_CWD = input.workspace.cwd ?? "";
  env.FABRIC_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.HERMES_FABRIC_WORKSPACE_PATH = input.workspace.cwd ?? "";
  env.FABRIC_WORKSPACE_WORKTREE_PATH = input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.HERMES_FABRIC_WORKSPACE_WORKTREE_PATH = input.workspace.providerRef ?? input.workspace.cwd ?? "";
  env.FABRIC_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.HERMES_FABRIC_WORKSPACE_BRANCH = input.workspace.branchName ?? "";
  env.FABRIC_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.HERMES_FABRIC_WORKSPACE_BASE_CWD = input.projectWorkspaceCwd ?? "";
  env.FABRIC_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.HERMES_FABRIC_WORKSPACE_REPO_ROOT = input.projectWorkspaceCwd ?? "";
  env.FABRIC_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.HERMES_FABRIC_WORKSPACE_REPO_URL = input.workspace.repoUrl ?? "";
  env.FABRIC_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.HERMES_FABRIC_WORKSPACE_REPO_REF = input.workspace.baseRef ?? "";
  env.FABRIC_PROJECT_ID = input.workspace.projectId ?? "";
  env.HERMES_FABRIC_PROJECT_ID = input.workspace.projectId ?? "";
  env.FABRIC_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.HERMES_FABRIC_PROJECT_WORKSPACE_ID = input.workspace.projectWorkspaceId ?? "";
  env.FABRIC_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  env.HERMES_FABRIC_ISSUE_ID = input.workspace.sourceIssueId ?? "";
  return env;
}
...
export async function realizeExecutionWorkspace(input: {
  ...
  const worktreeParentDir = configuredParentDir
    ...
    : path.join(repoRoot, ".fabric", "worktrees");
...
function resolveRuntimeServiceHealthUrl(
  ...
  if (!url || !isHermesFabricDevRuntimeService(input ?? {})) return url;
  try {
  ...
  for (const row of projectWorkspaceRows) {
    ...
    try {
      const refs = await startRuntimeServicesForWorkspaceControl({
        db,
        actor: { id: null, name: "Hermes Fabric", companyId: row.companyId },
        issue: null,
        workspace: {
          baseCwd: row.cwd,
