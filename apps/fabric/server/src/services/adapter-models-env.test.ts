...
describe("parseAdapterModelsEnv", () => {
  ...
  it("parses the per-adapter model map", () => {
    const m = parseAdapterModelsEnv({ HERMES_FABRIC_ADAPTER_MODELS: ENV });
  ...
  it("defaults label to id when omitted", () => {
    const m = parseAdapterModelsEnv({ HERMES_FABRIC_ADAPTER_MODELS: JSON.stringify({ pi_local: [{ id: "tensorix/x/y" }] }) });
    expect(m?.pi_local?.[0]).toEqual({ id: "tensorix/x/y", label: "tensorix/x/y" });
  });
  it("throws on invalid JSON (fail loud)", () => {
    expect(() => parseAdapterModelsEnv({ HERMES_FABRIC_ADAPTER_MODELS: "{bad" })).toThrow(/HERMES_FABRIC_ADAPTER_MODELS/);
  });
  it("throws when an entry lacks a string id", () => {
    expect(() => parseAdapterModelsEnv({ HERMES_FABRIC_ADAPTER_MODELS: JSON.stringify({ a: [{ label: "x" }] }) })).toThrow();
  });
});
