...
const TRANSFER_SCHEMA = {
  family: "fabric-upstream-transfer",
  version: "1.0.0",
  major: 1,
  minor: 0,
} as const;
...
const CLOUD_CREDENTIAL_PREFIX = "fabric-cloud-credential:";
...
export function cloudUpstreamService(db: Db, options: { instanceId?: string } = {}) {
  const sourceInstanceId = `fabric-local-${options.instanceId ?? "default"}`;
  ...
  return {
    ...
    createRun: async (input: { connectionId: string; companyId: string; retryOfRunId?: string | null }): Promise<CloudUpstreamRun> => {
      ...
      if (!preview.schemaCompatible) {
        throw badRequest("Cloud stack schema is not compatible with this local HermesFabric version");
      }
      ...
      const now = new Date();
      const initialEvents = [
        event(now.toISOString(), "connect", "completed", "Connected to the target HermesFabric Cloud stack."),
  ...
  async function buildBundle(connection: ConnectionRow, mode: "preview" | "apply"): Promise<LocalUpstreamExportBundle> {
    ...
    const source = {
      ...
      exporterVersion: "fabric-local-cloud-ui-v1",
...
async function fetchDiscovery(remoteUrl: string): Promise<Record<string, unknown>> {
  ...
  const discoveryUrl = new URL("/.well-known/fabric-upstream", parsed.origin);
  if (stackId) {
      ...
      .set({
        ...
        report: {
          ...
          errorMessage: optionalString(report.errorMessage)
            ?? "The server restarted while this cloud upstream run was running, so HermesFabric marked it failed instead of leaving it stuck.",
...
function targetFromDiscovery(discovery: Record<string, unknown>): CloudUpstreamTarget {
  ...
  return {
    ...
    origin,
    product: optionalString(discovery.product) ?? "HermesFabric Cloud",
