...
import { agentWakeupRequests, agents, heartbeatRuns, issues } from "@hermes-fabric/db";
import type { IssueCommentMetadata, IssueCommentPresentation, RunLivenessState } from "@hermes-fabric/shared";
...
export const SUCCESSFUL_RUN_HANDOFF_REQUIRED_NOTICE_BODY =
  "Hermes Agency needs a disposition before this issue can continue.";
export const SUCCESSFUL_RUN_HANDOFF_EXHAUSTED_NOTICE_BODY =
  "Hermes Agency could not resolve this issue's missing disposition automatically. The issue is blocked on a recovery owner.";
...
export function buildSuccessfulRunHandoffInstruction(input: {
  ...
  return [
    `Your previous run on ${issueLabel} succeeded, but the issue is still in \`in_progress\` and Hermes Agency cannot identify a valid issue disposition.`,
    "",
    "Resolve the missing disposition before creating or revising any new artifacts. Choose **exactly one** outcome and perform the matching Hermes Agency action:",
    "",
