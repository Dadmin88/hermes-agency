...
import type { BindMode, DeploymentExposure, DeploymentMode } from "@hermes-fabric/shared";
...
function resolveAgentJwtSecretStatus(
  ...
  const envValue = fabricEnv("AGENT_JWT_SECRET")?.trim();
  if (envValue) {
    return {
      status: "pass",
      message: "set",
  ...
  if (existsSync(envFilePath)) {
    ...
    const fileValue = typeof parsed.HERMES_FABRIC_AGENT_JWT_SECRET === "string" ? parsed.HERMES_FABRIC_AGENT_JWT_SECRET.trim() : "";
    if (fileValue) {
      return {
        status: "warn",
  ...
  return {
    status: "warn",
    message: "missing (run `pnpm hermes-fabric onboard`)",
  };
}
...
export function printStartupBanner(opts: StartupBannerOptions): void {
  ...
  const envFilePath = resolveHermesFabricEnvPath();
