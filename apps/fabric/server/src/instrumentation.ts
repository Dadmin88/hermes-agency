...
export function shutdownInstrumentation(): Promise<void> {
  shutdownPromise ??= (async () => {
    ...
    try {
      ...
      console.error("[fabric] OpenTelemetry shutdown failed", err);
    }
  })();
...
export function resolveProtocol(): {
  ...
  switch (raw) {
    ...
    default:
      // eslint-disable-next-line no-console
      console.warn(
        `[fabric] Unknown OTEL_EXPORTER_OTLP_PROTOCOL=${raw}; falling back to grpc. ` +
  ...
  try {
    ...
    const sdk = new NodeSDK({
      resource: resourceFromAttributes({
        [ATTR_SERVICE_NAME]: process.env.OTEL_SERVICE_NAME || "fabric",
    ...
    try {
      ...
      // eslint-disable-next-line no-console
      console.error(
        "[fabric] OpenTelemetry SDK failed to start; continuing without tracing",
        err,
      );
      return;
    }
    ...
    // eslint-disable-next-line no-console
    console.warn(
      "[fabric] OTEL_EXPORTER_OTLP_ENDPOINT is set but the @opentelemetry/* " +
