const FAVICON_BLOCK_START = "<!-- HERMES_FABRIC_FAVICON_START -->";
const FAVICON_BLOCK_END = "<!-- HERMES_FABRIC_FAVICON_END -->";
const RUNTIME_BRANDING_BLOCK_START = "<!-- HERMES_FABRIC_RUNTIME_BRANDING_START -->";
const RUNTIME_BRANDING_BLOCK_END = "<!-- HERMES_FABRIC_RUNTIME_BRANDING_END -->";
...
export function isWorktreeUiBrandingEnabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return isTruthyEnvValue((env.FABRIC_IN_WORKTREE ?? env.HERMES_FABRIC_IN_WORKTREE));
}
...
export function getWorktreeUiBranding(env: NodeJS.ProcessEnv = process.env): WorktreeUiBranding {
  ...
  const name = nonEmpty((env.FABRIC_WORKTREE_NAME ?? env.HERMES_FABRIC_WORKTREE_NAME)) ?? nonEmpty((env.FABRIC_INSTANCE_ID ?? env.HERMES_FABRIC_INSTANCE_ID)) ?? "worktree";
  const color = normalizeHexColor((env.FABRIC_WORKTREE_COLOR ?? env.HERMES_FABRIC_WORKTREE_COLOR)) ?? deriveColorFromSeed(name);
...
export function renderRuntimeBrandingMeta(branding: WorktreeUiBranding): string {
  ...
  return [
    '<meta name="fabric-worktree-enabled" content="true" />',
    `<meta name="fabric-worktree-name" content="${escapeHtmlAttribute(branding.name)}" />`,
    `<meta name="fabric-worktree-color" content="${escapeHtmlAttribute(branding.color)}" />`,
    `<meta name="fabric-worktree-text-color" content="${escapeHtmlAttribute(branding.textColor)}" />`,
  ].join("\n");
}
