...
Use when:
- Hermes is already running outside Hermes Agency and exposes its API server.
- Hermes Agency should invoke Hermes through the gateway HTTP API instead of spawning Hermes locally.
- The Hermes runtime may live on another host, a private overlay, in Docker, or behind a TLS reverse proxy.

Don't use when:
- Hermes Agency should start the Hermes CLI directly on the same host. Use the built-in hermes_local adapter for that flow.
- Hermes is the process calling Hermes Agency APIs after claiming its key. That is Hermes-originated Hermes Agency API usage, not the gateway adapter transport.
...
- Default Hermes dashboard port: 9119. If Hermes Agency is configured with a default dashboard root or chat URL such as http://127.0.0.1:9119 or http://127.0.0.1:9119/chat, it maps the base URL to /api automatically.
- Watch out: /chat and the dashboard root are browser UI routes. Hermes Agency tests /api/health and starts runs with /api/v1/runs after mapping them to the API base.
