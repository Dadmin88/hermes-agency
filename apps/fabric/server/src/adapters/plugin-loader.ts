...
function extractUiParserSource(
  ...
  const contractVersion = pkg.fabric?.adapterUiParser;
  if (contractVersion) {
    ...
    logger.info(
      { packageName },
      "Adapter has ./ui-parser export but no fabric.adapterUiParser version — loading anyway (future versions may require it)",
    );
  }
