...
describe("agent instructions service", () => {
  const originalHermesFabricHome = process.env.HERMES_FABRIC_HOME;
  const originalHermesFabricInstanceId = process.env.HERMES_FABRIC_INSTANCE_ID;
  const cleanupDirs = new Set<string>();

  afterEach(async () => {
    if (originalHermesFabricHome === undefined) delete process.env.HERMES_FABRIC_HOME;
    else process.env.HERMES_FABRIC_HOME = originalHermesFabricHome;
    if (originalHermesFabricInstanceId === undefined) delete process.env.HERMES_FABRIC_INSTANCE_ID;
    else process.env.HERMES_FABRIC_INSTANCE_ID = originalHermesFabricInstanceId;
  ...
  it("copies the existing bundle into the managed root when switching to managed mode", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-home-");
    const externalRoot = await makeTempDir("fabric-agent-instructions-external-");
    cleanupDirs.add(fabricHome);
    cleanupDirs.add(externalRoot);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
  ...
  it("creates the target entry file when switching to a new external root", async () => {
    ...
    const externalRoot = await makeTempDir("fabric-agent-instructions-new-external-");
    cleanupDirs.add(fabricHome);
    cleanupDirs.add(externalRoot);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
  ...
  it("filters junk files, dependency bundles, and python caches from bundle listings and exports", async () => {
    const externalRoot = await makeTempDir("fabric-agent-instructions-ignore-");
    cleanupDirs.add(externalRoot);
  ...
  it("recovers a managed bundle from disk when bundle config metadata is missing", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-recover-");
    cleanupDirs.add(fabricHome);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
  ...
  it("prefers the managed bundle on disk when managed metadata points at a stale root", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-stale-managed-");
    const staleRoot = await makeTempDir("fabric-agent-instructions-stale-root-");
    cleanupDirs.add(fabricHome);
  ...
  it("heals stale managed metadata when writing bundle files", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-heal-write-");
    const staleRoot = await makeTempDir("fabric-agent-instructions-heal-write-stale-");
    cleanupDirs.add(fabricHome);
    cleanupDirs.add(staleRoot);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
  ...
  it("heals stale managed metadata when deleting bundle files", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-heal-delete-");
    const staleRoot = await makeTempDir("fabric-agent-instructions-heal-delete-stale-");
    cleanupDirs.add(fabricHome);
    cleanupDirs.add(staleRoot);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
  ...
  it("recovers the managed bundle when stale root metadata is present but mode is missing", async () => {
    const fabricHome = await makeTempDir("fabric-agent-instructions-partial-managed-");
    const staleRoot = await makeTempDir("fabric-agent-instructions-partial-root-");
    cleanupDirs.add(fabricHome);
    cleanupDirs.add(staleRoot);
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test-instance";
