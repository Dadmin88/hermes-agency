...
describeEmbedded("PAP-9522 QA: routine secrets end-to-end", () => {
  ...
  const secretsTmpDir = path.join(os.tmpdir(), `fabric-qa-routine-secrets-${randomUUID()}`);
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-qa-routine-secrets-");
  ...
  afterAll(async () => {
    ...
    if (previousKeyFile === undefined) delete process.env.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE;
    else process.env.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE = previousKeyFile;
