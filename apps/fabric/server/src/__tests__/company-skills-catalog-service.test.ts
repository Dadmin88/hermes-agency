...
import { companies, companySkills, createDb } from "@hermes-fabric/db";
import {
  getEmbeddedPostgresTestSupport,
  startEmbeddedPostgresTestDatabase,
} from "./helpers/embedded-postgres.js";
import type { CatalogSkill, CatalogSkillFile } from "@hermes-fabric/shared";
...
describeEmbeddedPostgres("companySkillService.installFromCatalog", () => {
  ...
  beforeAll(async () => {
    ...
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-company-skills-catalog-");
    db = createDb(tempDb.connectionString);
    svc = await createService();
  }, 20_000);

  beforeEach(async () => {
    const home = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-catalog-home-"));
  ...
  it("restores portable catalog provenance when importing packaged skills", async () => {
    const companyId = await createCompany();
    const importedFiles = {
      "skills/hermes-fabric/bundled/software-development/review/SKILL.md": [
        "---",
        `key: "${sampleCatalogSkill.key}"`,
        'slug: "review"',
        'name: "review"',
      ...
      "skills/hermes-fabric/bundled/software-development/review/references/checklist.md": sampleReferenceMarkdown,
    };
  ...
  it("rejects duplicate slug conflicts", async () => {
    const companyId = await createCompany();
    const skillDir = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-existing-skill-"));
