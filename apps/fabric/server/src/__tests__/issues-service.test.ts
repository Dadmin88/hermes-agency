...
import { buildAgentMentionHref, buildProjectMentionHref, MAX_ISSUE_REQUEST_DEPTH, type IssueWorkMode } from "@hermes-fabric/shared";
...
describeEmbeddedPostgres("issueService.list participantAgentId", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-service-");
  ...
  it("ranks comment matches ahead of description-only matches", async () => {
    ...
    await db.insert(issueComments).values({
      ...
      body: "Reference: https://github.com/DeployFaith/Hermes_Agency/pull/3303",
    });
  ...
  it("hides plugin operation issues from default lists and inbox-style filters while preserving explicit retrieval", async () => {
    ...
    await db.insert(issues).values([
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:fabric.missions:operation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:fabric.missions:operation:evaluation",
      ...
      },
      {
        ...
        priority: "medium",
        assigneeAgentId: agentId,
        originKind: "plugin:hermes-fabric.content-machine:evaluation",
    ...
    await expect(svc.list(companyId, { originKind: "plugin:fabric.missions:operation" }))
    ...
    await expect(svc.list(companyId, { originId: "mission-alpha:operation-1" }))
      .resolves.toEqual([expect.objectContaining({ id: operationIssueId })]);
    await expect(svc.list(companyId, { originKindPrefix: "plugin:fabric.missions:operation" }))
...
describeEmbeddedPostgres("issueService.create workspace inheritance", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-create-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService blockers and dependency wake readiness", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-blockers-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.findMentionedProjectIds", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-mentioned-projects-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
    await ensureIssueRelationsTable(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.clearExecutionRunIfTerminal", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-execution-lock-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("accepted plan decomposition", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-accepted-plan-decomposition-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
...
describeEmbeddedPostgres("issueService.assertCheckoutOwner stale checkout adoption", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issues-checkout-owner-");
    db = createDb(tempDb.connectionString);
    svc = issueService(db);
  }, 20_000);
