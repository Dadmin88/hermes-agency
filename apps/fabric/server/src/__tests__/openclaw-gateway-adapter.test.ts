...
import { execute, testEnvironment } from "@hermes-fabric/adapter-openclaw-gateway/server";
import {
  buildOpenClawGatewayConfig,
  parseOpenClawGatewayStdoutLine,
} from "@hermes-fabric/adapter-openclaw-gateway/ui";
...
describe("openclaw gateway adapter execute", () => {
  it("runs connect -> agent -> agent.wait and forwards wake payload", async () => {
    ...
    try {
      ...
      expect(payload?.sessionKey).toBe("fabric:issue:issue-123");
      expect(String(payload?.message ?? "")).toContain("wake now");
      expect(String(payload?.message ?? "")).toContain("HERMES_FABRIC_RUN_ID=run-123");
      expect(String(payload?.message ?? "")).toContain("HERMES_FABRIC_TASK_ID=task-123");
      expect(String(payload?.message ?? "")).toContain("## HermesFabric Wake Payload");
      ...
      expect(payload?.fabric).toBeUndefined();
