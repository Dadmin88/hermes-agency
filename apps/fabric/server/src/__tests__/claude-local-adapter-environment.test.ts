...
import { resetClaudeCliCapabilitiesCacheForTests, testEnvironment } from "@hermes-fabric/adapter-claude-local/server";
...
describe("claude_local environment diagnostics", () => {
  ...
  it("creates a missing working directory when cwd is absolute", async () => {
    const cwd = path.join(
      os.tmpdir(),
      `fabric-claude-local-cwd-${Date.now()}-${Math.random().toString(16).slice(2)}`,
      "workspace",
    );
    ...
    expect(
      result.checks.some(
        (check) =>
          ...
          check.message === "Working directory is valid: /srv/fabric/workspace",
      ),
    ).toBe(true);
  ...
  it("warns and omits --effort for sandbox probes when the installed Claude CLI does not advertise it", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-claude-envtest-sandbox-effort-"));
