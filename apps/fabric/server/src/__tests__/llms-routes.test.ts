...
describe("llm routes", () => {
  ...
  it("documents timer heartbeats as opt-in for new hires", async () => {
    ...
    expect(res.text).toContain("Use the `fabric-create-agent` hiring skill (compatibility name) for end-to-end hiring");
  ...
  it("serves static Hermes Gateway configuration docs before the plugin is installed", async () => {
    ...
    expect(res.text).toContain("agentDefaultsPayload.fabricApiUrl");
    expect(res.text).toContain("hermes_local");
    expect(res.text).toContain("Hermes-originated Hermes Agency API usage");
