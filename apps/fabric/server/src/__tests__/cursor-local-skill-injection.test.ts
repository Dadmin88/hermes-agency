...
import { ensureCursorSkillsInjected } from "@hermes-fabric/adapter-cursor-local/server";
...
describe("cursor local adapter skill injection", () => {
  ...
  it("links missing HermesFabric skills into Cursor skills home", async () => {
    const skillsDir = await makeTempDir("fabric-cursor-skills-src-");
    const skillsHome = await makeTempDir("fabric-cursor-skills-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);

    await createSkillDir(skillsDir, "fabric");
    await createSkillDir(skillsDir, "fabric-create-agent");
    ...
    const injectedA = path.join(skillsHome, "fabric");
    const injectedB = path.join(skillsHome, "fabric-create-agent");
    expect((await fs.lstat(injectedA)).isSymbolicLink()).toBe(true);
    expect((await fs.lstat(injectedB)).isSymbolicLink()).toBe(true);
    expect(await fs.realpath(injectedA)).toBe(await fs.realpath(path.join(skillsDir, "fabric")));
    expect(await fs.realpath(injectedB)).toBe(
      await fs.realpath(path.join(skillsDir, "fabric-create-agent")),
    );
    expect(logs.some((line) => line.includes('Injected Cursor skill "fabric"'))).toBe(true);
    expect(logs.some((line) => line.includes('Injected Cursor skill "fabric-create-agent"'))).toBe(true);
  });

  it("preserves existing targets and only links missing skills", async () => {
    const skillsDir = await makeTempDir("fabric-cursor-preserve-src-");
    const skillsHome = await makeTempDir("fabric-cursor-preserve-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
    ...
    const existingTarget = path.join(skillsHome, "fabric");
    ...
    expect((await fs.lstat(path.join(skillsHome, "fabric-create-agent"))).isSymbolicLink()).toBe(true);
  });

  it("logs per-skill link failures and continues without throwing", async () => {
    const skillsDir = await makeTempDir("fabric-cursor-fail-src-");
    const skillsHome = await makeTempDir("fabric-cursor-fail-home-");
    cleanupDirs.add(skillsDir);
    cleanupDirs.add(skillsHome);
