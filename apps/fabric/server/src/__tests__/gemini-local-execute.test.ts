...
import { execute } from "@hermes-fabric/adapter-gemini-local/server";
...
describe("gemini execute", () => {
  it("passes prompt via --prompt and injects fabric env vars", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-execute-"));
    ...
    try {
      ...
      expect(promptArg).toContain("Follow the fabric heartbeat.");
      expect(promptArg).toContain("HermesFabric runtime note:");
      ...
      expect(invocationPrompt).toContain("HermesFabric runtime note:");
      expect(invocationPrompt).toContain("HERMES_FABRIC_API_URL");
      expect(invocationPrompt).toContain("HermesFabric API access note:");
  ...
  it("always passes --approval-mode yolo", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-yolo-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes turn-limit exhaustion into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-max-turns-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("normalizes Gemini exit code 53 as max-turn exhaustion", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-exit-53-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("does not normalize unstructured turn-limit text into scheduler stop metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-max-turn-text-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingGeminiCommand(commandPath, {
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-gemini-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "gemini");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(promptArg).toContain("## HermesFabric Resume Delta");
      ...
      expect(promptArg).not.toContain("Follow the fabric heartbeat.");
    } finally {
      if (previousHome === undefined) {
        delete process.env.HOME;
      } else {
