...
const multiMigrationPluginKey = "fabric.dbfixture";
const llmWikiPluginKey = "hermes-fabric.plugin-llm-wiki";
...
describe("buildPluginWorkerEnv", () => {
  ...
  it("passes only model provider keys through to environment driver plugins", () => {
    ...
    expect(env).toEqual({
      ...
      HERMES_FABRIC_DEPLOYMENT_EXPOSURE: "public",
      ANTHROPIC_API_KEY: "anthropic-token",
      OPENAI_API_KEY: "openai-token",
    });
  });
...
describeEmbeddedPostgres("plugin database namespaces", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-plugin-db-");
    db = createDb(tempDb.connectionString);
  }, 20_000);

  afterEach(async () => {
    for (const pluginKey of ["fabric.dbtest", "fabric.escape", "fabric.refresh", multiMigrationPluginKey, llmWikiPluginKey]) {
  ...
  async function createPluginPackage(manifest: HermesFabricPluginManifestV1, migrationSql: string) {
    const packageRoot = await mkdtemp(path.join(os.tmpdir(), "fabric-plugin-package-"));
  ...
  function llmWikiManifest(): HermesFabricPluginManifestV1 {
    return {
      id: llmWikiPluginKey,
      apiVersion: 1,
      version: "0.1.0",
  ...
  async function createInstallablePluginPackage(
    pluginManifest: HermesFabricPluginManifestV1,
    migrationSql: string,
  ) {
    ...
    await writeFile(
      ...
      JSON.stringify({
        ...
        type: "module",
        fabricPlugin: { manifest: "./manifest.js" },
      }),
      "utf8",
    );
    await writeFile(
  ...
  async function installPluginRecord(manifest: HermesFabricPluginManifestV1) {
  ...
  function manifest(pluginKey = "fabric.dbtest"): HermesFabricPluginManifestV1 {
    return {
      id: pluginKey,
      apiVersion: 1,
      version: "1.0.0",
  ...
  it("applies the bundled LLM Wiki migrations through the production validator", async () => {
    ...
    expect(constraints).not.toContain("fabric_distillation_cursor_company_id_wiki_id_source_sco_key");
    expect(constraints).not.toContain("fabric_distillation_work_i_company_id_wiki_id_idempotenc_key");
    expect(constraints).not.toContain("fabric_page_bindings_company_id_wiki_id_page_path_key");
    expect(uniqueColumnSets).not.toContain("wiki_pages:company_id,wiki_id,path");
    expect(uniqueColumnSets).not.toContain("fabric_distillation_cursors:company_id,wiki_id,source_scope,scope_key,source_kind");
    expect(uniqueColumnSets).not.toContain("fabric_distillation_work_items:company_id,wiki_id,idempotency_key");
    expect(uniqueColumnSets).not.toContain("fabric_page_bindings:company_id,wiki_id,page_path");
  });
  ...
  it("records a failed migration when SQL escapes the plugin namespace", async () => {
    const pluginManifest = manifest("fabric.escape");
    const packageRoot = await createPluginPackage(
      pluginManifest,
  ...
  it("refreshes persisted manifests from disk before activation", async () => {
    const staleManifest = manifest("fabric.refresh");
    const refreshedManifest: HermesFabricPluginManifestV1 = {
      ...staleManifest,
