...
import { canonicalizeExternalObjectUrl } from "@hermes-fabric/shared/external-objects-server";
...
describeEmbeddedPostgres("externalObjectService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-external-objects-");
  ...
  it("uses a mock plugin provider to detect and resolve non-GitHub objects", async () => {
    ...
    const manifest: HermesFabricPluginManifestV1 = {
      id: "fabric.mock-object-provider",
      apiVersion: 1,
      version: "1.0.0",
    ...
    const [plugin] = await db.insert(plugins).values({
      ...
      packageName: "@fabric/mock-object-provider",
