...
describe("forbidden token check", () => {
  it("derives username tokens without relying on whoami", () => {
    const tokens = resolveDynamicForbiddenTokens(
      { USER: "fabric", LOGNAME: "fabric", USERNAME: "pc" },
      {
        userInfo: () => ({ username: "fabric" }),
      },
    );

    expect(tokens).toEqual(["fabric", "pc"]);
  });
  ...
  it("merges dynamic and file-based forbidden tokens", async () => {
    ...
    try {
      const tokens = resolveForbiddenTokens(tokensFile, { USER: "fabric" }, {
        userInfo: () => ({ username: "fabric" }),
      });

      expect(tokens).toEqual(["fabric", "custom-token"]);
    } finally {
