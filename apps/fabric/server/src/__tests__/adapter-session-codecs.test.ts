...
import { sessionCodec as claudeSessionCodec } from "@hermes-fabric/adapter-claude-local/server";
import { sessionCodec as codexSessionCodec, isCodexUnknownSessionError } from "@hermes-fabric/adapter-codex-local/server";
import {
...
import { sessionCodec as acpxSessionCodec } from "@hermes-fabric/adapter-acpx-local/server";
...
describe("adapter session codecs", () => {
  ...
  it("preserves acpx session params required for compatibility checks", () => {
    const parsed = acpxSessionCodec.deserialize({
      sessionKey: "fabric:company:agent:task:fingerprint",
