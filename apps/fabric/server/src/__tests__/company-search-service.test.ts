...
import { companySearchQuerySchema, COMPANY_SEARCH_MAX_QUERY_LENGTH } from "@hermes-fabric/shared";
import {
...
describeEmbeddedPostgres("companySearchService", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-company-search-");
  ...
  async function createCompany(name = "Hermes Fabric") {
