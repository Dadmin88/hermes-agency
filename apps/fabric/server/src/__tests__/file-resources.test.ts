...
import { activityLog, agents, companies, createDb, executionWorkspaces, goals, issues, projects, projectWorkspaces, type Db } from "@hermes-fabric/db";
...
async function makeWorkspace() {
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-file-resources-"));
...
describeEmbeddedPostgres("workspace file resources", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-file-resources-");
...
describeEmbeddedPostgres("file resource route guards", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-file-resource-guards-");
    db = createDb(tempDb.connectionString);
  }, 60_000);
