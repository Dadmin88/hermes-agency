...
import { execute } from "@hermes-fabric/adapter-codex-local/server";
...
const payload = {
  ...
  fabricWakePayloadJson: process.env.HERMES_FABRIC_WAKE_PAYLOAD_JSON || null,
  ...
  fabricEnvKeys: Object.keys(process.env)
...
describe("codex execute", () => {
  it("uses a HermesFabric-managed CODEX_HOME outside worktree mode while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-default-"));
    ...
    const previousHermesFabricInWorktree = process.env.HERMES_FABRIC_IN_WORKTREE;
    const previousCodexHome = process.env.CODEX_HOME;
    process.env.HOME = root;
    process.env.HERMES_FABRIC_HOME = fabricHome;
    delete process.env.HERMES_FABRIC_INSTANCE_ID;
    delete process.env.HERMES_FABRIC_IN_WORKTREE;
    ...
    try {
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining("Using HermesFabric-managed Codex home"),
        }),
      );
    } finally {
      ...
      if (previousHermesFabricInWorktree === undefined) delete process.env.HERMES_FABRIC_IN_WORKTREE;
      else process.env.HERMES_FABRIC_IN_WORKTREE = previousHermesFabricInWorktree;
  ...
  it("emits a command note that Codex auto-applies repo-scoped AGENTS.md files", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-notes-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("logs HOME and the resolved executable path in invocation metadata", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-meta-"));
    const workspace = path.join(root, "workspace");
  ...
  it("injects bridge env into sandbox-managed remote runs", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-sandbox-"));
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.codexHome).toBe(path.join(remoteWorkspace, ".fabric-runtime", "codex", "home"));
  ...
  it("injects structured HermesFabric wake payloads into env and prompt", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const capture = JSON.parse(await fs.readFile(capturePath, "utf8")) as CapturePayload;
      expect(capture.fabricEnvKeys).toContain("HERMES_FABRIC_WAKE_PAYLOAD_JSON");
      expect(capture.fabricWakePayloadJson).not.toBeNull();
      expect(JSON.parse(capture.fabricWakePayloadJson ?? "{}")).toMatchObject({
        reason: "issue_commented",
        latestCommentId: "comment-2",
        commentIds: ["comment-1", "comment-2"],
      });
      expect(capture.prompt).toContain("## HermesFabric Wake Payload");
  ...
  it("classifies remote-compaction high-demand failures as retryable transient upstream errors", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-transient-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("persists retry-not-before metadata for codex usage-limit failures", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-usage-limit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    await fs.mkdir(workspace, { recursive: true });
    await writeFailingCodexCommand(
  ...
  it("uses safer invocation settings and a fresh-session handoff for codex transient fallback retries", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-fallback-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      expect(capture.prompt).toContain("HermesFabric session handoff:");
  ...
  it("renders execution-stage wake instructions for reviewer and executor roles", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-stage-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
    ...
    try {
      ...
      const executorResult = await execute({
        ...
        config: {
          ...
          cwd: workspace,
          env: {
            HERMES_FABRIC_TEST_CAPTURE_PATH: executorCapturePath,
          },
          promptTemplate: "Follow the fabric heartbeat.",
        },
        context: {
  ...
  it("renders an issue-scoped wake prompt even when the wake has no comments yet", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-issue-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    await fs.mkdir(workspace, { recursive: true });
  ...
  it("uses a compact wake delta instead of the full heartbeat prompt when resuming a session", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-resume-wake-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    ...
    try {
      ...
      expect(capture.prompt).toContain("## HermesFabric Resume Delta");
      expect(capture.prompt).toContain("Do not switch to another issue until you have handled this wake.");
      expect(capture.prompt).toContain("Second comment");
      expect(capture.prompt).not.toContain("Follow the fabric heartbeat.");
      expect(capture.prompt).not.toContain("You are managed instructions.");
      expect(invocationPrompt).toContain("## HermesFabric Resume Delta");
  ...
  it("uses a worktree-isolated CODEX_HOME while preserving shared auth and config", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    const homeSkill = path.join(isolatedCodexHome, "skills", "fabric");
    await fs.mkdir(workspace, { recursive: true });
    await fs.mkdir(sharedCodexHome, { recursive: true });
    await fs.writeFile(path.join(sharedCodexHome, "auth.json"), '{"token":"shared"}\n', "utf8");
    await fs.writeFile(path.join(sharedCodexHome, "config.toml"), 'model = "codex-mini-latest"\n', "utf8");
    ...
    process.env.HOME = root;
    process.env.HERMES_FABRIC_HOME = fabricHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "worktree-1";
    ...
    try {
      ...
      expect(capture.argv).toEqual(expect.arrayContaining(["exec", "--json", "-"]));
      expect(capture.prompt).toContain("Follow the fabric heartbeat.");
      expect(capture.fabricEnvKeys).toEqual(
      ...
      expect(logs).toContainEqual(
        expect.objectContaining({
          stream: "stdout",
          chunk: expect.stringContaining('Injected Codex skill "fabric"'),
        }),
      );
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
  ...
  it("respects an explicit CODEX_HOME config override even in worktree mode", async () => {
    const root = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-codex-execute-explicit-"));
    const workspace = path.join(root, "workspace");
    const commandPath = path.join(root, "codex");
    const capturePath = path.join(root, "capture.json");
    const sharedCodexHome = path.join(root, "shared-codex-home");
    ...
    try {
      ...
      expect((await fs.lstat(path.join(explicitCodexHome, "skills", "fabric"))).isSymbolicLink()).toBe(true);
      await expect(fs.lstat(path.join(fabricHome, "instances", "worktree-1", "codex-home"))).rejects.toThrow();
    } finally {
      if (previousHome === undefined) delete process.env.HOME;
      else process.env.HOME = previousHome;
      if (previousHermesFabricHome === undefined) delete process.env.HERMES_FABRIC_HOME;
