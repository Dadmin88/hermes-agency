...
import { agents, companies, createDb } from "@hermes-fabric/db";
import {
...
describeEmbeddedPostgres("teams catalog install with no caller adapter overrides", () => {
  ...
  beforeAll(async () => {
    ...
    tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-teams-catalog-no-overrides-"));
    process.env.HERMES_FABRIC_HOME = tempHome;
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-teams-catalog-no-overrides-");
