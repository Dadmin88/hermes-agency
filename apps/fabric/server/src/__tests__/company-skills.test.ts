...
describe("project workspace skill discovery", () => {
  ...
  it("finds bounded skill roots under supported workspace paths", async () => {
    const workspace = await makeTempDir("fabric-skill-workspace-");
    ...
    await writeSkillDir(path.join(workspace, "skills", ".system", "fabric"), "Hermes Fabric");
    ...
    expect(discovered).toEqual([
      ...
      { skillDir: path.resolve(workspace, "skills", ".system", "fabric"), inventoryMode: "full" },
  ...
  it("limits root SKILL.md imports to skill-related support folders", async () => {
    const workspace = await makeTempDir("fabric-root-skill-");
  ...
  it("parses inline object array items in skill frontmatter metadata", async () => {
    const workspace = await makeTempDir("fabric-inline-skill-yaml-");
  ...
  it("parses folded and literal block scalar descriptions in skill frontmatter", async () => {
    const foldedWorkspace = await makeTempDir("fabric-folded-skill-yaml-");
    ...
    const literalWorkspace = await makeTempDir("fabric-literal-skill-yaml-");
    ...
    const stripWorkspace = await makeTempDir("fabric-strip-skill-yaml-");
  ...
  it("parses YAML block-scalar chomping variants from SKILL.md frontmatter", async () => {
    const workspace = await makeTempDir("fabric-block-scalar-chomp-skill-");
    await fs.mkdir(workspace, { recursive: true });
    await fs.writeFile(
      path.join(workspace, "SKILL.md"),
      [
...
describe("missing local skill reconciliation", () => {
  it("flags local-path skills whose directory was removed", async () => {
    const workspace = await makeTempDir("fabric-missing-skill-dir-");
  ...
  it("flags local-path skills whose SKILL.md file was removed", async () => {
    const workspace = await makeTempDir("fabric-missing-skill-file-");
    const skillDir = path.join(workspace, "skills", "ghost");
    await writeSkillDir(skillDir, "Ghost");
