...
describeEmbeddedPostgres("plugin-managed agents", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-plugin-managed-agents-");
  ...
  async function seedCompanyAndPlugin(options: { requireApproval?: boolean; manifest?: HermesFabricPluginManifestV1 } = {}) {
    ...
    await db.insert(plugins).values({
      ...
      packageName: "@hermes-fabric/plugin-managed-agents-test",
  ...
  it("materializes declared managed agent instructions with local folder paths", async () => {
    const previousHome = process.env.HERMES_FABRIC_HOME;
    const previousInstance = process.env.HERMES_FABRIC_INSTANCE_ID;
    const tempHome = await fs.mkdtemp(path.join(os.tmpdir(), "fabric-managed-agent-home-"));
    const wikiRoot = await fs.realpath(await fs.mkdtemp(path.join(os.tmpdir(), "fabric-managed-agent-wiki-")));
    process.env.HERMES_FABRIC_HOME = tempHome;
    process.env.HERMES_FABRIC_INSTANCE_ID = "test";
    try {
      const pluginManifest = manifest();
      ...
      if (previousHome === undefined) delete process.env.HERMES_FABRIC_HOME;
      else process.env.HERMES_FABRIC_HOME = previousHome;
      if (previousInstance === undefined) delete process.env.HERMES_FABRIC_INSTANCE_ID;
      else process.env.HERMES_FABRIC_INSTANCE_ID = previousInstance;
  ...
  it("respects board approval policy for new managed agents", async () => {
    ...
    expect(approval?.payload).toMatchObject({
      agentId: created.agentId,
      sourcePluginKey: "fabric.managed-agents-test",
