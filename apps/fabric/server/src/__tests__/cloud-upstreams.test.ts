...
import { companies, cloudUpstreamConnections, cloudUpstreamRuns, companySkills, createDb } from "@hermes-fabric/db";
...
describe("cloud upstream credential storage", () => {
  const previousMasterKey = process.env.HERMES_FABRIC_SECRETS_MASTER_KEY;

  afterEach(() => {
    if (previousMasterKey === undefined) {
      delete process.env.HERMES_FABRIC_SECRETS_MASTER_KEY;
    } else {
      process.env.HERMES_FABRIC_SECRETS_MASTER_KEY = previousMasterKey;
    }
  });

  it("stores new credentials as encrypted envelopes and preserves legacy plaintext reads", async () => {
    process.env.HERMES_FABRIC_SECRETS_MASTER_KEY = "12345678901234567890123456789012";
    const sealed = await sealCloudUpstreamCredential("cloud-access-token");

    expect(sealed).toMatch(/^fabric-cloud-credential:/);
...
describeEmbeddedPostgres("cloud upstream persistence", () => {
  ...
  beforeAll(async () => {
    process.env.HERMES_FABRIC_SECRETS_MASTER_KEY = "12345678901234567890123456789012";
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-cloud-upstreams-");
  ...
  it("encrypts stored upstream credentials while keeping connection flows usable", async () => {
    ...
    vi.spyOn(globalThis, "fetch").mockImplementation(async (input, init) => {
      ...
      if (url.startsWith("https://cloud.example.test/.well-known/fabric-upstream")) {
        return jsonResponse({
          product: "HermesFabric Cloud",
          stack: {
            id: "stack-1",
      ...
      if (url === tokenUrl && init?.method === "POST") {
        ...
        expect(payload.codeVerifier).not.toContain("fabric-cloud-credential:");
        return jsonResponse({
    ...
    expect(row.privateKeyPem).toMatch(/^fabric-cloud-credential:/);
    expect(row.privateKeyPem).not.toContain("BEGIN PRIVATE KEY");
    expect(row.accessToken).toMatch(/^fabric-cloud-credential:/);
  ...
  it("marks orphaned running runs failed during startup reconciliation", async () => {
    ...
    await db.insert(cloudUpstreamConnections).values({
      ...
      targetProduct: "HermesFabric Cloud",
      targetSchemaMajor: 1,
