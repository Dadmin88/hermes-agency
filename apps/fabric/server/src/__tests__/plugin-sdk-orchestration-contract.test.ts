...
import type { Issue, HermesFabricPluginManifestV1 } from "@hermes-fabric/shared";
import { createTestHarness } from "../../../packages/plugins/sdk/src/testing.js";

function manifest(capabilities: HermesFabricPluginManifestV1["capabilities"]): HermesFabricPluginManifestV1 {
  return {
    id: "fabric.test-orchestration",
    apiVersion: 1,
    version: "0.1.0",
...
describe("plugin SDK orchestration contract", () => {
  it("supports expanded issue create fields and relation helpers", async () => {
    ...
    expect(created.originKind).toBe("plugin:fabric.test-orchestration");
  ...
  it("enforces plugin origin namespaces in the test harness", async () => {
    ...
    const created = await harness.ctx.issues.create({
      companyId,
      title: "Generated issue",
      originKind: "plugin:fabric.test-orchestration:feature",
    });

    expect(created.originKind).toBe("plugin:fabric.test-orchestration:feature");
    await expect(
    ...
    ).rejects.toThrow("Plugin may only use originKind values under plugin:fabric.test-orchestration");
    await expect(
  ...
  it("supports generic plugin operation issue visibility in the test harness", async () => {
    ...
    expect(created.originKind).toBe("plugin:fabric.test-orchestration:operation");
