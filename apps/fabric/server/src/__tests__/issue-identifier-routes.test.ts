...
import { companies, companyMemberships, createDb, issues } from "@hermes-fabric/db";
import {
...
describeEmbeddedPostgres("issue identifier routes", () => {
  ...
  beforeAll(async () => {
    tempDb = await startEmbeddedPostgresTestDatabase("fabric-issue-identifier-routes-");
