...
describe("redaction", () => {
  ...
  it("redacts common secret shapes from unstructured text", () => {
    ...
    const input = [
      ...
      `fabric {"HERMES_FABRIC_API_KEY":"fabric-json-secret"}`,
    ...
    expect(result).not.toContain("fabric-json-secret");
    expect(result).not.toContain("escaped-json-secret");
    expect(result).not.toContain("fabric-shell-secret");
  ...
  it("redacts inline secrets from command metadata without hiding safe command text", () => {
    const input = {
      ...
      env: {
        HERMES_FABRIC_RESOLVED_COMMAND: "env OPENAI_API_KEY=sk-live-example custom-acp --token ghp_example_secret",
        SAFE_VALUE: "visible",
      },
    };
    ...
    expect(result?.env).toEqual({
      HERMES_FABRIC_RESOLVED_COMMAND:
