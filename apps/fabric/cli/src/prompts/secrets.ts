...
import type { SecretProvider } from "@hermes-fabric/shared";
import type { SecretsConfig } from "../config/schema.js";
import { resolveDefaultSecretsKeyFilePath, resolveHermesFabricInstanceId } from "../config/home.js";
  ...
  if (provider !== "local_encrypted") {
    p.note(
      provider === "aws_secrets_manager"
        ? "AWS credentials must come from the HermesFabric server runtime (IAM role/workload identity, AWS_PROFILE/SSO/shared credentials, or short-lived shell env), not from HermesFabric company secrets."
