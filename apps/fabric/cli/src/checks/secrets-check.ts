...
export function secretsCheck(config: HermesFabricConfig, configPath?: string): CheckResult {
  ...
  if (provider !== "local_encrypted") {
    return {
      ...
      canRepair: false,
      repairHint: "Run `hermes-fabric configure --section secrets` and choose local_encrypted or aws_secrets_manager",
    };
  }

  const envMasterKey = process.env.HERMES_FABRIC_SECRETS_MASTER_KEY;
  if (envMasterKey && envMasterKey.trim().length > 0) {
    if (!decodeMasterKey(envMasterKey)) {
      return {
        ...
        canRepair: false,
        repairHint: "Set HERMES_FABRIC_SECRETS_MASTER_KEY to a valid key or unset it to use a key file",
      };
    }
    ...
    return withStrictModeNote(
      {
        name: "Secrets adapter",
        status: "pass",
        message: "Local encrypted provider configured via HERMES_FABRIC_SECRETS_MASTER_KEY",
      },
      config,
    );
  }

  const keyFileOverride = process.env.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE;
  ...
  try {
    ...
    return {
      ...
      canRepair: false,
      repairHint: "Check file permissions or set HERMES_FABRIC_SECRETS_MASTER_KEY",
    };
  }
...
function awsSecretsManagerCheck(): CheckResult {
  ...
  if (missingConfig.length > 0) {
    return {
      ...
      canRepair: false,
      repairHint:
        `Set ${missingConfig.join(", ")} in the HermesFabric server runtime. ${AWS_CREDENTIAL_SOURCE_HINT}. Do not store AWS root credentials or long-lived IAM user keys in HermesFabric secrets.`,
    };
  }
  ...
  const message =
    `AWS Secrets Manager provider configured for deployment ${process.env.HERMES_FABRIC_SECRETS_AWS_DEPLOYMENT_ID}; ` +
  ...
  if (staticEnvCredentials) {
    return {
      ...
      canRepair: false,
      repairHint:
        "AWS static environment credentials are visible. Use only short-lived shell credentials locally; prefer IAM role/workload identity for hosted deployments and never store AWS access keys in HermesFabric company secrets.",
    };
  }
...
function missingAwsSecretsManagerConfig(): string[] {
  ...
  if (
    !(
      process.env.HERMES_FABRIC_SECRETS_AWS_REGION?.trim() ||
      process.env.AWS_REGION?.trim() ||
      process.env.AWS_DEFAULT_REGION?.trim()
    )
  ) {
    missing.push("HERMES_FABRIC_SECRETS_AWS_REGION or AWS_REGION/AWS_DEFAULT_REGION");
  }
  if (!process.env.HERMES_FABRIC_SECRETS_AWS_DEPLOYMENT_ID?.trim()) {
    missing.push("HERMES_FABRIC_SECRETS_AWS_DEPLOYMENT_ID");
  }
  if (!process.env.HERMES_FABRIC_SECRETS_AWS_KMS_KEY_ID?.trim()) {
    missing.push("HERMES_FABRIC_SECRETS_AWS_KMS_KEY_ID");
  }
  return missing;
}
