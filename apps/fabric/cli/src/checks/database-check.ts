...
export async function databaseCheck(config: HermesFabricConfig, configPath?: string): Promise<CheckResult> {
  if (config.database.mode === "postgres") {
    if (!config.database.connectionString) {
      return {
        ...
        canRepair: false,
        repairHint: "Run `hermes-fabric configure --section database`",
      };
    }
