...
async function openConfiguredDb(configPath: string): Promise<{
  ...
  try {
    if (config.database.mode === "embedded-postgres") {
      ...
      const adminConnectionString = `postgres://fabric:fabric@127.0.0.1:${embeddedHandle.port}/postgres`;
      await ensurePostgresDatabase(adminConnectionString, "fabric");
      const connectionString = `postgres://fabric:fabric@127.0.0.1:${embeddedHandle.port}/fabric`;
...
export async function disableAllRoutinesInConfig(
  ...
  const companyId =
    ...
    ?? nonEmpty(process.env.HERMES_FABRIC_COMPANY_ID)
    ?? null;
  if (!companyId) {
    throw new Error("Company ID is required. Pass --company-id or set HERMES_FABRIC_COMPANY_ID.");
  }
