...
export const DEFAULT_WORKTREE_HOME = "~/.hermes-fabric-worktrees";
...
export function resolveWorktreeLocalPaths(opts: {
  ...
  const repoConfigDir = path.resolve(cwd, ".fabric");
  return {
    cwd,
    repoConfigDir,
...
export function buildWorktreeConfig(input: {
  sourceConfig: HermesFabricConfig | null;
  ...
  return {
    ...
    storage: {
      ...
      s3: {
        bucket: source?.storage.s3.bucket ?? "fabric",
...
export function buildWorktreeEnvEntries(
  ...
  return {
    HERMES_FABRIC_HOME: paths.homeDir,
    HERMES_FABRIC_INSTANCE_ID: paths.instanceId,
    HERMES_FABRIC_CONFIG: paths.configPath,
    HERMES_FABRIC_CONTEXT: paths.contextPath,
    HERMES_FABRIC_IN_WORKTREE: "true",
    ...(branding?.name ? { HERMES_FABRIC_WORKTREE_NAME: branding.name } : {}),
    ...(branding?.color ? { HERMES_FABRIC_WORKTREE_COLOR: branding.color } : {}),
  };
}
