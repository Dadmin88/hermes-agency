...
type OnboardDefaults = Pick<HermesFabricConfig, "database" | "logging" | "server" | "auth" | "storage" | "secrets">;

const TAILNET_BIND_WARNING =
  "No Tailscale address was detected during setup. The saved config will stay on loopback until Tailscale is available or HERMES_FABRIC_TAILNET_BIND_HOST is set.";

const ONBOARD_ENV_KEYS = [
  "HERMES_FABRIC_PUBLIC_URL",
  "DATABASE_URL",
  "HERMES_FABRIC_DB_BACKUP_ENABLED",
  "HERMES_FABRIC_DB_BACKUP_INTERVAL_MINUTES",
  "HERMES_FABRIC_DB_BACKUP_RETENTION_DAYS",
  "HERMES_FABRIC_DB_BACKUP_DIR",
  "HERMES_FABRIC_DEPLOYMENT_MODE",
  "HERMES_FABRIC_DEPLOYMENT_EXPOSURE",
  "HERMES_FABRIC_BIND",
  "HERMES_FABRIC_BIND_HOST",
  "HERMES_FABRIC_TAILNET_BIND_HOST",
  "HOST",
  "PORT",
  "SERVE_UI",
  "HERMES_FABRIC_ALLOWED_HOSTNAMES",
  "HERMES_FABRIC_AUTH_BASE_URL_MODE",
  "HERMES_FABRIC_AUTH_PUBLIC_BASE_URL",
  "BETTER_AUTH_URL",
  "BETTER_AUTH_BASE_URL",
  "HERMES_FABRIC_STORAGE_PROVIDER",
  "HERMES_FABRIC_STORAGE_LOCAL_DIR",
  "HERMES_FABRIC_STORAGE_S3_BUCKET",
  "HERMES_FABRIC_STORAGE_S3_REGION",
  "HERMES_FABRIC_STORAGE_S3_ENDPOINT",
  "HERMES_FABRIC_STORAGE_S3_PREFIX",
  "HERMES_FABRIC_STORAGE_S3_FORCE_PATH_STYLE",
  "HERMES_FABRIC_SECRETS_PROVIDER",
  "HERMES_FABRIC_SECRETS_STRICT_MODE",
  "HERMES_FABRIC_SECRETS_MASTER_KEY_FILE",
] as const;
...
function describeServerBinding(server: Pick<HermesFabricConfig["server"], "bind" | "customBindHost" | "host" | "port">): string {
...
function quickstartDefaultsFromEnv(opts?: { preferTrustedLocal?: boolean }): {
  ...
  const instanceId = resolveHermesFabricInstanceId();
  ...
  const publicUrl = preferTrustedLocal
    ? undefined
    : (
      process.env.HERMES_FABRIC_PUBLIC_URL?.trim() ||
      process.env.HERMES_FABRIC_AUTH_PUBLIC_BASE_URL?.trim() ||
  ...
  const deploymentMode = preferTrustedLocal
    ? "local_trusted"
    : (parseEnumFromEnv<DeploymentMode>(process.env.HERMES_FABRIC_DEPLOYMENT_MODE, DEPLOYMENT_MODES) ?? "local_trusted");
  const deploymentExposureFromEnv = parseEnumFromEnv<DeploymentExposure>(
    process.env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE,
    DEPLOYMENT_EXPOSURES,
  );
  const deploymentExposure =
    deploymentMode === "local_trusted" ? "private" : (deploymentExposureFromEnv ?? "private");
  const bindFromEnv = parseEnumFromEnv<BindMode>(process.env.HERMES_FABRIC_BIND, BIND_MODES);
  const customBindHostFromEnv = process.env.HERMES_FABRIC_BIND_HOST?.trim() || undefined;
  ...
  const resolvedBind = resolveRuntimeBind({
    ...
    tailnetBindHost: process.env.HERMES_FABRIC_TAILNET_BIND_HOST?.trim(),
  });
  const authPublicBaseUrl = publicUrl;
  const authBaseUrlModeFromEnv = parseEnumFromEnv<AuthBaseUrlMode>(
    process.env.HERMES_FABRIC_AUTH_BASE_URL_MODE,
    AUTH_BASE_URL_MODES,
  );
  const authBaseUrlMode = authBaseUrlModeFromEnv ?? (authPublicBaseUrl ? "explicit" : "auto");
  const allowedHostnamesFromEnv = process.env.HERMES_FABRIC_ALLOWED_HOSTNAMES
    ? process.env.HERMES_FABRIC_ALLOWED_HOSTNAMES
      .split(",")
  ...
  const storageProvider =
    parseEnumFromEnv<StorageProvider>(process.env.HERMES_FABRIC_STORAGE_PROVIDER, STORAGE_PROVIDERS) ??
    defaultStorage.provider;
  const secretsProvider =
    parseEnumFromEnv<SecretProvider>(process.env.HERMES_FABRIC_SECRETS_PROVIDER, SECRET_PROVIDERS) ??
    defaultSecrets.provider;
  const databaseBackupEnabled = parseBooleanFromEnv(process.env.HERMES_FABRIC_DB_BACKUP_ENABLED) ?? true;
  const databaseBackupIntervalMinutes = Math.max(
    1,
    parseNumberFromEnv(process.env.HERMES_FABRIC_DB_BACKUP_INTERVAL_MINUTES) ?? 60,
  );
  const databaseBackupRetentionDays = Math.max(
    1,
    parseNumberFromEnv(process.env.HERMES_FABRIC_DB_BACKUP_RETENTION_DAYS) ?? 30,
  );
  const defaults: OnboardDefaults = {
    database: {
      ...
      backup: {
        ...
        dir: resolvePathFromEnv(process.env.HERMES_FABRIC_DB_BACKUP_DIR) ?? resolveDefaultBackupDir(instanceId),
      },
    },
    logging: {
      mode: "file",
    ...
    storage: {
      ...
      localDisk: {
        baseDir:
          resolvePathFromEnv(process.env.HERMES_FABRIC_STORAGE_LOCAL_DIR) ?? defaultStorage.localDisk.baseDir,
      },
      s3: {
        bucket: process.env.HERMES_FABRIC_STORAGE_S3_BUCKET ?? defaultStorage.s3.bucket,
        region: process.env.HERMES_FABRIC_STORAGE_S3_REGION ?? defaultStorage.s3.region,
        endpoint: process.env.HERMES_FABRIC_STORAGE_S3_ENDPOINT ?? defaultStorage.s3.endpoint,
        prefix: process.env.HERMES_FABRIC_STORAGE_S3_PREFIX ?? defaultStorage.s3.prefix,
        forcePathStyle:
          parseBooleanFromEnv(process.env.HERMES_FABRIC_STORAGE_S3_FORCE_PATH_STYLE) ??
          defaultStorage.s3.forcePathStyle,
      },
    },
    secrets: {
      provider: secretsProvider,
      strictMode: parseBooleanFromEnv(process.env.HERMES_FABRIC_SECRETS_STRICT_MODE) ?? defaultSecrets.strictMode,
      localEncrypted: {
        keyFilePath:
          resolvePathFromEnv(process.env.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE) ??
  ...
  if (deploymentMode === "local_trusted" && process.env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE !== undefined) {
    ignoredEnvKeys.push({
      key: "HERMES_FABRIC_DEPLOYMENT_EXPOSURE",
      reason: "Ignored because deployment mode local_trusted always forces private exposure",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.HERMES_FABRIC_BIND !== undefined) {
    ignoredEnvKeys.push({
      key: "HERMES_FABRIC_BIND",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
  if (deploymentMode === "local_trusted" && process.env.HERMES_FABRIC_BIND_HOST !== undefined) {
    ignoredEnvKeys.push({
      key: "HERMES_FABRIC_BIND_HOST",
      reason: "Ignored because deployment mode local_trusted always uses loopback reachability",
    });
  }
...
function canCreateBootstrapInviteImmediately(config: Pick<HermesFabricConfig, "database" | "server">): boolean {
...
export async function onboard(opts: OnboardOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" hermes-fabric onboard ")));
  const configPath = resolveConfigPath(opts.config);
  const instance = describeLocalInstancePaths(resolveHermesFabricInstanceId());
  p.log.message(
    pc.dim(
  ...
  let existingConfig: HermesFabricConfig | null = null;
  ...
  if (existingConfig) {
    p.log.message(
      pc.dim("Existing HermesFabric install detected; keeping the current configuration unchanged."),
    );
    p.log.message(pc.dim(`Use ${pc.cyan("hermes-fabric configure")} if you want to change settings.`));

    const jwtSecret = ensureAgentJwtSecret(configPath);
    const envFilePath = resolveAgentJwtEnvFile(configPath);
    if (jwtSecret.created) {
      p.log.success(`Created ${pc.cyan("HERMES_FABRIC_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    } else if (process.env.HERMES_FABRIC_AGENT_JWT_SECRET?.trim()) {
      p.log.info(`Using existing ${pc.cyan("HERMES_FABRIC_AGENT_JWT_SECRET")} from environment`);
    } else {
      p.log.info(`Using existing ${pc.cyan("HERMES_FABRIC_AGENT_JWT_SECRET")} in ${pc.dim(envFilePath)}`);
    }
    ...
    p.note(
      [
        ...
        "Agent auth: HERMES_FABRIC_AGENT_JWT_SECRET configured",
      ].join("\n"),
      "Configuration ready",
    );
    ...
    p.note(
      [
        `Run: ${pc.cyan("hermes-fabric run")}`,
        `Reconfigure later: ${pc.cyan("hermes-fabric configure")}`,
        `Diagnose setup: ${pc.cyan("hermes-fabric doctor")}`,
      ].join("\n"),
      "Next commands",
    );
    ...
    if (!shouldRunNow && !opts.invokedByRun && process.stdin.isTTY && process.stdout.isTTY) {
      const answer = await p.confirm({
        message: "Start HermesFabric now?",
        initialValue: true,
      });
    ...
    if (shouldRunNow && !opts.invokedByRun) {
      process.env.HERMES_FABRIC_OPEN_ON_LISTEN = "true";
      const { runCommand } = await import("./run.js");
      await runCommand({ config: configPath, repair: true, yes: true });
      return;
    }

    p.outro("Existing HermesFabric setup is ready.");
    return;
  }
  ...
  let llm: HermesFabricConfig["llm"] | undefined;
  ...
  if (setupMode === "advanced") {
    ...
    if (database.mode === "postgres" && database.connectionString) {
      ...
      try {
        const { createDb } = await import("@hermes-fabric/db");
        const db = createDb(database.connectionString);
        await db.execute("SELECT 1");
        s.stop("Database connection successful");
      } catch {
        s.stop(pc.yellow("Could not connect to database — you can fix this later with `hermes-fabric doctor`"));
      }
    }
  ...
  const config: HermesFabricConfig = {
    $meta: {
      version: 1,
  ...
  if (server.deploymentMode === "authenticated" && database.mode === "embedded-postgres") {
    p.log.info(
      [
        ...
        `Next: ${pc.cyan("hermes-fabric run")}`,
        `Then: ${pc.cyan("hermes-fabric auth bootstrap-ceo")}`,
      ].join("\n"),
    );
  }
