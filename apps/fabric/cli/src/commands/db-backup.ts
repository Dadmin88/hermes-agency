...
import { formatDatabaseBackupResult, runDatabaseBackup } from "@hermes-fabric/db";
import {
  expandHomePrefix,
...
function resolveConnectionString(configPath?: string): { value: string; source: string } {
  ...
  return {
    value: `postgres://fabric:fabric@127.0.0.1:${port}/fabric`,
...
export async function dbBackupCommand(opts: DbBackupOptions): Promise<void> {
  ...
  p.intro(pc.bgCyan(pc.black(" fabric db:backup ")));

  const configPath = resolveConfigPath(opts.config);
  const config = readConfig(opts.config);
  const connection = resolveConnectionString(opts.config);
  const defaultDir = resolveDefaultBackupDir(resolveHermesFabricInstanceId());
  ...
  const filenamePrefix = opts.filenamePrefix?.trim() || "fabric";
