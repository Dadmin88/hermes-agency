...
import { ensureAgentJwtSecret, loadHermesFabricEnvFile, mergeHermesFabricEnvEntries, readHermesFabricEnvEntries, resolveHermesFabricEnvFile } from "../config/env.js";
import { expandHomePrefix } from "../config/home.js";
import type { HermesFabricConfig } from "../config/schema.js";
import { readConfig, resolveConfigPath, writeConfig } from "../config/store.js";
import { printHermesFabricCliBanner } from "../utils/banner.js";
...
function isCurrentSourceConfigPath(sourceConfigPath: string): boolean {
  const currentConfigPath = process.env.HERMES_FABRIC_CONFIG;
...
const WORKTREE_NAME_PREFIX = "fabric-";
...
function resolveWorktreeHome(explicit?: string): string {
  return explicit ?? process.env.HERMES_FABRIC_WORKTREES_DIR ?? DEFAULT_WORKTREE_HOME;
}

function resolveWorktreeStartPoint(explicit?: string): string | undefined {
  return explicit ?? nonEmpty(process.env.HERMES_FABRIC_WORKTREE_START_POINT) ?? undefined;
}
...
function createConfiguredStorageFromHermesFabricConfig(config: HermesFabricConfig): ConfiguredStorage {
...
function openConfiguredStorage(configPath: string): ConfiguredStorage {
  ...
  return createConfiguredStorageFromHermesFabricConfig(config);
}
...
function collectClaimedWorktreePorts(homeDir: string, currentInstanceId: string, cwd: string): {
  ...
  if (repoManagedWorktreesRoot && existsSync(repoManagedWorktreesRoot)) {
    for (const entry of readdirSync(repoManagedWorktreesRoot, { withFileTypes: true })) {
      ...
      const configPath = path.resolve(repoManagedWorktreesRoot, entry.name, ".fabric", "config.json");
      if (existsSync(configPath)) {
        configPaths.add(configPath);
      }
    }
...
export function resolveSourceConfigPath(opts: WorktreeInitOptions): string {
  ...
  const sourceHome = path.resolve(expandHomePrefix(opts.fromDataDir ?? "~/.hermes-fabric"));
...
export function resolveWorktreeReseedTargetPaths(input: {
  ...
  const envEntries = readHermesFabricEnvEntries(resolveHermesFabricEnvFile(input.configPath));
  const homeDir = nonEmpty(envEntries.HERMES_FABRIC_HOME);
  const instanceId = nonEmpty(envEntries.HERMES_FABRIC_INSTANCE_ID);

  if (!homeDir || !instanceId) {
    throw new Error(
      `Target config ${input.configPath} does not look like a worktree-local HermesFabric instance. Expected HERMES_FABRIC_HOME and HERMES_FABRIC_INSTANCE_ID in the adjacent .env.`,
    );
  }
...
function resolveExistingGitWorktree(selector: string, cwd: string): MergeSourceChoice | null {
  ...
  if (existsSync(directPath)) {
    return {
      ...
      hasHermesFabricConfig: existsSync(path.resolve(directPath, ".fabric", "config.json")),
...
async function ensureRepairTargetWorktree(input: {
  ...
  const currentConfigPath = path.resolve(currentRoot, ".fabric", "config.json");
  ...
  if (existing) {
    return {
      ...
      configPath: path.resolve(existing.worktree, ".fabric", "config.json"),
  ...
  return {
    rootPath: targetPath,
    configPath: path.resolve(targetPath, ".fabric", "config.json"),
    label: branchName,
    branchName,
    created: true,
  };
...
function resolveSourceConnectionString(config: HermesFabricConfig, envEntries: Record<string, string>, portOverride?: number): string {
  ...
  return `postgres://fabric:fabric@127.0.0.1:${port}/fabric`;
}

export function copySeededSecretsKey(input: {
  sourceConfigPath: string;
  sourceConfig: HermesFabricConfig;
  ...
  const sourceInlineMasterKey =
    nonEmpty(input.sourceEnvEntries.HERMES_FABRIC_SECRETS_MASTER_KEY) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HERMES_FABRIC_SECRETS_MASTER_KEY) : null);
  ...
  const sourceKeyFileOverride =
    nonEmpty(input.sourceEnvEntries.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE) ??
    (allowProcessEnvFallback ? nonEmpty(process.env.HERMES_FABRIC_SECRETS_MASTER_KEY_FILE) : null);
...
async function seedWorktreeDatabase(input: {
  sourceConfigPath: string;
  sourceConfig: HermesFabricConfig;
  targetConfig: HermesFabricConfig;
  ...
  const sourceEnvFile = resolveHermesFabricEnvFile(input.sourceConfigPath);
  const sourceEnvEntries = readHermesFabricEnvEntries(sourceEnvFile);
  ...
  try {
    if (input.sourceConfig.database.mode === "embedded-postgres") {
      ...
      const sourceAdminConnectionString = `postgres://fabric:fabric@127.0.0.1:${sourceHandle.port}/postgres`;
      await ensurePostgresDatabase(sourceAdminConnectionString, "fabric");
    }
    ...
    const adminConnectionString = `postgres://fabric:fabric@127.0.0.1:${targetHandle.port}/postgres`;
    await ensurePostgresDatabase(adminConnectionString, "fabric");
    const targetConnectionString = `postgres://fabric:fabric@127.0.0.1:${targetHandle.port}/fabric`;
...
async function runWorktreeInit(opts: WorktreeInitOptions): Promise<void> {
  ...
  if (opts.force) {
    ...
    // <repo>/.fabric/worktrees/ holding every repo-managed worktree
  ...
  const sourceEnvEntries = readHermesFabricEnvEntries(resolveHermesFabricEnvFile(sourceConfigPath));
  const existingAgentJwtSecret =
    nonEmpty(sourceEnvEntries.HERMES_FABRIC_AGENT_JWT_SECRET) ??
    nonEmpty(process.env.HERMES_FABRIC_AGENT_JWT_SECRET);
  mergeHermesFabricEnvEntries(
    {
      ...buildWorktreeEnvEntries(paths, branding),
      ...(existingAgentJwtSecret ? { HERMES_FABRIC_AGENT_JWT_SECRET: existingAgentJwtSecret } : {}),
    },
    paths.envPath,
  );
  ensureAgentJwtSecret(paths.configPath);
  loadHermesFabricEnvFile(paths.configPath);
  ...
  p.outro(
    pc.green(
      `Worktree ready. Run HermesFabric inside this repo and the CLI/server will use ${paths.instanceId} automatically.`,
    ),
  );
}

export async function worktreeInitCommand(opts: WorktreeInitOptions): Promise<void> {
  printHermesFabricCliBanner();
  p.intro(pc.bgCyan(pc.black(" hermes-fabric worktree init ")));
  await runWorktreeInit(opts);
}

export async function worktreeMakeCommand(nameArg: string, opts: WorktreeMakeOptions): Promise<void> {
  printHermesFabricCliBanner();
  p.intro(pc.bgCyan(pc.black(" hermes-fabric worktree:make ")));
...
function toMergeSourceChoices(cwd: string): MergeSourceChoice[] {
  ...
  return parseGitWorktreeList(cwd).map((entry) => {
    ...
    return {
      ...
      branch: entry.branch,
      branchLabel,
      hasHermesFabricConfig: existsSync(path.resolve(worktreePath, ".fabric", "config.json")),
...
export async function worktreeCleanupCommand(nameArg: string, opts: WorktreeCleanupOptions): Promise<void> {
  printHermesFabricCliBanner();
  p.intro(pc.bgCyan(pc.black(" hermes-fabric worktree:cleanup ")));
...
export async function worktreeEnvCommand(opts: WorktreeEnvOptions): Promise<void> {
  ...
  const envPath = resolveHermesFabricEnvFile(configPath);
  const envEntries = readHermesFabricEnvEntries(envPath);
  const out = {
    HERMES_FABRIC_CONFIG: configPath,
    ...(envEntries.HERMES_FABRIC_HOME ? { HERMES_FABRIC_HOME: envEntries.HERMES_FABRIC_HOME } : {}),
    ...(envEntries.HERMES_FABRIC_INSTANCE_ID ? { HERMES_FABRIC_INSTANCE_ID: envEntries.HERMES_FABRIC_INSTANCE_ID } : {}),
    ...(envEntries.HERMES_FABRIC_CONTEXT ? { HERMES_FABRIC_CONTEXT: envEntries.HERMES_FABRIC_CONTEXT } : {}),
    ...envEntries,
  };
...
function resolveAttachmentLookupStorages(input: {
  ...
  const orderedConfigPaths = [
    ...
    ...toMergeSourceChoices(process.cwd())
      .filter((choice) => choice.hasHermesFabricConfig)
      .map((choice) => path.resolve(choice.worktree, ".fabric", "config.json")),
  ];
...
async function openConfiguredDb(configPath: string): Promise<OpenDbHandle> {
  ...
  const envEntries = readHermesFabricEnvEntries(resolveHermesFabricEnvFile(configPath));
    ...
    if (migrationState.status !== "upToDate") {
      const pending =
        migrationState.reason === "pending-migrations"
          ...
          : "";
      throw new Error(
        `Database for ${configPath} is not up to date.${pending} Run \`pnpm db:migrate\` (or start HermesFabric once) before using worktree merge history.`,
      );
    }
...
function resolveRunningEmbeddedPostgresPid(config: HermesFabricConfig): number | null {
  if (config.database.mode !== "embedded-postgres") {
    return null;
  }
...
export async function worktreeListCommand(opts: WorktreeListOptions): Promise<void> {
  ...
  for (const choice of choices) {
    const flags = [
      ...
      choice.hasHermesFabricConfig ? "fabric" : "no-fabric-config",
...
function resolveEndpointFromChoice(choice: MergeSourceChoice): ResolvedWorktreeEndpoint {
  ...
  return {
    ...
    configPath: path.resolve(choice.worktree, ".fabric", "config.json"),
...
function resolveWorktreeEndpointFromSelector(
  ...
  if (existsSync(directPath)) {
    ...
    const configPath = path.resolve(directPath, ".fabric", "config.json");
    if (!existsSync(configPath)) {
      throw new Error(`Resolved worktree path ${directPath} does not contain .fabric/config.json.`);
    }
    return {
      rootPath: directPath,
      configPath,
  ...
  if (!matched.hasHermesFabricConfig && !matched.isCurrent) {
    throw new Error(`Resolved worktree "${selector}" does not look like a HermesFabric worktree.`);
  }
...
async function promptForSourceEndpoint(excludeWorktreePath?: string): Promise<ResolvedWorktreeEndpoint> {
  ...
  const currentEndpoint = resolveCurrentEndpoint();
  const choices = toMergeSourceChoices(process.cwd())
    .filter((choice) => choice.hasHermesFabricConfig || choice.isCurrent)
  ...
  if (choices.length === 0) {
    throw new Error("No HermesFabric worktrees were found. Run `hermes-fabric worktree:list` to inspect the repo worktrees.");
  }
...
export async function worktreeMergeHistoryCommand(sourceArg: string | undefined, opts: WorktreeMergeHistoryOptions): Promise<void> {
  ...
  if (path.resolve(sourceEndpoint.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target HermesFabric configs are the same. Choose different --from/--to worktrees.");
  }
...
async function runWorktreeReseed(opts: WorktreeReseedOptions): Promise<void> {
  ...
  if (path.resolve(source.configPath) === path.resolve(targetEndpoint.configPath)) {
    throw new Error("Source and target HermesFabric configs are the same. Choose different --from/--to values.");
  }
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop HermesFabric in ${targetEndpoint.rootPath} before reseeding, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  ...
  const confirmed = opts.yes
    ? true
    : await p.confirm({
      message: `Overwrite the isolated HermesFabric DB for ${targetEndpoint.label} from ${source.label} using ${seedMode} seed mode?`,
      initialValue: false,
    });
  if (p.isCancel(confirmed) || !confirmed) {
...
export async function worktreeReseedCommand(opts: WorktreeReseedOptions): Promise<void> {
  printHermesFabricCliBanner();
  p.intro(pc.bgCyan(pc.black(" hermes-fabric worktree reseed ")));
  await runWorktreeReseed(opts);
}

export async function worktreeRepairCommand(opts: WorktreeRepairOptions): Promise<void> {
  printHermesFabricCliBanner();
  p.intro(pc.bgCyan(pc.black(" hermes-fabric worktree repair ")));
  ...
  if (path.resolve(source.configPath) === path.resolve(target.configPath)) {
    throw new Error("Source and target HermesFabric configs are the same. Use --from-config/--from-instance to point repair at a different source.");
  }

  const targetConfig = existsSync(target.configPath) ? readConfig(target.configPath) : null;
  const targetEnvEntries = readHermesFabricEnvEntries(resolveHermesFabricEnvFile(target.configPath));
  const targetHasWorktreeEnv = Boolean(
    nonEmpty(targetEnvEntries.HERMES_FABRIC_HOME) && nonEmpty(targetEnvEntries.HERMES_FABRIC_INSTANCE_ID),
  );
  ...
  if (runningTargetPid && !opts.allowLiveTarget) {
    throw new Error(
      `Target worktree database appears to be running (pid ${runningTargetPid}). Stop HermesFabric in ${target.rootPath} before repairing, or re-run with --allow-live-target if you want to override this guard.`,
    );
  }
  if (runningTargetPid && opts.allowLiveTarget) {
    p.log.warning(`Proceeding even though the target embedded PostgreSQL appears to be running (pid ${runningTargetPid}).`);
...
export function registerWorktreeCommands(program: Command): void {
  const worktree = program.command("worktree").description("Worktree-local HermesFabric instance helpers");

  program
    .command("worktree:make")
    .description("Create ~/NAME as a git worktree, then initialize an isolated HermesFabric instance inside it")
    .argument("<name>", "Worktree name — auto-prefixed with fabric- if needed (created at ~/fabric-NAME)")
    .option("--start-point <ref>", "Remote ref to base the new branch on (env: HERMES_FABRIC_WORKTREE_START_POINT)")
    .option("--instance <id>", "Explicit isolated instance id")
    .option("--home <path>", `Home root for worktree instances (env: HERMES_FABRIC_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source HERMES_FABRIC_HOME used when deriving the source config")
  ...
  worktree
    .command("env")
    .description("Print shell exports for the current worktree-local HermesFabric instance")
  ...
  program
    ...
    .description("List git worktrees visible from this repo and whether they look like HermesFabric worktrees")
  ...
  worktree
    .command("reseed")
    .description("Re-seed an existing worktree-local instance from another HermesFabric instance or worktree")
    .option("--from <worktree>", "Source worktree path, directory name, branch name, or current")
    .option("--to <worktree>", "Target worktree path, directory name, branch name, or current (defaults to current)")
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source HERMES_FABRIC_HOME used when deriving the source config")
  ...
  worktree
    .command("repair")
    .description("Create or repair a linked worktree-local HermesFabric instance without touching the primary checkout")
    .option("--branch <name>", "Existing branch/worktree selector to repair, or a branch name to create under .fabric/worktrees")
    .option("--home <path>", `Home root for worktree instances (env: HERMES_FABRIC_WORKTREES_DIR, default: ${DEFAULT_WORKTREE_HOME})`)
    .option("--from-config <path>", "Source config.json to seed from")
    .option("--from-data-dir <path>", "Source HERMES_FABRIC_HOME used when deriving the source config")
  ...
  program
    ...
    .argument("<name>", "Worktree name — auto-prefixed with fabric- if needed")
