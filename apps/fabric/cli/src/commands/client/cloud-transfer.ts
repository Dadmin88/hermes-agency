...
export class LocalUpstreamPushCoordinator {
  ...
  readonly #fabricCompanyId: string;
  ...
  constructor(options: LocalUpstreamPushCoordinatorOptions) {
    ...
    this.#fabricCompanyId = options.fabricCompanyId;
    this.#fetch = options.fetch ?? fetch;
    this.#headers = options.headers ?? (() => ({}));
  }

  async preview(bundle: LocalUpstreamExportBundle): Promise<unknown> {
    return this.post(`/api/companies/${encodeURIComponent(this.#fabricCompanyId)}/upstream-imports/preview`, {
  ...
  async apply(bundle: LocalUpstreamExportBundle): Promise<unknown> {
    const run = await this.post(`/api/companies/${encodeURIComponent(this.#fabricCompanyId)}/upstream-imports/runs`, {
      mode: "apply",
      manifest: bundle.manifest,
      entities: bundle.entities,
    }) as { run?: { id?: unknown } };
