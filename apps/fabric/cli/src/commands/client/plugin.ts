...
} from "../../../../packages/plugins/create-fabric-plugin/src/index.js";
...
/** Result of probing the HermesFabric instance the CLI is about to talk to. */
...
export function renderLocalPluginInstallHint(packagePath: string): string {
  return [
    ...
    pc.dim(`Keep ${pc.cyan("pnpm dev")} running in ${packagePath}; HermesFabric watches rebuilt dist output and reloads the plugin worker.`),
  ].join("\n");
}

/**
 * Probe `GET /api/health` on the instance the CLI is configured to talk to so a
 * developer can confirm *which* HermesFabric they are about to install into. This
...
export function formatTargetDiagnostics(diag: TargetDiagnostics): string {
  const lines = [pc.dim(`Target HermesFabric: ${pc.cyan(diag.apiBase)}`)];

  if (!diag.reachable) {
    lines.push(pc.yellow(`  health: unreachable${diag.error ? ` (${diag.error.split("\n")[0]})` : ""}`));
    lines.push(
      pc.dim(
        `  Verify the right instance is running, then pass ${pc.cyan("--api-base <url>")} or set ${pc.cyan("HERMES_FABRIC_API_URL")} if it lives elsewhere.`,
      ),
    );
    return lines.join("\n");
  }
...
export function buildPluginInitNextCommands(outputDir: string): string[] {
  ...
  return [
    ...
    "pnpm install",
    "pnpm dev",
    `hermes-fabric plugin install ${quotedOutputDir}`,
  ];
}
...
export function registerPluginCommands(program: Command): void {
  ...
  addCommonClientOptions(
    plugin
      ...
      .description("Scaffold a local HermesFabric plugin project")
      ...
      .option("--sdk-path <path>", "Local @hermes-fabric/plugin-sdk package path")
    ...
    plugin
      ...
      .description(
        "Install a plugin from a local path or npm package.\n" +
          "  Examples:\n" +
          "    hermes-fabric plugin install ./my-plugin              # local path\n" +
          "    hermes-fabric plugin install @acme/plugin-linear      # npm package\n" +
          "    hermes-fabric plugin install @acme/plugin-linear@1.2  # pinned version",
      )
      ...
      .option(
        "--no-verify-target",
        "Skip the pre-install probe that reports which HermesFabric instance the plugin installs into",
      )
  ...
  addCommonClientOptions(
    plugin
      .command("target")
      .description(
        "Show which HermesFabric instance plugin commands will talk to.\n" +
          ...
          for (const ex of rows) {
            console.log(
              `${pc.bold(ex.displayName)}  ${pc.dim(ex.pluginKey)}\n` +
                `  ${ex.description}\n` +
                `  ${pc.cyan(`hermes-fabric plugin install ${ex.localPath}`)}`,
            );
          }
        } catch (err) {
          handleCommandError(err);
