...
import type { Agent, Company } from "@hermes-fabric/shared";
import { createAgentKeySchema, createBoardApiKeySchema } from "@hermes-fabric/shared";
import { loginBoardCli } from "../../client/board-auth.js";
import { HermesFabricApiClient } from "../../client/http.js";
...
export function registerConnectCommand(program: Command): void {
  addCommonClientOptions(
    program
      ...
      .option("--api-key-env-var-name <name>", "Env var name to store in the profile", "HERMES_FABRIC_API_KEY")
...
async function connectWizard(opts: ConnectOptions) {
  if (!process.stdin.isTTY || !process.stdout.isTTY) {
    throw new Error("`hermes-fabric connect` is interactive. For scripts, pass --api-base/--api-key or use context set/token commands.");
  }

  p.intro(pc.bgCyan(pc.black(" hermes-fabric connect ")));
  ...
  const boardLogin = await loginBoardCli({
    ...
    command: "hermes-fabric connect",
  });
  const boardApi = new HermesFabricApiClient({ apiBase, apiKey: boardLogin.token });
  ...
  const apiKeyEnvVarName = opts.apiKeyEnvVarName?.trim() || "HERMES_FABRIC_API_KEY";
...
async function verifyHealth(apiBase: string): Promise<void> {
  const api = new HermesFabricApiClient({ apiBase });
...
function buildExports(input: {
  ...
  return [
    `export HERMES_FABRIC_API_URL='${escaped(input.apiBase)}'`,
    input.companyId ? `export HERMES_FABRIC_COMPANY_ID='${escaped(input.companyId)}'` : null,
    input.agentId ? `export HERMES_FABRIC_AGENT_ID='${escaped(input.agentId)}'` : null,
