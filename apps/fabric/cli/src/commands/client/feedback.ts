...
import type { Company, FeedbackTrace, FeedbackTraceBundle } from "@hermes-fabric/shared";
import {
...
export async function resolveFeedbackCompanyId(
  ...
  if (!companyId) {
    throw new Error(
      "Company ID is required. Pass --company-id, set HERMES_FABRIC_COMPANY_ID, or configure a CLI context default.",
    );
  }
  return companyId;
}
...
export function renderFeedbackReport(input: {
  ...
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("HermesFabric Feedback Report")));
...
export function renderFeedbackExportSummary(exported: FeedbackExportResult): string {
  const lines: string[] = [];
  lines.push("");
  lines.push(pc.bold(pc.magenta("HermesFabric Feedback Export")));
