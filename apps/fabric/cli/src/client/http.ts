...
export class HermesFabricApiClient {
  ...
  private async request<T>(
    ...
    if (this.runId) {
      headers["x-fabric-run-id"] = this.runId;
    }
...
function buildConnectionErrorMessage(input: {
  ...
  const lines = [
    "Could not reach the HermesFabric API.",
    "",
  ...
  lines.push(
    "",
    "This usually means the HermesFabric server is not running, the configured URL is wrong, or the request is being blocked before it reaches HermesFabric.",
    "",
    "Try:",
    "- Start HermesFabric with `pnpm dev` or `pnpm hermes-fabric run`.",
    `- Verify the server is reachable with \`curl ${healthUrl}\`.`,
    `- If HermesFabric is running elsewhere, pass \`--api-base ${input.apiBase.replace(/\/+$/, "")}\` or set \`HERMES_FABRIC_API_URL\`.`,
  );
  return lines.join("\n");
}
