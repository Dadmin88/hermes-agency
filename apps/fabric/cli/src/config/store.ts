...
import { fabricConfigSchema, type HermesFabricConfig } from "./schema.js";
import {
...
function findConfigFileFromAncestors(startDir: string): string | null {
  ...
  while (true) {
    const candidate = path.resolve(currentDir, ".fabric", DEFAULT_CONFIG_BASENAME);
...
export function resolveConfigPath(overridePath?: string): string {
  ...
  if (process.env.HERMES_FABRIC_CONFIG) return path.resolve(process.env.HERMES_FABRIC_CONFIG);
  return findConfigFileFromAncestors(process.cwd()) ?? resolveDefaultConfigPath(resolveHermesFabricInstanceId());
}
...
export function readConfig(configPath?: string): HermesFabricConfig | null {
  const filePath = resolveConfigPath(configPath);
  if (!fs.existsSync(filePath)) return null;
  const raw = parseJson(filePath);
  const migrated = migrateLegacyConfig(raw);
  const parsed = fabricConfigSchema.safeParse(migrated);
  if (!parsed.success) {
