...
const JWT_SECRET_ENV_KEY = "HERMES_FABRIC_AGENT_JWT_SECRET";
...
function renderEnvFile(entries: Record<string, string>) {
  const lines = [
    "# HermesFabric environment variables",
    "# Generated by HermesFabric CLI commands",
...
export function resolveHermesFabricEnvFile(configPath?: string): string {
...
export function loadHermesFabricEnvFile(configPath?: string): void {
...
export function writeAgentJwtEnv(secret: string, filePath = resolveEnvFilePath()): void {
  mergeHermesFabricEnvEntries({ [JWT_SECRET_ENV_KEY]: secret }, filePath);
}

export function readHermesFabricEnvEntries(filePath = resolveEnvFilePath()): Record<string, string> {
  if (!fs.existsSync(filePath)) return {};
  return parseEnvFile(fs.readFileSync(filePath, "utf-8"));
}

export function writeHermesFabricEnvEntries(entries: Record<string, string>, filePath = resolveEnvFilePath()): void {
...
export function mergeHermesFabricEnvEntries(
  entries: Record<string, string>,
  filePath = resolveEnvFilePath(),
): Record<string, string> {
  const current = readHermesFabricEnvEntries(filePath);
  const next = {
    ...current,
    ...Object.fromEntries(
  ...
  writeHermesFabricEnvEntries(next, filePath);
  return next;
}
