...
function createBaseHermesFabricEnv(options: TestHermesFabricEnv) {
  const env = { ...process.env };
  for (const key of Object.keys(env)) {
    if (key.startsWith("HERMES_FABRIC_")) {
      delete env[key];
    }
  }

  env.HERMES_FABRIC_CONFIG = options.configPath;
  env.HERMES_FABRIC_HOME = options.fabricHome;
  env.HERMES_FABRIC_INSTANCE_ID = options.instanceId;
  env.HERMES_FABRIC_CONTEXT = path.join(options.fabricHome, "context.json");
  env.HERMES_FABRIC_AUTH_STORE = path.join(options.fabricHome, "auth.json");
...
function createServerEnv(
  ...
  options: Omit<TestHermesFabricEnv, "configPath">,
) {
  const env = createBaseHermesFabricEnv({
    configPath,
    ...options,
  });
  ...
  env.SERVE_UI = "false";
  env.HERMES_FABRIC_DB_BACKUP_ENABLED = "false";
  env.HEARTBEAT_SCHEDULER_ENABLED = "false";
  env.HERMES_FABRIC_MIGRATION_AUTO_APPLY = "true";
  env.HERMES_FABRIC_UI_DEV_MIDDLEWARE = "false";

  return env;
}

function createCliEnv(options: TestHermesFabricEnv) {
  const env = createBaseHermesFabricEnv(options);
  delete env.DATABASE_URL;
  delete env.PORT;
  delete env.HOST;
  delete env.SERVE_UI;
  delete env.HERMES_FABRIC_DB_BACKUP_ENABLED;
  delete env.HEARTBEAT_SCHEDULER_ENABLED;
  delete env.HERMES_FABRIC_MIGRATION_AUTO_APPLY;
  delete env.HERMES_FABRIC_UI_DEV_MIDDLEWARE;
  return env;
}
...
async function runCliJson<T>(
  args: string[],
  opts: TestHermesFabricEnv & { apiBase?: string; includeConfigArg?: boolean },
) {
  const repoRoot = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../../..");
  const cliArgs = ["--silent", "hermes-fabric", ...args];
  if (opts.apiBase) {
...
async function waitForServer(
  ...
  while (Date.now() - startedAt < 30_000) {
    if (child.exitCode !== null) {
      throw new Error(
        `hermes-fabric run exited before healthcheck succeeded.\nstdout:\n${output.stdout.join("")}\nstderr:\n${output.stderr.join("")}`,
      );
    }
...
describeEmbeddedPostgres("hermes-fabric company import/export e2e", () => {
  let tempRoot = "";
  let configPath = "";
  let exportDir = "";
  let apiBase = "";
  let fabricHome = "";
  let cliShellHome = "";
  let fabricInstanceId = "";
  let serverProcess: ServerProcess | null = null;
  let tempDb: Awaited<ReturnType<typeof startEmbeddedPostgresTestDatabase>> | null = null;

  beforeAll(async () => {
    tempRoot = mkdtempSync(path.join(os.tmpdir(), "fabric-company-cli-e2e-"));
    configPath = path.join(tempRoot, "config", "config.json");
    exportDir = path.join(tempRoot, "exported-company");
    fabricHome = path.join(tempRoot, "fabric-home");
    cliShellHome = path.join(tempRoot, "shell-home");
    fabricInstanceId = "company-cli-e2e";
    mkdirSync(fabricHome, { recursive: true });
    mkdirSync(cliShellHome, { recursive: true });

    tempDb = await startEmbeddedPostgresTestDatabase("fabric-company-cli-db-");
    ...
    const child = spawn(
      "pnpm",
      ["hermes-fabric", "run", "--config", configPath],
      {
        cwd: repoRoot,
        env: createServerEnv(configPath, port, tempDb.connectionString, {
          fabricHome,
          instanceId: fabricInstanceId,
  ...
  it("exports a company package and imports it into new and existing companies", async () => {
    ...
    const expectedContextPath = path.join(fabricHome, "context.json");
    const leakedContextPath = path.join(cliShellHome, ".fabric", "context.json");
    ...
    expect(readFileSync(path.join(exportDir, ".fabric.yaml"), "utf8")).toContain('schema: "fabric/v1"');
    ...
    writeFileSync(zipPath, createStoredZipArchive(portableFiles, "fabric-demo"));
