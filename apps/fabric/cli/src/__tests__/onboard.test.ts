...
function createExistingConfigFixture() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-onboard-"));
  const runtimeRoot = path.join(root, "runtime");
  const configPath = path.join(root, ".fabric", "config.json");
...
function createFreshConfigPath() {
  const root = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-onboard-fresh-"));
  return path.join(root, ".fabric", "config.json");
}
...
describe("onboard", () => {
  beforeEach(() => {
    ...
    delete process.env.HERMES_FABRIC_BIND;
    delete process.env.HERMES_FABRIC_BIND_HOST;
    delete process.env.HERMES_FABRIC_TAILNET_BIND_HOST;
  ...
  it("keeps --yes onboarding on local trusted loopback defaults", async () => {
    ...
    process.env.HERMES_FABRIC_BIND = "lan";

    await onboard({ config: configPath, yes: true, invokedByRun: true });

    const raw = JSON.parse(fs.readFileSync(configPath, "utf8")) as HermesFabricConfig;
  ...
  it("creates instance-root config and data paths for a fresh HERMES_FABRIC_HOME", async () => {
    const home = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-onboard-home-"));
    const cwd = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-onboard-cwd-"));
    process.chdir(cwd);
  ...
  it("supports authenticated/private quickstart bind presets", async () => {
    const configPath = createFreshConfigPath();
    process.env.HERMES_FABRIC_TAILNET_BIND_HOST = "100.64.0.8";
