...
import { ApiConnectionError, ApiRequestError, HermesFabricApiClient } from "../client/http.js";

describe("HermesFabricApiClient", () => {
  afterEach(() => {
    vi.restoreAllMocks();
  });
  ...
  it("adds authorization and run-id headers", async () => {
    ...
    const client = new HermesFabricApiClient({
    ...
    expect(headers["x-fabric-run-id"]).toBe("run-abc");
  ...
  it("returns null on ignoreNotFound", async () => {
    ...
    const client = new HermesFabricApiClient({ apiBase: "http://localhost:3100" });
  ...
  it("throws ApiConnectionError with recovery guidance when fetch fails", async () => {
    ...
    await expect(client.post("/api/companies/import/preview", {})).rejects.toThrow(
      /pnpm dev|pnpm hermes-fabric run/,
    );
  });
