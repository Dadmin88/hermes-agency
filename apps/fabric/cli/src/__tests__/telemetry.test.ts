...
describe("cli telemetry", () => {
  ...
  it("respects telemetry.enabled=false from the config file", async () => {
    const root = fs.mkdtempSync(path.join(os.tmpdir(), "fabric-cli-telemetry-"));
    const configPath = makeConfigPath(root, false);
    process.env.HERMES_FABRIC_HOME = path.join(root, "home");
    process.env.HERMES_FABRIC_INSTANCE_ID = "telemetry-test";
