...
if (worktreeEnvBootstrap.missingEnv) {
  console.error(
    `[fabric] linked git worktree at ${repoRoot} is missing ${path.relative(repoRoot, worktreeEnvBootstrap.envPath)}. Run \`hermes-fabric worktree init\` in this worktree before \`pnpm dev\`.`,
  );
  process.exit(1);
}
...
const ignoredRelativePaths = new Set([
  ".fabric/dev-server-restart-request.json",
  ".fabric/dev-server-status.json",
]);
...
for (let index = 0; index < cliArgs.length; index += 1) {
  ...
  if (arg === "--bind") {
    ...
    if (!value || value.startsWith("--") || !BIND_MODES.includes(value as BindMode)) {
      console.error(`[fabric] invalid --bind value. Use one of: ${BIND_MODES.join(", ")}`);
      process.exit(1);
    }
  ...
  if (arg === "--bind-host") {
    ...
    if (!value || value.startsWith("--")) {
      console.error("[fabric] --bind-host requires a value");
      process.exit(1);
    }
    bindHost = value;
    index += 1;
...
if (bindMode === "custom" && !bindHost) {
  console.error("[fabric] --bind custom requires --bind-host <host>");
  process.exit(1);
}

const env: NodeJS.ProcessEnv = {
  ...process.env,
  HERMES_FABRIC_UI_DEV_MIDDLEWARE: "true",
};

if (mode === "dev") {
  env.HERMES_FABRIC_DEV_SERVER_STATUS_FILE = devServerStatusFilePath;
  env.HERMES_FABRIC_DEV_SERVER_STATUS_TOKEN = devServerStatusToken ?? "";
  env.HERMES_FABRIC_MIGRATION_AUTO_APPLY ??= "true";
}

if (mode === "watch") {
  delete env.HERMES_FABRIC_DEV_SERVER_STATUS_TOKEN;
  env.HERMES_FABRIC_MIGRATION_PROMPT ??= "never";
  env.HERMES_FABRIC_MIGRATION_AUTO_APPLY ??= "true";
}
...
if (tailscaleAuth || bindMode) {
  ...
  if (tailscaleAuth) {
    console.log("[fabric] note: --tailscale-auth/--authenticated-private are legacy aliases for --bind lan");
  }
  env.HERMES_FABRIC_BIND = effectiveBind;
  if (bindHost) {
    env.HERMES_FABRIC_BIND_HOST = bindHost;
  } else {
    delete env.HERMES_FABRIC_BIND_HOST;
  }
  if (effectiveBind === "loopback" && !tailscaleAuth) {
    delete env.HERMES_FABRIC_DEPLOYMENT_MODE;
    delete env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE;
    delete env.HERMES_FABRIC_AUTH_BASE_URL_MODE;
    console.log("[fabric] dev mode: local_trusted (bind=loopback)");
  } else {
    env.HERMES_FABRIC_DEPLOYMENT_MODE = "authenticated";
    env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE = "private";
    env.HERMES_FABRIC_AUTH_BASE_URL_MODE = "auto";
    console.log(
      `[fabric] dev mode: authenticated/private (bind=${effectiveBind}${bindHost ? `:${bindHost}` : ""})`,
    );
  }
} else {
  delete env.HERMES_FABRIC_BIND;
  delete env.HERMES_FABRIC_BIND_HOST;
  delete env.HERMES_FABRIC_DEPLOYMENT_MODE;
  delete env.HERMES_FABRIC_DEPLOYMENT_EXPOSURE;
  delete env.HERMES_FABRIC_AUTH_BASE_URL_MODE;
  console.log("[fabric] dev mode: local_trusted (default)");
}
...
if (existingRunner) {
  console.log(
    `[fabric] ${devService.serviceName} already running (pid ${existingRunner.pid}${typeof existingRunner.metadata?.childPid === "number" ? `, child ${existingRunner.metadata.childPid}` : ""})`,
  );
  process.exit(0);
}
...
async function getMigrationStatusPayload() {
  const status = await runPnpm(
    ["--filter", "@hermes-fabric/db", "exec", "tsx", "src/migration-status.ts", "--json"],
    { env },
  );
  if (status.code !== 0) {
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        `[fabric] Command failed with code ${status.code}: pnpm --filter @hermes-fabric/db exec tsx src/migration-status.ts --json\n`,
    );
  ...
  try {
    ...
    process.stderr.write(
      status.stderr ||
        status.stdout ||
        "[fabric] migration-status returned invalid JSON payload\n",
    );
...
async function maybePreflightMigrations(options: { interactive?: boolean; autoApply?: boolean; exitOnDecline?: boolean } = {}) {
  ...
  const autoApply = options.autoApply ?? env.HERMES_FABRIC_MIGRATION_AUTO_APPLY === "true";
  ...
  if (!shouldApply) {
    if (exitOnDecline) {
      process.stderr.write(
        `[fabric] Pending migrations detected (${formatPendingMigrationSummary(pendingMigrations)}). Refusing to start watch mode against a stale schema.\n`,
      );
      process.exit(1);
    }
    return;
...
async function buildPluginSdk() {
  console.log("[fabric] building plugin sdk...");
  const result = await runPnpm(
    ["--filter", "@hermes-fabric/plugin-sdk", "build"],
    { stdio: "inherit" },
  );
  if (result.signal) {
  ...
  if (result.code !== 0) {
    console.error("[fabric] plugin sdk build failed");
...
async function startServerChild() {
  ...
  child = spawn(
    pnpmBin,
    ["--filter", "@hermes-fabric/server", serverScript, ...forwardedArgs],
