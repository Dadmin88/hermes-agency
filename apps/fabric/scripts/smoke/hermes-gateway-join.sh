...
HERMES_FABRIC_API_URL="${HERMES_FABRIC_API_URL:-http://localhost:3100}"
...
Creates a Hermes gateway agent from an agent-only HermesFabric invite, approves the
join request, claims the one-time HermesFabric API key, and verifies the stored
...
Notes:
  HERMES_GATEWAY_API_BASE_URL is stored on the HermesFabric adapter and must be
  reachable by the HermesFabric server. HERMES_GATEWAY_PROBE_URL is only used by
  this operator shell to preflight /health, which is useful when HermesFabric talks
  ...
  claimed HermesFabric agent API key and is written chmod 600.
...
redact_text() {
  local text="$1"
  local secret
  for secret in "${HERMES_GATEWAY_API_KEY:-}" "${CLAIM_SECRET:-}" "${AGENT_API_KEY:-}" "${HERMES_FABRIC_AUTH_HEADER:-}" "${HERMES_FABRIC_COOKIE:-}" "${HERMES_FABRIC_API_KEY:-}"; do
    if [[ -n "$secret" ]]; then
...
Provide one of:
  HERMES_FABRIC_AUTH_HEADER="Bearer <board-token>"
  HERMES_FABRIC_COOKIE="<board-session-cookie>"
...
probe_hermes_gateway() {
  ...
  [[ -n "$HERMES_GATEWAY_API_KEY" ]] || fail "HERMES_GATEWAY_API_KEY or API_SERVER_KEY is required before any HermesFabric state is mutated"
  ...
  if [[ "$code" != "200" ]]; then
    fail "Hermes gateway health probe failed before mutating HermesFabric state: ${health_url} returned HTTP ${code}. Start Hermes with API_SERVER_ENABLED=true API_SERVER_KEY=<key> hermes gateway run --replace --accept-hooks, or set HERMES_GATEWAY_API_BASE_URL/HERMES_GATEWAY_API_KEY."
  fi
}

log "checking HermesFabric health"
...
JOIN_PAYLOAD="$(jq -nc \
  ...
  --arg fabricApiUrl "$HERMES_FABRIC_API_URL_FOR_HERMES" \
  ...
  | if $fabricApiUrl != "" then .agentDefaultsPayload.fabricApiUrl = $fabricApiUrl else . end
...
if [[ -n "$HERMES_FABRIC_API_URL_FOR_HERMES" ]]; then
  STORED_HERMES_FABRIC_API_URL="$(jq -r '.adapterConfig.fabricApiUrl // empty' <<<"$RESPONSE_BODY")"
  [[ "$STORED_HERMES_FABRIC_API_URL" == "$HERMES_FABRIC_API_URL_FOR_HERMES" || "$(strip_trailing_slash "$STORED_HERMES_FABRIC_API_URL")" == "$(strip_trailing_slash "$HERMES_FABRIC_API_URL_FOR_HERMES")" ]] \
    || fail "stored fabricApiUrl mismatch"
fi
