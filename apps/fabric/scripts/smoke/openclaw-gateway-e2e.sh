...
OPENCLAW_CONFIG_DIR="${OPENCLAW_CONFIG_DIR:-${OPENCLAW_TMP_DIR}/openclaw-fabric-smoke}"
...
HERMES_FABRIC_API_URL_FOR_OPENCLAW="${HERMES_FABRIC_API_URL_FOR_OPENCLAW:-http://host.docker.internal:3100}"
...
require_board_auth() {
  if [[ ${#AUTH_HEADERS[@]} -eq 0 ]]; then
    fail "board auth required. Set HERMES_FABRIC_COOKIE or HERMES_FABRIC_AUTH_HEADER."
  fi
...
create_and_approve_gateway_join() {
  ...
  join_payload="$(jq -nc \
    ...
    --arg fabricApiUrl "$HERMES_FABRIC_API_URL_FOR_OPENCLAW" \
...
persist_claimed_key_artifacts() {
  ...
  local skill_dir="${OPENCLAW_CONFIG_DIR%/}/skills/fabric"
  local claimed_file="${workspace_dir}/fabric-claimed-api-key.json"
  local claimed_raw_file="${workspace_dir}/fabric-claimed-api-key.raw.json"
  ...
  if [[ -n "$container" ]]; then
    ...
    docker cp "$claimed_file" "${container}:/home/node/.openclaw/workspace/fabric-claimed-api-key.json" >/dev/null 2>&1 || true
    docker exec "$container" sh -lc "chmod 644 /home/node/.openclaw/workspace/fabric-claimed-api-key.json" >/dev/null 2>&1 || true
  fi

  if [[ "$AUTO_INSTALL_SKILL" == "1" ]]; then
    api_request "GET" "/skills/fabric"
    assert_status "200"
    {
      echo "HERMES_FABRIC_API_URL=${HERMES_FABRIC_API_URL_FOR_OPENCLAW}"
      echo
      printf "%s\n" "$RESPONSE_BODY"
    } > "${skill_dir}/SKILL.md"
...
inject_agent_api_key_payload_template() {
  ...
  local base_message
  base_message="Set HERMES_FABRIC_API_KEY=${AGENT_API_KEY} in your run context before running HermesFabric heartbeat steps."
...
main() {
  ...
  wait_http_ready "${HERMES_FABRIC_API_URL%/}/api/health" 15 || fail "HermesFabric API health endpoint not reachable"
  api_request "GET" "/health"
  assert_status "200"
  log "fabric health deploymentMode=$(jq -r '.deploymentMode // "unknown"' <<<"$RESPONSE_BODY") exposure=$(jq -r '.deploymentExposure // "unknown"' <<<"$RESPONSE_BODY")"
