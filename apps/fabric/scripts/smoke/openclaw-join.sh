...
SMOKE_IMAGE="${SMOKE_IMAGE:-fabric-openclaw-smoke:local}"
SMOKE_CONTAINER_NAME="${SMOKE_CONTAINER_NAME:-fabric-openclaw-smoke}"
...
assert_status "200"
if ! grep -q "HermesFabric OpenClaw Gateway Onboarding" <<<"$RESPONSE_BODY"; then
...
for _ in $(seq 1 "$SMOKE_TIMEOUT_SEC"); do
  ...
  MATCH_COUNT="$(jq -r --arg agentId "$CREATED_AGENT_ID" '[.events[] | select(((.body.fabric.agentId // "") == $agentId))] | length' <<<"$LAST_EVENTS")"
