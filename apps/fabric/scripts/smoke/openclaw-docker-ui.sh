...
HERMES_FABRIC_HOST_PORT="${HERMES_FABRIC_HOST_PORT:-3100}"
HERMES_FABRIC_HOST_FROM_CONTAINER="${HERMES_FABRIC_HOST_FROM_CONTAINER:-host.docker.internal}"
...
COMPOSE_OVERRIDE="${OPENCLAW_DOCKER_DIR}/.fabric-openclaw.override.yml"
...
detect_fabric_base_url() {
  ...
  for candidate in "$HERMES_FABRIC_HOST_FROM_CONTAINER" "$bridge_gateway"; do
    [[ -n "$candidate" ]] || continue
    health_url="http://${candidate}:${HERMES_FABRIC_HOST_PORT}/api/health"
    if compose exec -T openclaw-gateway node -e "fetch('${health_url}').then((r)=>process.exit(r.ok?0:1)).catch(()=>process.exit(1))" >/dev/null 2>&1; then
      echo "http://${candidate}:${HERMES_FABRIC_HOST_PORT}"
      return 0
    fi
  done
  return 1
...
fabric_base_url="$(detect_fabric_base_url || true)"
...
HermesFabric URL for OpenClaw container:
EOF
  if [[ -n "$fabric_base_url" ]]; then
    cat <<EOF
  $fabric_base_url
...
EOF
    ...
    cat <<EOF
  Auto-detect failed. Try: http://host.docker.internal:${HERMES_FABRIC_HOST_PORT}
  (Do not use http://127.0.0.1:${HERMES_FABRIC_HOST_PORT} inside the container.)
  If HermesFabric rejects the host, run on host machine:
    pnpm hermes-fabric allowed-hostname host.docker.internal
  Then restart HermesFabric and re-run this script.
EOF
  fi
  cat <<EOF
