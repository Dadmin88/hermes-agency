#!/usr/bin/env node
/**
 * Screenshot utility for HermesFabric UI.
 *
 * Reads the board token from ~/.hermes-fabric/auth.json and injects it as a
...
const rawUrl = args[0];
const outPath = args[1] || "/tmp/fabric-screenshot.png";
...
function loadBoardToken() {
  const authPath = path.resolve(os.homedir(), ".fabric/auth.json");
  try {
...
if (!cred) {
  console.error("No board token found in ~/.hermes-fabric/auth.json");
  process.exit(1);
}
...
(async () => {
  ...
  try {
    ...
    // Scope the auth header to the HermesFabric origin only
