...
const DEFAULT_QUERY = "\"Co-Authored-By: HermesFabric <noreply@fabric.ing>\"";
const DEFAULT_CACHE_FILE = path.resolve("data/fabric-commit-metrics-cache.json");
...
const HERMES_FABRIC_EMAIL = "noreply@fabric.ing";
const HERMES_FABRIC_NAME = "fabric";
...
function printHelp() {
  console.log(`Usage: tsx scripts/fabric-commit-metrics.ts [options]
...
function normalizeContributor(input: {
  ...
  if ((email && email === HERMES_FABRIC_EMAIL) || (displayName && displayName.toLowerCase() === HERMES_FABRIC_NAME)) {
    return null;
  }
...
function printSummary(summary: Summary) {
  console.log("HermesFabric commit metrics");
    ...
    while (true) {
      const response = await fetch(`${this.apiBase}${pathname}`, {
        headers: {
          ...
          "User-Agent": "fabric-commit-metrics",
