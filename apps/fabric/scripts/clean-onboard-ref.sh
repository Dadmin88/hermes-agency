...
Environment overrides:
  ...
  PC_DATA=/tmp/data           HermesFabric data dir to use
  HERMES_FABRIC_HOST=127.0.0.1    Host passed to the onboarded server
  HERMES_FABRIC_PORT=3232         Port passed to the onboarded server
...
export PC_TEST_ROOT="${PC_TEST_ROOT:-$(mktemp -d /tmp/fabric-clean-ref.XXXXXX)}"
export PC_HOME="${PC_HOME:-$PC_TEST_ROOT/home}"
export PC_CACHE="${PC_CACHE:-$PC_TEST_ROOT/npm-cache}"
export PC_DATA="${PC_DATA:-$PC_TEST_ROOT/fabric-data}"
export PC_REPO="${PC_REPO:-$PC_TEST_ROOT/repo}"
export HERMES_FABRIC_HOST="${HERMES_FABRIC_HOST:-127.0.0.1}"
export HERMES_FABRIC_PORT="${HERMES_FABRIC_PORT:-3100}"
export HERMES_FABRIC_OPEN_ON_LISTEN="${HERMES_FABRIC_OPEN_ON_LISTEN:-false}"
...
echo "HERMES_FABRIC_HOST: $HERMES_FABRIC_HOST"
echo "HERMES_FABRIC_PORT: $HERMES_FABRIC_PORT"
...
env \
  ...
  HOST="$HERMES_FABRIC_HOST" \
  PORT="$HERMES_FABRIC_PORT" \
  HERMES_FABRIC_OPEN_ON_LISTEN="$HERMES_FABRIC_OPEN_ON_LISTEN" \
  pnpm hermes-fabric onboard --yes --data-dir "$PC_DATA"
