...
# This keeps @hermes-fabric/server publish artifacts self-contained for static UI serving.
# When HERMES_FABRIC_RELEASE_REUSE_UI_DIST=1 and ui/dist already exists, reuse that
...
case "${HERMES_FABRIC_RELEASE_REUSE_UI_DIST:-}" in
  1|true|TRUE|yes|YES)
...
if [ "$should_reuse_existing_ui_dist" = true ] && [ -f "$UI_DIST/index.html" ]; then
  echo "  -> Reusing existing @hermes-fabric/ui dist output"
else
  echo "  -> Building @hermes-fabric/ui..."
  pnpm --dir "$REPO_ROOT" --filter @hermes-fabric/ui build
fi
