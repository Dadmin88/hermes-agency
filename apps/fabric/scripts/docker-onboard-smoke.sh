...
IMAGE_NAME="${IMAGE_NAME:-fabric-onboard-smoke}"
HOST_PORT="${HOST_PORT:-3131}"
HERMES_FABRIC_VERSION="${HERMES_FABRIC_VERSION:-latest}"
...
HERMES_FABRIC_DEPLOYMENT_MODE="${HERMES_FABRIC_DEPLOYMENT_MODE:-authenticated}"
HERMES_FABRIC_DEPLOYMENT_EXPOSURE="${HERMES_FABRIC_DEPLOYMENT_EXPOSURE:-private}"
HERMES_FABRIC_PUBLIC_URL="${HERMES_FABRIC_PUBLIC_URL:-http://localhost:${HOST_PORT}}"
SMOKE_AUTO_BOOTSTRAP="${SMOKE_AUTO_BOOTSTRAP:-true}"
SMOKE_ADMIN_NAME="${SMOKE_ADMIN_NAME:-Smoke Admin}"
SMOKE_ADMIN_EMAIL="${SMOKE_ADMIN_EMAIL:-smoke-admin@fabric.local}"
SMOKE_ADMIN_PASSWORD="${SMOKE_ADMIN_PASSWORD:-fabric-smoke-password}"
...
write_metadata_file() {
  ...
  mkdir -p "$(dirname "$SMOKE_METADATA_FILE")"
  {
    ...
    printf 'SMOKE_HERMES_FABRIC_VERSION=%q\n' "$HERMES_FABRIC_VERSION"
  } >"$SMOKE_METADATA_FILE"
}
...
generate_bootstrap_invite_url() {
  ...
  if bootstrap_output="$(
    docker exec \
      -e HERMES_FABRIC_DEPLOYMENT_MODE="$HERMES_FABRIC_DEPLOYMENT_MODE" \
      -e HERMES_FABRIC_DEPLOYMENT_EXPOSURE="$HERMES_FABRIC_DEPLOYMENT_EXPOSURE" \
      -e HERMES_FABRIC_PUBLIC_URL="$HERMES_FABRIC_PUBLIC_URL" \
      -e HERMES_FABRIC_HOME="/fabric" \
      "$CONTAINER_NAME" bash -lc \
      'timeout 20s npx --yes "hermes-fabric@${HERMES_FABRIC_VERSION}" auth bootstrap-ceo --data-dir "$HERMES_FABRIC_HOME" --base-url "$HERMES_FABRIC_PUBLIC_URL"' \
      2>&1
  )"; then
    bootstrap_status=0
  else
...
post_json_with_cookies() {
  ...
  curl -sS \
    ...
    -H "Origin: $HERMES_FABRIC_PUBLIC_URL" \
    -X POST \
    "$url" \
    --data "$body"
}
...
sign_up_or_sign_in() {
  ...
  signup_status="$(post_json_with_cookies \
    "$HERMES_FABRIC_PUBLIC_URL/api/auth/sign-up/email" \
  ...
  signin_status="$(post_json_with_cookies \
    "$HERMES_FABRIC_PUBLIC_URL/api/auth/sign-in/email" \
...
auto_bootstrap_authenticated_smoke() {
  local health_url="$HERMES_FABRIC_PUBLIC_URL/api/health"
  local health_json
  ...
  if [[ "$health_json" == *'"bootstrapStatus":"ready"'* ]]; then
    ...
    accept_status="$(post_json_with_cookies \
      "$HERMES_FABRIC_PUBLIC_URL/api/invites/$invite_token/accept" \
      '{"requestType":"human"}' \
      "$accept_response")"
  ...
  local session_json
  session_json="$(get_with_cookies "$HERMES_FABRIC_PUBLIC_URL/api/auth/get-session")"
  ...
  companies_json="$(get_with_cookies "$HERMES_FABRIC_PUBLIC_URL/api/companies")"
...
docker build \
  --build-arg HERMES_FABRIC_VERSION="$HERMES_FABRIC_VERSION" \
...
echo "    Public URL: $HERMES_FABRIC_PUBLIC_URL"
echo "    Smoke auto-bootstrap: $SMOKE_AUTO_BOOTSTRAP"
echo "    Detached mode: $SMOKE_DETACH"
echo "    Data dir: $DATA_DIR"
echo "    Deployment: $HERMES_FABRIC_DEPLOYMENT_MODE/$HERMES_FABRIC_DEPLOYMENT_EXPOSURE"
...
TMP_DIR="$(mktemp -d "${TMPDIR:-/tmp}/fabric-onboard-smoke.XXXXXX")"
COOKIE_JAR="$TMP_DIR/cookies.txt"

if ! wait_for_http "$HERMES_FABRIC_PUBLIC_URL/api/health" 90 1; then
  echo "Smoke bootstrap failed: server did not become ready at $HERMES_FABRIC_PUBLIC_URL/api/health" >&2
  exit 1
fi

if [[ "$SMOKE_AUTO_BOOTSTRAP" == "true" && "$HERMES_FABRIC_DEPLOYMENT_MODE" == "authenticated" ]]; then
  auto_bootstrap_authenticated_smoke
fi
...
if [[ "$SMOKE_DETACH" == "true" ]]; then
  ...
  echo "    Smoke base URL: $HERMES_FABRIC_PUBLIC_URL"
  echo "    Smoke admin credentials: $SMOKE_ADMIN_EMAIL / $SMOKE_ADMIN_PASSWORD"
