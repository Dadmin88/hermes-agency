...
Operational contract for the hosted `aws_secrets_manager` secret provider used by Hermes Fabric Cloud.

## Scope

- Hosted provider for Hermes Fabric-managed secrets when Hermes Fabric Cloud runs on AWS.
- Source of truth for secret values is AWS Secrets Manager, not Postgres.
- Hermes Fabric stores only metadata needed for ownership, bindings, version selection, audit, and runtime resolution.
- AWS provider bootstrap credentials are deployment/runtime credentials, not Hermes Fabric-managed company secrets.
- Remote import for existing AWS secrets is metadata-only. Preview/import uses
  AWS inventory metadata and creates Hermes Fabric external references; it does not
  copy plaintext into Hermes Fabric.
...
The AWS provider has a chicken-and-egg boundary: Hermes Fabric cannot use
`company_secrets` to unlock the AWS provider that stores those secrets. The
initial AWS trust must exist before the Hermes Fabric server starts.

Allowed bootstrap locations:

- Infrastructure IAM or workload identity attached to the Hermes Fabric server
  runtime.
- Process environment or orchestrator secret store used to start the Hermes Fabric
  server.
...
keys into the Hermes Fabric board UI. Do not store those bootstrap keys in
`company_secrets`.

## Hermes Fabric Cloud Bootstrap

Hermes Fabric Cloud must provision the AWS backing resources before any board user
can create AWS-backed company secrets:

1. Create or select the deployment KMS key.
2. Create the Hermes Fabric server runtime role for the deployment.
...
4. Configure the server runtime with the non-secret provider environment
   variables below.
5. Run `hermes-fabric doctor` or the provider health endpoint from the deployed
...
Once this is in place, the board UI can create Hermes Fabric-managed AWS secrets and
Hermes Fabric will write them under the deployment/company namespace.
...
```sh
aws sso login --profile fabric-dev
AWS_PROFILE=fabric-dev \
HERMES_FABRIC_SECRETS_PROVIDER=aws_secrets_manager \
HERMES_FABRIC_SECRETS_AWS_REGION=us-east-1 \
HERMES_FABRIC_SECRETS_AWS_DEPLOYMENT_ID=dev-local \
HERMES_FABRIC_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-... \
pnpm dev
```
...
should not be written to Hermes Fabric config, committed to `.env` files, stored in
`company_secrets`, or used as the default Hermes Fabric Cloud bootstrap path.
...
HERMES_FABRIC_SECRETS_PROVIDER=aws_secrets_manager
HERMES_FABRIC_SECRETS_AWS_REGION=us-east-1
HERMES_FABRIC_SECRETS_AWS_DEPLOYMENT_ID=prod-us-1
HERMES_FABRIC_SECRETS_AWS_KMS_KEY_ID=arn:aws:kms:us-east-1:123456789012:key/abcd-...
```
...
```sh
HERMES_FABRIC_SECRETS_AWS_PREFIX=fabric
HERMES_FABRIC_SECRETS_AWS_ENVIRONMENT=production
HERMES_FABRIC_SECRETS_AWS_PROVIDER_OWNER=fabric
HERMES_FABRIC_SECRETS_AWS_ENDPOINT=
HERMES_FABRIC_SECRETS_AWS_DELETE_RECOVERY_DAYS=30
```

Naming convention for Hermes Fabric-managed secrets:

```text
fabric/{deploymentId}/{companyId}/{secretKey}
```

Tag set for Hermes Fabric-managed secrets:

- `fabric:managed-by=fabric`
- `fabric:provider-owner=<owner tag>`
- `fabric:deployment-id=<deployment id>`
- `fabric:company-id=<company id>`
- `fabric:secret-key=<secret key>`
- `fabric:environment=<environment tag>`

## IAM And KMS Assumptions

Launch posture:

- One Hermes Fabric app role per deployment.
- One deployment-scoped KMS key per deployment at launch.
- Future per-company KMS keys remain compatible because Hermes Fabric stores provider refs and version metadata separately from values.
...
```text
arn:aws:secretsmanager:<region>:<account-id>:secret:fabric/<deployment-id>/*
```
...
- Prefer workload identity / role-based auth. Do not store AWS credentials inline in Hermes Fabric config.
...
```json
{
  ...
  "Statement": [
    {
      "Sid": "HermesFabricDeploymentSecrets",
      "Effect": "Allow",
      "Action": [
      ...
      "Resource": "arn:aws:secretsmanager:<region>:<account-id>:secret:fabric/<deployment-id>/*"
    },
    {
      "Sid": "HermesFabricDeploymentKms",
      "Effect": "Allow",
      "Action": [
        "kms:Encrypt",
        "kms:Decrypt",
...
- Hermes Fabric-managed secrets may be deleted only by Hermes Fabric or an operator with equivalent break-glass access.
- External references may resolve through Hermes Fabric runtime, but Hermes Fabric should not delete the external secret resource.
...
```json
{
  "Sid": "HermesFabricRemoteSecretInventory",
  "Effect": "Allow",
...
operator-approved external prefixes that Hermes Fabric is allowed to consume:

```json
{
  "Sid": "HermesFabricResolveImportedExternalReferences",
  "Effect": "Allow",
...
permissions scoped to `fabric/<deployment-id>/*`; do not broaden them for
remote import.

Safe scoping guidance:

- Prefer one Hermes Fabric runtime role per environment/account.
...
Hermes Fabric also blocks importing refs under its own managed namespace as
external references. Use the Hermes Fabric-managed flow for
`fabric/{deploymentId}/{companyId}/{secretKey}` resources.

## Existing AWS Secrets

V1 keeps existing AWS Secrets Manager entries as **linked external references**, not adopted
Hermes Fabric-managed resources.

Use the Hermes Fabric-managed flow when Hermes Fabric should create and rotate the value. The AWS
...
```text
/fabric-bench/anthropic_api_key
```

In that mode Hermes Fabric stores only the path or ARN, resolves it at runtime, and records
redacted access events. Operators rotate the actual value in AWS. Update the Hermes Fabric
reference only when the AWS path, ARN, or pinned provider version changes.

Hermes Fabric does not currently offer an "adopt existing AWS secret" flow that takes over future
`PutSecretValue` writes for an arbitrary existing secret. Adding that later requires explicit
confirmation UX, scope validation, expected Hermes Fabric tags, and security/cloud-ops review.

## Data Custody

- Hermes Fabric stores `externalRef`, `providerVersionRef`, provider id, fingerprint hash, status, and binding metadata.
- Hermes Fabric does not store AWS secret plaintext in `company_secret_versions.material`.
- Runtime resolution fetches the value from AWS only when a bound consumer needs it.

## Rotation Runbook

Manual Hermes Fabric-managed rotation:

1. Write the new value through the Hermes Fabric secret rotate flow.
2. Hermes Fabric creates a new AWS secret version with `PutSecretValue`.
3. Hermes Fabric records the new `providerVersionRef` in `company_secret_versions`.
4. Re-run or restart affected workloads that consume `latest`, or pin consumers to a specific Hermes Fabric version before rollout when you need staged release safety.

Guidance:

- Prefer pinned Hermes Fabric secret versions for risky rollouts.
...
- Hermes Fabric database metadata for secret ownership, bindings, status, and provider version refs.
- AWS Secrets Manager namespace under the configured deployment prefix.
- The configured KMS key and its decrypt permissions.

Restore checklist:

1. Restore Hermes Fabric database metadata.
2. Confirm the same AWS Secrets Manager namespace still exists.
3. Confirm the Hermes Fabric runtime role can call `GetSecretValue` on the restored prefix.
4. Confirm the role still has decrypt access to the CMK referenced by `HERMES_FABRIC_SECRETS_AWS_KMS_KEY_ID`.
...
1. Stop or pause affected Hermes Fabric runs.
2. Audit recent Hermes Fabric secret access events for impacted secret ids and consumers.
...
4. Rotate impacted secrets in AWS through Hermes Fabric-managed versioning.
...
- `HERMES_FABRIC_SECRETS_PROVIDER=aws_secrets_manager`
- The required `HERMES_FABRIC_SECRETS_AWS_*` environment variables set.

Suggested smoke:

1. Create a test secret through the Hermes Fabric board or API under a throwaway company.
2. Confirm the resulting AWS secret name matches `fabric/{deploymentId}/{companyId}/{secretKey}`.
3. Rotate the secret once and confirm a new `providerVersionRef` appears in Hermes Fabric metadata.
