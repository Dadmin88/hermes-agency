...
Hermes Fabric V1 explicitly excludes a plugin framework in [doc/SPEC-implementation.md](../SPEC-implementation.md), but the long-horizon spec says the architecture should leave room for extensions. This report studies the `opencode` plugin system and translates the useful patterns into a Hermes Fabric-shaped design.

Assumption for this document: Hermes Fabric is a single-tenant operator-controlled instance. Plugin installation should therefore be global across the instance. "Companies" are still first-class Hermes Fabric objects, but they are organizational records, not tenant-isolation boundaries for plugin trust or installation.
...
That model works well for a local coding tool. It should not be copied literally into Hermes Fabric.

The main conclusion is:

- Hermes Fabric should copy `opencode`'s typed SDK, deterministic loading, low authoring friction, and clear extension surfaces.
- Hermes Fabric should not copy `opencode`'s trust model, project-local plugin loading, "override by name collision" behavior, or arbitrary in-process mutation hooks for core business logic.
- Hermes Fabric should use multiple extension classes instead of one generic plugin bag:
...
If Hermes Fabric does this well, the examples you listed become straightforward:
...
Relevant Hermes Fabric files reviewed for current extension seams:
...
It is too dangerous for Hermes Fabric core actions.

However, the concept of plugins contributing agent-usable tools is very valuable for Hermes Fabric — as long as plugin tools are namespaced (cannot shadow core tools) and capability-gated.
...
Hermes Fabric should absolutely do this.
...
Hermes Fabric should copy this discipline.
...
Hermes Fabric should adopt this style for plugin actions, automations, and UI schemas.
...
Hermes Fabric can benefit from that with adapters, secret backends, storage providers, and connector modules.
...
That is the correct mindset for Hermes Fabric too.

## What Hermes Fabric Should Not Copy Directly
...
Hermes Fabric is a control plane for an operator-managed instance with company objects.
...
Hermes Fabric is not project-scoped. It is instance-scoped.
The comparable unit is:

- instance-installed plugin package

Hermes Fabric should not auto-load arbitrary code from a workspace repo like `.fabric/plugins` or project directories.
...
For Hermes Fabric, equivalent hooks into:
...
Hermes Fabric should not allow plugins to silently replace:
...
For Hermes Fabric it would be risky because it combines:
...
Hermes Fabric should require an explicit operator install step.

## Why Hermes Fabric Needs A Different Shape

The products are solving different problems.

| Topic | OpenCode | Hermes Fabric |
|---|---|---|
| Primary unit | local project/worktree | single-tenant operator instance with company objects |
| Trust assumption | local power user on own machine | operator managing one trusted Hermes Fabric instance |
...
That means Hermes Fabric should borrow the good ideas from `opencode` but use a stricter architecture.

## Hermes Fabric Already Has Useful Pre-Plugin Seams

Hermes Fabric has several extension-like seams already:
...
This is good news.
Hermes Fabric does not need to invent extensibility from scratch.
It needs to unify and harden existing seams.

## Recommended Hermes Fabric Plugin Model
...
Hermes Fabric already has this pattern implicitly:
...
- `plugin` means integration code that talks to Hermes Fabric through a typed plugin protocol and capability model
...
- invoke explicit Hermes Fabric actions through the API
...
**What the host controls:** where plugin components appear, the bridge API, capability enforcement, and shared UI primitives (`@hermes-fabric/plugin-sdk/ui`) with design tokens and common components.
...
Hermes Fabric should treat plugin installation as a global instance-level action.

Examples:

- install `@fabric/plugin-linear` once
- make it available everywhere immediately
- optionally store mappings over Hermes Fabric objects if one company maps to a different Linear team than another

## 6. Use project workspaces as the primary anchor for local tooling

Hermes Fabric already has a concrete workspace model for projects:
...
`opencode` makes tools a first-class extension point. This is one of the highest-value surfaces for Hermes Fabric too.
...
Plugins should be able to emit custom events that other plugins can subscribe to. For example, the git plugin detects a push and emits `plugin.@fabric/plugin-git.push-detected`. The GitHub Issues plugin subscribes to that event and updates PR links.
...
A `@hermes-fabric/plugin-test-harness` package should provide a mock host with in-memory stores, synthetic event emission, and `getData`/`performAction`/`executeTool` simulation. Plugin authors should be able to write unit tests without a running Hermes Fabric instance.

A `create-fabric-plugin` CLI should scaffold a working plugin with manifest, worker, UI bundle, test file, and build config.

Low authoring friction was called out as one of `opencode`'s best qualities. The test harness and starter template are how Hermes Fabric achieves the same.

## 14. Support hot plugin lifecycle

Plugin install, uninstall, upgrade, and config changes should take effect without restarting the Hermes Fabric server. This is critical for developer workflow and operator experience.
...
`opencode` does not have a formal SDK versioning story because plugins run in-process and are effectively pinned to the current runtime. Hermes Fabric's out-of-process model means plugins may be built against one SDK version and run on a host that has moved forward. This needs explicit rules.

Recommended approach:

- **Single SDK package**: `@hermes-fabric/plugin-sdk` with subpath exports — root for worker code, `/ui` for frontend code. One dependency, one version, one changelog.
- **SDK major version = API version**: `@hermes-fabric/plugin-sdk@2.x` targets `apiVersion: 2`. Plugins built with SDK 1.x declare `apiVersion: 1` and continue to work.
...
## A Concrete SDK Shape For Hermes Fabric

An intentionally narrow first pass could look like this:

```ts
import { definePlugin, z } from "@hermes-fabric/plugin-sdk";

export default definePlugin({
  id: "@fabric/plugin-linear",
  version: "0.1.0",
  ...
  async register(ctx) {
    ctx.jobs.register("linear-pull", { cron: "*/5 * * * *" }, async (job) => {
      // sync Linear issues into plugin-owned state or explicit HermesFabric entities
    });
    ...
    ctx.events.on("plugin.@fabric/plugin-git.push-detected", async (event) => {
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, ErrorBoundary } from "@hermes-fabric/plugin-sdk/ui";
...
Any Hermes Fabric plugin system has to preserve core control-plane invariants from the repo docs.
...
If it needs mappings over specific Hermes Fabric objects, those are plugin data, not enable/disable boundaries.
...
- `actor_type = plugin`
- `actor_id = <plugin-id>` (e.g. `@fabric/plugin-linear`)
...
- reuse core tables when the data is clearly part of Hermes Fabric itself
...
If a concept is becoming part of Hermes Fabric's actual product model, it should get a normal first-party table.

Examples:

- `project_workspaces` is already a core table because project workspaces are now part of Hermes Fabric itself
...
### 3. Scope plugin data to Hermes Fabric objects before adding custom schemas

A lot of plugin data naturally hangs off existing Hermes Fabric objects:
...
If Hermes Fabric eventually needs extension-owned tables, I would only allow that for:
...
Package idea: `@fabric/plugin-workspace-files`
...
Package idea: `@fabric/plugin-terminal`
...
Package idea: `@fabric/plugin-git`
...
- Board opens a project page and reviews the diff for that project's workspace without leaving Hermes Fabric.
- Board reviews the diff after a run without leaving Hermes Fabric.
...
- optional `events.emit` (e.g. `plugin.@fabric/plugin-git.push-detected`)
- `activity.log.write`
...
The git plugin can emit `plugin.@fabric/plugin-git.push-detected` events that other plugins (e.g. GitHub Issues) subscribe to for cross-plugin coordination.
...
Package idea: `@fabric/plugin-linear`

This plugin syncs Hermes Fabric work with Linear. It is useful for:

- importing backlog from Linear
- linking Hermes Fabric issues to Linear issues
...
- Plugin settings:
  ...
  - status mapping between Hermes Fabric and Linear
...
- Board enables the plugin, maps a Linear team, and imports a backlog into Hermes Fabric.
- Hermes Fabric issue status changes push to Linear and Linear comments arrive back through webhooks.
...
Package idea: `@fabric/plugin-github-issues`

This plugin syncs Hermes Fabric issues with GitHub Issues and optionally links PRs. It is useful for:

- importing repo backlogs
- mirroring issue status and comments
- linking PRs to Hermes Fabric issues
- tracking cross-repo work from inside one company view
- bridging engineering workflow with Hermes Fabric task governance
...
- Plugin settings:
  ...
  - whether new Hermes Fabric issues should create GitHub issues automatically
...
- Dashboard widget:
  - open PRs linked to active Hermes Fabric issues
  - webhook failures
  - sync lag metrics

Core workflows:

- Board imports GitHub Issues for a repo into Hermes Fabric.
- GitHub webhooks update status/comment state in Hermes Fabric.
- A PR is linked back to the Hermes Fabric issue so the board can follow delivery status.
...
- `events.subscribe(issue.updated)`
- `events.subscribe(issue.comment.created)`
- `events.subscribe(plugin.@fabric/plugin-git.push-detected)` (cross-plugin coordination)
- `jobs.schedule`
- `webhooks.receive`
- `http.outbound`
- `secrets.read-ref`
...
Package idea: `@fabric/plugin-grafana`

This plugin surfaces external metrics and dashboards inside Hermes Fabric. It is useful for:

- company KPI visibility
- infrastructure/incident monitoring
- showing deploy, traffic, latency, or revenue charts next to work
- creating Hermes Fabric issues from anomalous metrics
...
- Dashboard widgets:
  ...
  - link out to Grafana and link in to the full Hermes Fabric plugin page
- Full metrics page:
...
- Board sees service degradation or business KPI movement directly on the Hermes Fabric dashboard.
- Board clicks into the full metrics page to inspect the relevant Grafana panels.
- Board creates a Hermes Fabric issue from a threshold breach with a metric snapshot attached.
...
- do not make Grafana alerting logic part of Hermes Fabric core; keep it as additive signal and issue creation

## Child Process / Server Tracking

Package idea: `@fabric/plugin-runtime-processes`
...
Package idea: `@fabric/plugin-stripe`

This plugin pulls Stripe revenue and subscription data into Hermes Fabric. It is useful for:
...
- Failed payment spikes or churn events can generate Hermes Fabric issues for follow-up.
...
- Stripe data should stay additive to Hermes Fabric core
...
- plugin UI bundle loading, host bridge, `@hermes-fabric/plugin-sdk/ui`
...
- `@hermes-fabric/plugin-test-harness` and `create-fabric-plugin` starter template
...
Hermes Fabric should not implement "an OpenCode-style generic in-process hook system."
Hermes Fabric should implement "a plugin platform with multiple trust tiers":
...
## Concrete Next Steps I Would Take In Hermes Fabric
...
8. Ship `@hermes-fabric/plugin-test-harness` and `create-fabric-plugin` starter template.
