# Hermes Fabric Plugin System Specification

Status: proposed complete spec for the post-V1 plugin system

This document is the complete specification for Hermes Fabric's plugin and extension architecture.
...
- Plugin UI bundles currently run as same-origin JavaScript inside the main Hermes Fabric app. Treat plugin UI as trusted code, not a sandboxed frontend capability boundary.
- Manifest capabilities currently gate worker-side host RPC calls. They do not prevent plugin UI code from calling ordinary Hermes Fabric HTTP APIs directly.
...
- The current runtime ships a small host-provided plugin UI component kit through `@hermes-fabric/plugin-sdk/ui`, but does not support plugin asset uploads/reads yet. Treat plugin asset APIs as future-scope ideas, not current implementation promises.
...
Hermes Fabric plugin design is based on the following assumptions:

1. Hermes Fabric is single-tenant and self-hosted.
2. Plugin installation is global to the instance.
3. "Companies" remain core Hermes Fabric business objects, but they are not plugin trust boundaries.
4. Board governance, approval gates, budget hard-stops, and core task invariants remain owned by Hermes Fabric core.
...
2. Let plugins add major capabilities without editing Hermes Fabric core.
...
4. Depend on project-local plugin folders such as `.fabric/plugins`.
...
The single Hermes Fabric deployment an operator installs and controls.

### 5.2 Company

A first-class Hermes Fabric business object inside the instance.
...
A trusted in-process extension loaded directly by Hermes Fabric core.
...
An installable instance-wide extension package loaded through the Hermes Fabric plugin runtime.
...
Hermes Fabric has two extension classes.
...
Hermes Fabric already has a concrete workspace model:
...
Plugins live under the Hermes Fabric instance directory.

Suggested layout:

- `~/.hermes-fabric/instances/default/plugins/package.json`
- `~/.hermes-fabric/instances/default/plugins/node_modules/`
- `~/.hermes-fabric/instances/default/plugins/.cache/`
- `~/.hermes-fabric/instances/default/data/plugins/<plugin-id>/`
...
Hermes Fabric should add CLI commands:

- `pnpm hermes-fabric plugin list`
- `pnpm hermes-fabric plugin install <package[@version]>`
- `pnpm hermes-fabric plugin uninstall <plugin-id>`
- `pnpm hermes-fabric plugin upgrade <plugin-id> [version]`
- `pnpm hermes-fabric plugin doctor <plugin-id>`
...
- UI slot IDs are automatically namespaced by plugin ID (e.g. `@fabric/plugin-linear:sync-health-widget`), so cross-plugin collisions are structurally impossible
...
export interface HermesFabricPluginManifestV1 {
  ...
  minimumHermesFabricVersion?: string;
  capabilities: string[];
  entrypoints: {
    worker: string;
    ui?: string;
...
- `minimumHostVersion` is preferred, with `minimumHermesFabricVersion` retained for
...
Plugins may contribute tools that Hermes Agency agents can use during runs.
...
- Hermes Fabric server starts one worker process per installed plugin
...
Receives one typed Hermes Fabric domain event.
...
Trusted orchestration plugins can create and update Hermes Fabric issues through `ctx.issues` instead of importing server internals. The public issue contract includes parent/project/goal links, board or agent assignees, blocker IDs, labels, billing code, request depth, execution workspace inheritance, and plugin origin metadata.

Plugins that perform durable work should declare managed Hermes Fabric resources rather than using private plugin state:
...
export function definePlugin(definition: PluginDefinition): HermesFabricPlugin;
...
6. For recurring business workflows that should create visible Hermes Fabric work, prefer managed routines and managed resources over jobs. Jobs remain useful for private plugin-runtime maintenance tasks.
...
```tsx
// dist/ui/index.tsx
import { usePluginData, usePluginAction, MetricCard, StatusBadge } from "@hermes-fabric/plugin-sdk/ui";
...
- The host provides **design tokens and shared components** via `@hermes-fabric/plugin-sdk/ui` so plugins can match the host's visual language without being forced to.
...
### 19.0.1 Plugin UI SDK (`@hermes-fabric/plugin-sdk/ui`)
...
Hermes Fabric-internal pages. It exposes `resolveHref(to)`, `navigate(to,
...
Hermes Fabric navigation because those can force a full document reload.
...
- Plugin bundles must not import from host internals. They may only import from `@hermes-fabric/plugin-sdk/ui` and their own dependencies.
...
## 19.6 Shared Components In `@hermes-fabric/plugin-sdk/ui`
...
The `@hermes-fabric/plugin-sdk/ui` subpath should also export an `ErrorBoundary` component that plugin authors can use to catch rendering errors without crashing the host page.
...
- secret ref fields: any schema property annotated with `"format": "secret-ref"` renders as a secret picker that resolves through the Hermes Fabric secret provider system rather than a plain text input
...
1. Core Hermes Fabric data stays in first-party tables.
2. Most plugin-owned data starts in generic extension tables.
3. Plugin data should scope to existing Hermes Fabric objects before new tables are introduced.
4. Arbitrary third-party schema migrations are out of scope for the first plugin system.

## 21.2 Core Table Reuse

If data becomes part of the actual Hermes Fabric product model, it should become a first-party table.

Examples:

- `project_workspaces` is already first-party
- if Hermes Fabric later decides git state is core product data, it should become a first-party table too
...
2. Secret refs resolve through the existing Hermes Fabric secret provider system.
...
6. The operator may force-purge immediately via CLI: `pnpm hermes-fabric plugin purge <plugin-id>`.
...
Plugin install, uninstall, upgrade, and config changes **must** take effect without restarting the Hermes Fabric server. This is a normative requirement, not optional.
...
### 27.1 `@hermes-fabric/plugin-test-harness`
...
```ts
import { createTestHarness } from "@hermes-fabric/plugin-test-harness";
...
For developing a plugin against a running Hermes Fabric instance:

- The operator installs the plugin from a local path: `pnpm hermes-fabric plugin install ./path/to/plugin`
...
The host should publish a starter template (`create-fabric-plugin`) that scaffolds:

- `package.json` with correct `fabricPlugin` keys
...
- `@fabric/plugin-workspace-files`
- `@fabric/plugin-terminal`
- `@fabric/plugin-git`
- `@fabric/plugin-linear`
- `@fabric/plugin-github-issues`
- `@fabric/plugin-grafana`
- `@fabric/plugin-runtime-processes`
- `@fabric/plugin-stripe`
...
- `@hermes-fabric/plugin-sdk` — the complete plugin SDK

The package uses subpath exports to separate worker and UI concerns:

- `@hermes-fabric/plugin-sdk` — worker-side SDK (context, events, state, tools, logger, `definePlugin`, `z`)
- `@hermes-fabric/plugin-sdk/ui` — frontend SDK (bridge hooks, shared components, design tokens)
...
2. **Tied to API version**: Each major SDK version corresponds to exactly one plugin `apiVersion`. When `@hermes-fabric/plugin-sdk@2.x` ships, it targets `apiVersion: 2`. Plugins built with SDK 1.x continue to declare `apiVersion: 1`.
...
1. Plugin author updates `@hermes-fabric/plugin-sdk` dependency.
...
- webhooks
- settings page
- plugin UI bundle loading, host bridge, and `@hermes-fabric/plugin-sdk/ui`
...
- `@hermes-fabric/plugin-test-harness`
- `create-fabric-plugin` starter template
...
Hermes Fabric should not implement a generic in-process hook bag modeled directly after local coding tools.

Hermes Fabric should implement:
...
That is the complete target design for the Hermes Fabric plugin system.
