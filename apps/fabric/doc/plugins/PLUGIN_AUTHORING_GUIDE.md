...
This guide describes the current, implemented way to create a Hermes Fabric plugin in this repo.
...
- Plugin UI runs as same-origin JavaScript inside the main Hermes Fabric app.
...
- The host provides a small shared React component kit through
  `@hermes-fabric/plugin-sdk/ui`; use it for common Hermes Fabric controls before
...
Hermes Fabric owns inline markdown rendering; plugins must not return React, HTML,
...
```bash
hermes-fabric plugin init @yourscope/plugin-name --output /absolute/path/to/plugin-repos
```
...
Inside this monorepo, the scaffold uses `workspace:*` for `@hermes-fabric/plugin-sdk`.

Outside this monorepo, the scaffold snapshots `@hermes-fabric/plugin-sdk` from the local Hermes Fabric checkout into a `.fabric-sdk/` tarball so you can build and test a plugin without publishing anything to npm first. Pass `--sdk-path /absolute/path/to/fabric/packages/plugins/sdk` if you have more than one Hermes Fabric checkout.

## Local development workflow

See the short [Local Plugin Development guide](./LOCAL_PLUGIN_DEVELOPMENT.md) for the full happy path (`pnpm dev` → `hermes-fabric plugin install <absolute-path>` → `hermes-fabric plugin list`) and reload semantics.
...
## Managed Hermes Fabric resources

Plugins that provide durable Hermes Fabric business objects should declare them in
...
- Managed agents: declare top-level `agents[]` and require
  ...
  normal Hermes Agency agents with plugin ownership metadata, not background plugin
  workers.
...
- Managed routines: declare top-level `routines[]` and require
  ...
  jobs that should create visible Hermes Fabric issues. Prefer managed routines over
...
- Managed skills: declare top-level `skills[]` and require `skills.managed`.
  ...
  synced into Hermes Fabric managed agents.
  ...
  agents: [
    {
      ...
      instructions: {
        content: "Follow the HermesFabric heartbeat and produce concise research briefs.",
      },
    },
  ],
  projects: [
...
- Use managed routines for recurring or externally triggered work that should
  ...
  triggers, and each run has the normal Hermes Fabric issue/audit trail.
...
- Keep defaults conservative. Managed declarations are suggestions owned by the
  plugin, but the resulting resources are normal Hermes Fabric records that the
...
- `usePluginToast`
- `useHostContext`
- typed slot props from `@hermes-fabric/plugin-sdk/ui`
...
Use shared components from `@hermes-fabric/plugin-sdk/ui` when the plugin needs a
Hermes Fabric-native control. The host owns the implementation, so plugins inherit
the board's current styling, ordering, recent selections, and dark-mode behavior
without importing `ui/src` internals.

Prefer shared components for common Hermes Fabric UX patterns to reduce drift and
...
Use `FileTree` from `@hermes-fabric/plugin-sdk/ui` whenever the plugin only needs
