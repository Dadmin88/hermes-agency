...
Low-level reference for how Hermes Fabric packages are prepared and published to npm.
...
Hermes Fabric no longer uses release branches or Changesets for publishing.

## Why the CLI needs special packaging

The CLI package, `hermes-fabric`, imports code from workspace packages such as:
...
### Manual first publish for `@hermes-fabric/ui`
...
npm view @hermes-fabric/ui version

# make sure the dist payload is fresh
pnpm --filter @hermes-fabric/ui build
...
- If `npm view @hermes-fabric/ui version` already returns the same version that is in [`ui/package.json`](../ui/package.json), do not republish. Bump the version or use the normal repo-wide release flow in [`scripts/release.sh`](../scripts/release.sh).
...
- For an organization-scoped package like `@hermes-fabric/ui`, the `hermes-fabric` npm organization must exist and the publisher must be a member with permission to publish to that scope.
...
Hermes Fabric uses calendar versions:
...
- `hermes-fabric@2026.318.1-canary.2`

This keeps the default install path unchanged while allowing explicit installs with:

```bash
npx hermes-fabric@canary onboard
```
...
- every published internal `@hermes-fabric/*` dependency referenced by that manifest exists on npm
...
Hermes Fabric does not auto-publish every non-private workspace package anymore.
...
Example for `@hermes-fabric/adapter-acpx-local` from the repo root:

```bash
# safe preview
pnpm run release:bootstrap-package -- @hermes-fabric/adapter-acpx-local

# one-time first publish from an authenticated maintainer machine
pnpm run release:bootstrap-package -- @hermes-fabric/adapter-acpx-local --publish --otp 123456
```
...
1. open `https://www.npmjs.com/package/@hermes-fabric/adapter-acpx-local`
2. go to `Settings` → `Trusted publishing`
3. add repository `hermes-fabric/fabric`
