...
Hermes Fabric uses PostgreSQL via [Drizzle ORM](https://orm.drizzle.team/). There are three ways to run the database, from simplest to most production-ready.
...
1. Creates a `~/.hermes-fabric/instances/default/db/` directory for storage
2. Ensures the `fabric` database exists
3. Runs migrations automatically for empty databases
4. Starts serving requests

Data persists across restarts in `~/.hermes-fabric/instances/default/db/`. To reset local dev data, delete that directory.
...
When `DATABASE_URL` is unset, this command targets the current embedded PostgreSQL instance for your active Hermes Fabric config/instance.
...
Docker note: the Docker quickstart image also uses embedded PostgreSQL by default. Persist `/fabric` to keep DB state across container restarts (see `doc/DOCKER.md`).
...
If you later run the app with a pooled runtime URL, set `DATABASE_MIGRATION_URL` to the direct connection URL. Hermes Fabric uses it for startup schema checks/migrations and plugin namespace migrations, while the app continues to use `DATABASE_URL` for runtime queries:
...
If your hosted database requires transaction-pooling-only connections, use a direct or session-pooled connection for Hermes Fabric until runtime pooling support is documented in this guide. Do not edit database client source files as part of deployment setup.
...
| `DATABASE_URL` | Mode |
|---|---|
| Not set | Embedded PostgreSQL (`~/.hermes-fabric/instances/default/db/`) |
...
Hermes Fabric stores current-user sidebar membership state in:
...
Hermes Fabric supports automatic and manual logical database backups. These dumps include
...
`hermes-fabric db:backup` / `pnpm db:backup` commands and backup retention
configuration.
...
Hermes Fabric stores secret metadata and versions in:
...
- Default key file: `~/.hermes-fabric/instances/default/secrets/master.key` (auto-created if missing).
- CLI config location: `~/.hermes-fabric/instances/default/config.json` under `secrets.localEncrypted.keyFilePath`.
...
- `HERMES_FABRIC_SECRETS_MASTER_KEY` (32-byte key as base64, hex, or raw 32-char string)
