...
`pnpm test:hermes-gateway-smoke` does not start Docker, Hermes, or Hermes Fabric. It
...
- Board/operator auth is required through `HERMES_FABRIC_AUTH_HEADER`,
  `HERMES_FABRIC_COOKIE`, or a board-capable `HERMES_FABRIC_API_KEY`.
- Diagnostic files are redacted before they are written, except the join output
  file intentionally contains the claimed Hermes Fabric agent key and is written
  `chmod 600`.
...
- `HERMES_FABRIC_API_URL`: Hermes Fabric URL used by the operator shell.
- `HERMES_FABRIC_API_URL_FOR_HERMES`: Hermes Fabric URL used from inside the Hermes
  container or remote Hermes host.
- `HERMES_GATEWAY_API_BASE_URL`: Hermes gateway URL stored on the Hermes Fabric
  adapter, reachable by the Hermes Fabric server.
...
## Docker Desktop or Linux host Hermes Fabric

Use this when Hermes Fabric runs on the host at `127.0.0.1:3100` and Docker can
...
## Same Docker network as Hermes Fabric

Use this when Hermes Fabric is a container on a Docker network and the Hermes smoke
...
HERMES_FABRIC_API_URL=http://127.0.0.1:3100 \
HERMES_FABRIC_AUTH_HEADER='Bearer <board-token>' \
HERMES_CONTAINER_NAME=fabric-hermes-gateway-smoke \
HERMES_SMOKE_NETWORK=fabric_default \
HERMES_DOCKER_ADD_HOST=0 \
HERMES_GATEWAY_API_BASE_URL=http://fabric-hermes-gateway-smoke:8642 \
HERMES_GATEWAY_PROBE_URL=http://127.0.0.1:8642 \
HERMES_FABRIC_API_URL_FOR_HERMES=http://fabric:3100 \
HERMES_GATEWAY_ALLOW_INSECURE_HTTP=1 \
pnpm smoke:hermes-gateway-e2e
```

Change `fabric_default` and `fabric` to your Compose network and service
name. The unsafe HTTP flag is required because Hermes Fabric stores a non-loopback
`http://` gateway URL for private Docker DNS.

## LAN or private-network Hermes Fabric

Use this when Hermes Fabric is exposed on a private IP or tailnet address and the
Hermes container can reach that address.

```sh
HERMES_FABRIC_API_URL=http://192.168.1.20:3100 \
HERMES_FABRIC_AUTH_HEADER='Bearer <board-token>' \
HERMES_FABRIC_API_URL_FOR_HERMES=http://192.168.1.20:3100 \
...
Use this when Hermes Fabric should talk to Hermes through a TLS hostname. The smoke
container still publishes a local port, and your reverse proxy forwards the TLS
hostname to that port.

```sh
HERMES_FABRIC_API_URL=https://fabric.example.com \
HERMES_FABRIC_AUTH_HEADER='Bearer <board-token>' \
HERMES_FABRIC_API_URL_FOR_HERMES=https://fabric.example.com \
...
`HERMES_GATEWAY_API_BASE_URL` to the URL Hermes Fabric should store on the adapter.
