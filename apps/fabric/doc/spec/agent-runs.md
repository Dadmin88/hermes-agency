...
This spec defines how Hermes Fabric actually runs agents while staying runtime-agnostic.
...
1. Hermes Fabric is adapter-agnostic. The key is a protocol, not a specific runtime.
...
6. Heartbeats run the configured adapter process, Hermes Fabric manages lifecycle, and on exit Hermes Fabric parses JSON output and updates state.
...
- `run.startedAt`
- `heartbeat.reason`
- `fabric.skill` (shared Hermes Fabric skill text block)
...
2. Preferred transport is env vars (`HERMES_FABRIC_*`) injected at runtime.
