...
For `claude_local` and `codex_local`, Hermes Fabric assumes the CLI is already installed and authenticated on the host machine.
...
Hermes Fabric stores resumable session state per `(agent, taskKey, adapterType)`.
