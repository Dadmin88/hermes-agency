...
must be attached to the Hermes Fabric issue before the agent chooses a final
...
```sh
skills/fabric/scripts/fabric-upload-artifact.sh path/to/output.webm \
  --title "Walkthrough render" \
  --summary "Rendered walkthrough for review"
```

The helper uses the authenticated Hermes Fabric API from the current heartbeat
environment:
...
`artifact` and `provider` to `fabric`, with metadata canonicalized from the
...
Workspace file links resolve only inside registered Hermes Fabric workspaces. The
...
when it carries both that `projectId` and `workspaceId`. Hermes Fabric does not
...
2. Upload it with `skills/fabric/scripts/fabric-upload-artifact.sh`.
...
```sh
skills/fabric/scripts/fabric-upload-artifact.sh render.bin \
  --title "Demo video render" \
...
```sh
curl -sS -X POST \
  "$HERMES_FABRIC_API_URL/api/companies/$HERMES_FABRIC_COMPANY_ID/issues/$HERMES_FABRIC_TASK_ID/attachments" \
...
```sh
curl -sS -X POST \
  "$HERMES_FABRIC_API_URL/api/issues/$HERMES_FABRIC_TASK_ID/work-products" \
  -H "Authorization: Bearer $HERMES_FABRIC_API_KEY" \
  -H "X-HermesFabric-Run-Id: $HERMES_FABRIC_RUN_ID" \
...
Use `type: "artifact"`, `provider: "fabric"`, and metadata containing the
