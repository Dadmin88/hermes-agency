...
`PAP-447` asks how Hermes Fabric should support worktree-driven coding workflows for local coding agents without turning that into a universal product requirement.
...
- the workflow should reuse the same Hermes Fabric instance and embedded database instead of creating a blank environment
- local agent auth should remain low-friction

At the same time, we do not want to hard-code "every agent uses git worktrees" into Hermes Fabric:

- some operators use Hermes Fabric to manage Hermes Fabric and want worktrees heavily
- other operators will not want worktrees at all
- not every adapter runs in a local git repository
- not every adapter runs on the same machine as Hermes Fabric
- Claude and Codex expose different built-in affordances, so Hermes Fabric should not overfit to one tool

## Core Product Decision

Hermes Fabric should model **execution workspaces**, not **worktrees**.
...
- local adapters may receive a real filesystem cwd produced by Hermes Fabric
...
- Hermes Fabric should not assume that every adapter can see or use a host filesystem path directly
...
If Hermes Fabric makes worktrees agent-first, it will blur:
...
- Hermes Fabric core resolves and records execution workspace state
...
That gives Hermes Fabric room to support:
...
- Hermes Fabric handles workspace runtime services under the hood using project/workspace policy
...
Once Hermes Fabric is creating isolated issue checkouts, it is implicitly touching a bigger workflow:
...
So Hermes Fabric should distinguish:
...
- only add adapter-specific runtime UI when an adapter truly needs settings that Hermes Fabric cannot infer
...
- when issue moves to done, should Hermes Fabric auto-commit?
- should it auto-push?
...
Hermes Fabric already has the right foundation for a project-first model.
...
For server-spawned local adapters, Hermes Fabric already injects a short-lived local JWT:
...
- Codex CLI does not present a single first-class portable CLI worktree abstraction that Hermes Fabric should mirror directly

Implication:

- for `codex_local`, Hermes Fabric should usually create/select the checkout itself and then launch Codex inside that cwd
...
- but Hermes Fabric should still treat that as an adapter optimization, not the canonical cross-adapter model
...
Because of that, Hermes Fabric should separate:
...
Hermes Fabric should be able to express intentions such as:
...
These are not specific to Hermes Fabric. They are a common property of working in a dev workspace, whether local or remote.
...
```json
{
  "workspaceStrategy": {
    ...
    "worktreeParentDir": ".fabric/instances/default/worktrees/projects/{{project.id}}",
...
Do not model this as a Hermes Fabric-specific `devServer` flag.
...
Hermes Fabric should distinguish between:
...
Hermes Fabric should normalize the reported metadata without requiring every adapter to look like a host-local process.
...
In addition to the local git helper, Hermes Fabric should define a generic runtime service manager contract.
...
- remote sandbox adapter: create its own isolated workspace from repo/ref/branch intent and report the realized remote workspace metadata back to Hermes Fabric
...
For runtime services specifically, Hermes Fabric should eventually track normalized fields such as:
...
- runtime services are expressed generically, not as Hermes Fabric-only dev-server flags
...
3. Inject resolved execution workspace details into `context.fabricWorkspace` for local adapters and into a generic execution-workspace intent payload for adapters that need structured remote realization.
...
- same embedded DB / Hermes Fabric instance can be reused through env/config injection when appropriate
...
- Hermes Fabric can decide whether to reuse or start a fresh service deterministically
...
3. Test against Hermes Fabric-on-Hermes Fabric workflows first.
...
7. Workspace runtime services are modeled generically, not as Hermes Fabric-specific dev-server toggles.
