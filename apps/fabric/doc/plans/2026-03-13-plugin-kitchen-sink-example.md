...
Add a new first-party example plugin, `Kitchen Sink (Example)`, that demonstrates every currently implemented Hermes Fabric plugin API surface in one place.
...
The plugin is successful if a contributor can install it and, without reading the SDK first, discover and exercise the current plugin runtime surface area from inside Hermes Fabric.
...
- package: `@hermes-fabric/plugin-kitchen-sink-example`
- plugin id: `fabric.kitchen-sink-example` or `fabric-kitchen-sink-example`

Recommendation: use `fabric-kitchen-sink-example` to match current in-repo example naming style.
...
- subscribe to a core Hermes Fabric event such as `issue.created`
...
Recommendation: default to a Hermes Fabric-local endpoint or a stable public echo endpoint to avoid flaky docs.
