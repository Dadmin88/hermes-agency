...
This document defines the product and UI plan for skill management in Hermes Fabric.
...
- Hermes Fabric company import/export can include skills as package content
...
3. The UI must always tell the truth about what Hermes Fabric knows:
   - desired state in Hermes Fabric
...
Hermes Fabric should treat skills at two scopes:
...
- created locally inside Hermes Fabric later
...
7. “Understand what Hermes Fabric knows vs what the adapter knows.”
...
- importing from a `skills.sh`-compatible source should work without requiring a Hermes Fabric-specific package layout
...
This matters because Codex and similar adapters may already have local skills that Hermes Fabric did not install.

These should be clearly marked:

- external
- not managed by Hermes Fabric
...
- `desired_only`: Hermes Fabric wants it, adapter does not show it yet
- `external`: adapter has it but Hermes Fabric does not manage it
- `drifted`: adapter has a conflicting or unexpected version/location
- `unmanaged`: adapter does not support sync, Hermes Fabric only tracks desired state
...
- Hermes Fabric can still track desired skills
...
- Hermes Fabric can see adapter skills
...
- `.fabric.yaml` may add Hermes Fabric-specific fidelity, but should not replace the base shortname association model
...
- `.fabric.yaml` may add Hermes Fabric-specific skill fidelity later if needed, but should not be required for ordinary agent-to-skill association
...
Hermes Fabric should have a company-scoped skill model or managed package model representing:
...
Hermes Fabric should separately store:
...
5. If agent skill associations are path-based instead of shortname-based, the format will feel too technical and too Hermes Fabric-specific.
...
That gives Hermes Fabric one coherent skill story instead of forcing package management, adapter sync, and agent configuration into the same screen.
