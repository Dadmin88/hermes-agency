...
We were asked to find the hook surfaces exposed by `pi` and `pi-mono`, then decide which ideas transfer cleanly into Hermes Fabric.
...
- current Hermes Fabric plugin and adapter surfaces in this repo
...
- Hermes Fabric should copy only the safe subset:
  - typed event subscriptions
  - read-only run lifecycle events
  - explicit worker lifecycle hooks
  - plugin-to-plugin events
- Hermes Fabric should not copy the dangerous subset:
...
## What Hermes Fabric already has

Hermes Fabric already has several hook-like surfaces, but they are much narrower and safer:
...
## What transfers well to Hermes Fabric

These ideas from `pi-mono` fit Hermes Fabric with little conceptual risk:

### 1. Read-only run lifecycle subscriptions

Hermes Fabric should continue exposing run and transcript events to plugins, for example:
...
This matches Hermes Fabric's control-plane posture: observe, react, automate.

### 2. Plugin-to-plugin events

Hermes Fabric already has this. It is worth keeping and extending.

This is the clean replacement for many ad hoc hook chains.

### 3. Explicit worker lifecycle hooks

Hermes Fabric already has `setup()` and `onHealth()`. That is the right shape.

If more lifecycle is needed, it should stay explicit and host-controlled.

### 4. Trusted adapter-level prompt/runtime middleware

Some `pi-mono` ideas do belong in Hermes Fabric, but only inside trusted adapter/runtime code:
...
These `pi-mono` capabilities are a bad fit for Hermes Fabric core:

### 1. Arbitrary mutation hooks on control-plane decisions

Hermes Fabric should not let general plugins rewrite:
...
Hermes Fabric should keep plugin tools namespaced and non-shadowing.

### 3. Project-local plugin loading

Hermes Fabric is an operator-controlled control plane. Repo-local plugin auto-loading would make behavior too implicit and too hard to govern.
...
They do not map directly to Hermes Fabric's board-and-issues model.

## Recommended Hermes Fabric direction
...
If the question is "what works for Hermes Fabric too?":
