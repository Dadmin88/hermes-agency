...
Hermes Fabric's module system lets you extend the control plane with new capabilities — revenue tracking, observability, notifications, dashboards — without forking core. Modules are self-contained packages that register routes, UI pages, database tables, and lifecycle hooks.
...
|---------|-----------|----------------|
| **Module** | A package that extends Hermes Fabric's API, UI, and data model | Yes |
...
modules/
  observability/
    fabric.module.json     # manifest (required)
    src/
...
- **`id`**: Unique identifier, used as the npm package name suffix (`@hermes-fabric/mod-observability`)
...
```typescript
import type { ModuleAPI } from "@hermes-fabric/core";
...
```
1. Scan modules/ directory for fabric.module.json manifests
...
```jsonc
// fabric.config.json (or env vars)
{
  "modules": {
...
export const modulePages = [
  {
    path: "/observability",
    label: "Observability",
    component: lazy(() => import("@hermes-fabric/mod-observability/ui")),
  },
];
...
export const dashboardWidgets = [
  {
    ...
    label: "Token Burn Rate",
    placement: "dashboard",
    component: lazy(() => import("@hermes-fabric/mod-observability/ui").then(m => ({ default: m.TokenBurnRateWidget }))),
  },
];
```
  ...
  "modules": [
    {
      ...
      "repo": "github:fabric/mod-observability",
      "version": "0.1.0",
  ...
  "templates": [
    {
      ...
      "url": "https://store.fabric.ing/templates/startup-in-a-box.json",
      "tags": ["startup", "team"]
    }
  ]
}
...
```bash
pnpm hermes-fabric store list                    # browse available modules and templates
pnpm hermes-fabric store install <module-id>     # install a module
pnpm hermes-fabric store import <template-id>    # import a company template
pnpm hermes-fabric store export                  # export current company as template
```
...
| **Multi-tenant** | Separate companies/orgs within one Hermes Fabric instance | `server:started` |
...
4. **Module config** — `fabric.config.json` with per-module config, env var interpolation
...
10. **`@hermes-fabric/module-sdk`** — TypeScript types for `ModuleAPI`, `HookEvent`, `HookHandler`, manifest schema
