...
Turn the `agent-os` research into a low-risk Hermes Fabric execution plan that preserves Hermes Fabric's control-plane model while testing the few runtime ideas that appear worth adopting.

## Decision summary

Hermes Fabric should not absorb `agent-os` as a product model or orchestration layer.

Hermes Fabric should evaluate `agent-os` in three narrow areas:
...
This work is successful when Hermes Fabric has:
...
- a proposed runtime capability model that fits current Hermes Fabric adapters
...
- replace Hermes Fabric heartbeats, issues, comments, approvals, or budgets with `agent-os` primitives
- introduce Rust/sidecar requirements for all local execution paths
- migrate all adapters at once
- add runtime workflow/queue abstractions to Hermes Fabric core

## Existing Hermes Fabric integration points
...
- prove that Hermes Fabric can drive one local agent through an `agent-os` runtime without breaking heartbeat semantics
...
- keep Hermes Fabric's existing heartbeat, issue, workspace, and comment flow authoritative
...
- introduce a Hermes Fabric-native capability vocabulary without coupling the product to `agent-os`
...
- determine whether a layered/snapshotted root model improves some Hermes Fabric workloads
...
- identify where Hermes Fabric should prefer explicit typed tools over ambient shell access
...
- one concrete proposal for typed-tool adoption in Hermes Fabric
...
- runtime experimentation could blur the boundary between Hermes Fabric as control plane and Hermes Fabric as execution platform
