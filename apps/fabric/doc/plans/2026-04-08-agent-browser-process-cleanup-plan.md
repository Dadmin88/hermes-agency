...
Yes, there is a likely root cause in Hermes Fabric's local execution model.
...
If Codex, Claude, Cursor, or a skill launched through them starts Chrome or Chromium helpers, Hermes Fabric can lose ownership of those descendants even when it still believes it handled the run correctly.
...
This is the clearest internal comparison point: Hermes Fabric already has one local-process subsystem that treats process-group ownership as the right abstraction.
...
- Hermes Fabric may think it cancelled the run because the parent process is gone
...
This work is successful when Hermes Fabric can:
...
- require every skill author to add bespoke browser teardown logic before Hermes Fabric can clean up correctly
...
- make the leak measurable from Hermes Fabric's side before changing execution semantics
...
- add a control-plane cleanup action or CLI utility for stale local run processes owned by Hermes Fabric
...
Treat this as a Hermes Fabric executor ownership bug, not an `agent-browser` bug.
