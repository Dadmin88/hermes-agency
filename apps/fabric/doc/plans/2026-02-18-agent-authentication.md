...
Agents need API keys to authenticate with Hermes Fabric. The current approach
...
**Trust model:** The adapter process runs on the same machine as the Hermes Fabric
server (or is invoked directly by it). There is no meaningful network boundary.

**Approach:** Hermes Fabric generates a token and passes it directly to the agent
...
`HERMES_FABRIC_API_URL`, `HERMES_FABRIC_AGENT_ID`, `HERMES_FABRIC_COMPANY_ID`. We need to
add a `HERMES_FABRIC_API_KEY` (JWT) to the set of injected env vars.
...
**Approach:** Similar to `claude setup-token` -- the developer runs a Hermes Fabric CLI
...
```
fabric auth login
# Opens browser -> user confirms -> token stored at ~/.hermes-fabric/credentials
```
...
managed by the Hermes Fabric server itself.
...
1. A company admin (user or agent) generates an **invite URL** from Hermes Fabric.
...
3. The agent fetches the URL, which returns an **onboarding document**
   ...
   - The Hermes Fabric SKILL.md (or a link to it)
   - What information Hermes Fabric needs from the agent (e.g. webhook URL, adapter
...
5. Hermes Fabric stores the pending registration.
...
7. On approval, Hermes Fabric provisions the agent's credentials and sends the
   first heartbeat.

**Token format:** Hermes Fabric issues an API key (or JWT) upon approval, delivered
...
```json
{
  ...
  "onboarding": {
    ...
    "skillUrl": "https://app.fabric.ing/skills/fabric/SKILL.md",
    "requiredFields": {
      "name": "Your display name",
      "adapterType": "How HermesFabric should send you heartbeats",
...
1. Generate a Hermes Fabric invite link for the company.
...
4. A Hermes Fabric company member approves the new agent.
5. Hermes Fabric begins sending heartbeats to the OpenClaw webhook endpoint.
...
| Priority | Item                              | Notes                                                                                            |
| -------- | --------------------------------- | ------------------------------------------------------------------------------------------------ |
| **P0**   | Local adapter JWT injection       | Unblocks zero-config local auth. Mint a JWT per heartbeat, pass as `HERMES_FABRIC_API_KEY`.          |
...
| **P3**   | CLI auth flow                     | `hermes-fabric auth login` for developer-managed remote agents.                                      |
