# Hermes Fabric Memory Service Plan

## Goal

Define a Hermes Fabric memory service and surface API that can sit above multiple memory backends, while preserving Hermes Fabric's control-plane requirements:

- company scoping
- auditability
- provenance back to Hermes Fabric work objects
- budget and cost visibility
- plugin-first extensibility

This plan is based on the external landscape summarized in `doc/memory-landscape.md`, the AWS AgentCore comparison captured in [PAP-1274](/PAP/issues/PAP-1274), and the current Hermes Fabric architecture in:
...
Hermes Fabric should add a company-scoped memory control plane with company default plus agent override resolution, shared hook delivery, and full operation attribution, while leaving extraction and storage semantics to built-ins and plugins.
...
Built-ins are useful for a zero-config local path, but most providers should arrive through the existing Hermes Fabric plugin runtime.

That keeps the core small and matches the broader Hermes Fabric direction that specialized knowledge systems live at the edges.

### 4. Hermes Fabric owns routing, provenance, and policy

Providers should not decide how Hermes Fabric entities map to governance.

Hermes Fabric core should own:
...
### 5. Hermes Fabric exposes shared hooks, providers own extraction

Hermes Fabric should emit a common set of memory hooks that built-ins, third-party adapters, and plugins can all use.

Those hooks should pass structured Hermes Fabric source objects plus normalized metadata. The provider then decides how to extract from those objects.

Hermes Fabric should not force one extraction pipeline or one canonical "memory text" transform before the provider sees the input.
...
Hermes Fabric should still log those changes in activity and preserve full auditability. Approval-gated memory governance can remain an enterprise or future policy layer.
...
A mapping from a Hermes Fabric target to a binding.
...
The normalized Hermes Fabric scope passed into a provider request.
...
A normalized trigger emitted by Hermes Fabric when something memory-relevant happens.
...
A normalized capture, record-write, query, browse, get, correction, or delete action performed through Hermes Fabric.

Hermes Fabric should log every memory operation whether the provider is local, plugin-backed, or external.

## Required Adapter Contract

The required core should be small enough to fit `memsearch`, `mem0`, `Memori`, `MemOS`, or `OpenViking`, but strong enough to satisfy Hermes Fabric's attribution and inspectability requirements.
...
This contract intentionally does not force a provider to expose its internal graph, file tree, or ontology. It does require enough structure for Hermes Fabric to browse, attribute, and audit what happened.
...
That lines up with the Hermes Fabric plan at a high level: provider configuration, scoped writes, scoped retrieval, provider-managed extraction as a capability, and a browse and inspect surface.

The concrete changes Hermes Fabric should take from AWS are:
...
Hermes Fabric should do the same:
...
AWS exposes list and retrieve surfaces directly. Hermes Fabric should not make browse optional at the portable layer.
...
Hermes Fabric should add cursor-based pagination to:
...
Hermes Fabric should keep its own control-plane-centric model, but the adapter contract needs obvious places to map those concepts:
...
AWS exposes extraction jobs explicitly. Hermes Fabric should too.
...
### 7. Keep Hermes Fabric provenance primary

Hermes Fabric should continue to center:
...
## What Hermes Fabric Should Persist

Hermes Fabric should not mirror the full provider memory corpus into Postgres unless the provider is a Hermes Fabric-managed local provider.

Hermes Fabric core should persist:
...
Hermes Fabric should expose one shared hook system for memory.
...
1. `pre_run_hydrate`
   Before an agent run starts, Hermes Fabric may call `query(... intent = "agent_preamble")` using the active binding.

2. `post_run_capture`
   After a run finishes, Hermes Fabric may call `capture(...)` with structured run output, excerpts, and provenance.

3. `issue_comment_capture`
   When enabled on the binding, Hermes Fabric may call `capture(...)` for selected issue comments.

4. `issue_document_capture`
   When enabled on the binding, Hermes Fabric may call `capture(...)` for selected issue documents.
...
Hermes Fabric should give agents both automatic recall and explicit tools, with simple guidance:
...
Hermes Fabric needs a first-class UI for memory, otherwise providers become black boxes.
...
Hermes Fabric should treat memory accounting as two related but distinct concerns:
...
- if a memory operation runs inside a normal Hermes Fabric agent heartbeat and the model usage is already counted on that run, do not create a duplicate `cost_event`
- instead, store the memory operation with `attributionMode = "included_in_run"` and link it to the related `heartbeatRunId`
- if a memory provider makes a direct metered model call outside the agent run accounting path, the provider must report usage and Hermes Fabric should create a `cost_event`
...
Hermes Fabric should record evaluation-oriented metrics where possible:
...
This is important because a memory system that "works" but silently burns budget or silently fails extraction is not acceptable in Hermes Fabric.
...
- it matches Hermes Fabric's local-first posture
- it is inspectable
...
- How much source payload should Hermes Fabric snapshot inside `memory_operations` for debugging without duplicating large transcripts?
...
- Hermes Fabric owns bindings, resolution, hooks, provenance, policy, and attribution.
- Providers own extraction, ranking, storage, and provider-native memory semantics.

That gives Hermes Fabric a stable memory service without locking the product to one memory philosophy or one vendor, and it integrates the AWS lessons without importing AWS's model into core.
