...
This document defines the V1 design for a Hermes Fabric adapter that integrates with
...
4. synthesized stdout events for Hermes Fabric UI/CLI
...
3. **Agent callback auth to Hermes Fabric** uses a bootstrap exchange flow (no long-lived Hermes Fabric key in prompt).
4. **Webhooks are V1**, polling remains fallback.
5. **Skill delivery** is fetch-on-demand from Hermes Fabric endpoints, not full SKILL.md prompt injection.
...
- `fabricPublicUrl` (optional override; else `HERMES_FABRIC_PUBLIC_URL` env)
...
## Hermes Fabric Callback + Auth Flow (V1)

Cursor agents run remotely, so we cannot inject local env like `HERMES_FABRIC_API_KEY`.

### Public URL

The adapter must resolve a callback base URL in this order:

1. `adapterConfig.fabricPublicUrl`
2. `process.env.HERMES_FABRIC_PUBLIC_URL`

If empty, fail `testEnvironment` and runtime execution with a clear error.

### Bootstrap Exchange

Goal: avoid putting long-lived Hermes Fabric credentials in prompt text.

Flow:

1. Before launch/follow-up, Hermes Fabric mints a one-time bootstrap token bound to:
   - `agentId`
   - `companyId`
   - `runId`
...
4. Hermes Fabric validates bootstrap token and returns a run-scoped bearer JWT.
5. Cursor agent uses returned bearer token for all Hermes Fabric API calls.
...
1. Prompt includes a compact instruction to fetch skills from Hermes Fabric.
2. After auth exchange, agent fetches:
   - `GET /api/skills/index`
   - `GET /api/skills/fabric`
   - `GET /api/skills/fabric-create-agent` when needed
...
- include webhook object when enabled:
  - `url: <fabricPublicUrl>/api/adapters/cursor-cloud/webhooks`
...
4. Route by Cursor `agentId` to active Hermes Fabric run context.
...
5. `fabricPublicUrl` present and reachable shape-valid
...
- poll interval
- timeout/grace
- Hermes Fabric public URL override
- enable webhooks
...
| Skill delivery | local skill dir injection | authenticated fetch from Hermes Fabric skill endpoints |
| Hermes Fabric auth | injected local run JWT env var | bootstrap token exchange -> run JWT |
...
2. Attach image payloads from Hermes Fabric context.
3. Add richer PR metadata surfacing in Hermes Fabric UI.
