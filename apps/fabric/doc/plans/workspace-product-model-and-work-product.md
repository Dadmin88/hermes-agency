...
Hermes Fabric needs to support two very different but equally valid ways of working:
...
Today, Hermes Fabric already has the beginnings of this model:
...
- making Hermes Fabric feel like a code review tool instead of a control plane
...
6. Seamlessly upgrade existing Hermes Fabric users to the new model without forcing disruptive reconfiguration.
7. Support cloud-hosted Hermes Fabric deployments where execution happens in remote or adapter-managed environments rather than local workers.

## Non-Goals

- Turning Hermes Fabric into a full code review product
...
This object must be recorded explicitly so that Hermes Fabric can:
...
Hermes Fabric should treat PRs as a type of work product linked back to:
...
Hermes Fabric already has users and existing project/task data. Any new model must preserve continuity.
...
### 6. Cloud-hosted Hermes Fabric must be a first-class deployment mode

Hermes Fabric cannot assume that it is running on the same machine as the code.

In cloud deployments, Hermes Fabric may:
...
- `Runtime service`: a process or service Hermes Fabric owns or tracks for a workspace
...
It also lets cloud-hosted Hermes Fabric deployments point at codebases and remotes without pretending the Hermes Fabric host has direct filesystem access.
...
This lets Hermes Fabric support:
...
Without an explicit `execution workspace` record, Hermes Fabric has nowhere stable to attach:
...
Hermes Fabric needs a single place to show:
...
- Hermes Fabric-owned processes should update health/status automatically
- external providers should at least store link, provider, external id, and latest known state
- cloud agents should be able to create work product records without Hermes Fabric owning the execution host
...
## 1. Hermes Fabric host and execution host must be decoupled

Hermes Fabric may run:
...
without the Hermes Fabric host itself running local git or local preview processes.
...
Hermes Fabric should not copy its product framing wholesale, but there are several good patterns worth applying.
...
This is a strong pattern for Hermes Fabric.
...
Hermes Fabric should keep workspace automation centered on a small set of hooks:
...
Hermes Fabric should provide the equivalent visibility in the project `Code` surface:
...
Hermes Fabric should treat execution workspaces as the natural home for:
...
Hermes Fabric should ensure that execution workspaces and work product have clean structured API surfaces, not just UI-only representation.
...
Hermes Fabric should continue treating cleanup policy as part of the core workspace model:
...
Hermes Fabric should not adopt these Worktrunk assumptions as universal product rules:

- every execution workspace is a local git worktree
- the Hermes Fabric host has direct shell and filesystem access
- every workflow is merge-centric
- every user wants developer-tool-level workspace detail in the main navigation

### Product implication

Hermes Fabric should borrow Worktrunk's good execution patterns while keeping the broader Hermes Fabric model:
...
This is how Hermes Fabric supports:
...
That keeps Hermes Fabric focused on coordination and visibility instead of splitting outputs across many hidden subsystems.
...
- cloud-hosted Hermes Fabric deployments remain first-class

That is a better fit for Hermes Fabric than either extreme:
