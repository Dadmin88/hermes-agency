...
This document translates [workspace-product-model-and-work-product.md](/Users/dotta/Hermes Fabric-subissues/doc/plans/workspace-product-model-and-work-product.md) into an implementation-ready engineering plan.
...
This repo already uses `HERMES_FABRIC_DEPLOYMENT_MODE` for auth/deployment behavior (`local_trusted | authenticated`).
...
- `HERMES_FABRIC_EXECUTION_TOPOLOGY=local|cloud|hybrid`
...
- `local`
  - Hermes Fabric may create host-local worktrees, processes, and paths
- `cloud`
  - Hermes Fabric should assume no durable host-local execution workspace management
...
- `provider text not null`
  - `fabric | github | vercel | s3 | custom`
...
- use `HERMES_FABRIC_EXECUTION_TOPOLOGY` as a defaulting guardrail
