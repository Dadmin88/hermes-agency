...
This document defines the next-stage plan for Hermes Fabric company import/export.

The core shift is:

- move from a Hermes Fabric-specific JSON-first portability package toward a markdown-first package format
...
This plan is about implementation and rollout inside Hermes Fabric.
...
Hermes Fabric already has portability primitives in the repo:
...
3. a vendor-neutral base package spec for agent-company runtimes, not just Hermes Fabric
4. the company package model is explicitly an extension of Agent Skills
5. no future dependency on `fabric.manifest.json`
6. implicit folder discovery by convention for the common case
7. an always-emitted `.fabric.yaml` sidecar for high-fidelity Hermes Fabric-specific details
...
11. adapter-aware skill sync surfaces so Hermes Fabric can read, diff, enable, disable, and reconcile skills where the adapter supports it

## 3. Product Goals

### 3.1 Goals

- A user can point Hermes Fabric at a local folder or GitHub repo and import a company package without any registry.
...
- Export produces a clean vendor-neutral package plus a Hermes Fabric sidecar.
...
Hermes Fabric must not redefine `SKILL.md`.
...
- Hermes Fabric-specific fidelity lives in `.fabric.yaml`
- Hermes Fabric may resolve and install `SKILL.md` packages, but it must not require a Hermes Fabric-only skill format
...
### 5.4 Base Package Vs Hermes Fabric Extension
...
- base package:
  ...
  - no Hermes Fabric-only runtime fields by default
- Hermes Fabric extension:
  - `.fabric.yaml`
  - adapter/runtime/permissions/budget/workspace fidelity
  - emitted by Hermes Fabric tools as a sidecar while the base package stays readable

### 5.5 Relationship To Current V1 Manifest

`fabric.manifest.json` is not part of the future package direction.
...
Hermes Fabric import/export should support these entity kinds:
...
In Hermes Fabric V2 portability:
...
- Hermes Fabric can treat “this set of agents came from team package X” as the imported-team model
...
Hermes Fabric should support source references in package metadata with:
...
Hermes Fabric should define a new adapter surface area around skills:
...
Planned Hermes Fabric behavior:

- if an adapter supports read, Hermes Fabric should show current skills in the UI
- if an adapter supports write, Hermes Fabric should let the user enable/disable imported skills
- if an adapter supports sync, Hermes Fabric should compute desired vs actual state and offer reconcile actions
...
- omit duplicated inline prompt content from `.fabric.yaml` when `AGENTS.md` already carries the instructions
- preserve references and attribution
- emit `.fabric.yaml` alongside the base package
...
## 10. Storage Model Inside Hermes Fabric
...
Hermes Fabric should add managed package/provenance records so imports are not anonymous one-off copies.
...
- `hermes-fabric company export <company-id> --out <path>`
- `hermes-fabric company import <path-or-url> --dry-run`
- `hermes-fabric company import <path-or-url> --target existing -C <company-id>`
...
1. Should imported skill packages be stored as managed package files in Hermes Fabric storage, or only referenced at import time?
...
3. Should Hermes Fabric support direct local folder selection in the web UI, or keep that CLI-only initially?
...
This keeps Hermes Fabric aligned with:
