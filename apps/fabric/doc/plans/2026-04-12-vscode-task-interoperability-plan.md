...
Hermes Fabric should not replace its workspace runtime service model with VS Code tasks.
...
- Hermes Fabric owns long-running workspace services and their desired state
- Hermes Fabric shows operators exactly which named thing they are starting or stopping
- Hermes Fabric distinguishes long-running services from one-shot jobs
...
This is directionally correct for Hermes Fabric because it gives the control plane an explicit model for service lifecycle, health, reuse, and restart behavior.
...
The issue is that the configuration surface is too low-level and Hermes Fabric does not yet leverage existing workspace metadata.

## Recommendation

Keep Hermes Fabric runtime services as the source of truth for service supervision.
...
4. `Runtime service instance`
   The live process record that already exists today in Hermes Fabric.

In that model, VS Code tasks are a way to populate workspace commands.
Only commands that map cleanly to Hermes Fabric service or job semantics should become runnable in Hermes Fabric.

## Why Not Fully Adopt VS Code Tasks

VS Code tasks are broader than Hermes Fabric runtime services.
...
That creates a bad fit if Hermes Fabric tries to use `tasks.json` as its only runtime model:
...
- problem matcher readiness is useful metadata, but it is not enough to replace Hermes Fabric's persisted service lifecycle model

The right boundary is interoperability, not replacement.

## Interoperability Contract

Hermes Fabric should support a conservative subset of VS Code tasks and clearly mark unsupported entries.

### Supported in phase 1

- `shell` and `process` tasks with a concrete command Hermes Fabric can resolve
...
- selected variable substitution when Hermes Fabric can resolve it safely from workspace context
- mapping task metadata into Hermes Fabric readiness/expose hints
- limited compound-task launch flows

### Not supported initially

- extension-provided task types Hermes Fabric cannot execute directly
...
Hermes Fabric needs an explicit classification layer instead of assuming every VS Code task is a service.

Recommended classification:

- `service`
  Explicitly marked by Hermes Fabric metadata, or confidently inferred from background/watch task semantics

- `job`
  One-shot command expected to exit

- `unsupported`
  Present in `tasks.json`, but not safely runnable by Hermes Fabric
...
- source: Hermes Fabric or VS Code
...
For a workspace with `cwd`, Hermes Fabric should look for `.vscode/tasks.json`.
...
Hermes Fabric should map imported tasks with explicit, documented rules.

Recommended rules:

1. A task becomes a `job` by default.
2. A task becomes a `service` only when:
   - Hermes Fabric metadata marks it as a service, or
...
Hermes Fabric-specific metadata can live in a namespaced field on the imported task definition, for example:
...
Success condition:
an operator can see what Hermes Fabric would import and why.
...
Goal:
run supported imported service commands through the current Hermes Fabric supervisor.
...
Success condition:
imported service commands behave like native Hermes Fabric services once adopted.
...
- removal of Hermes Fabric's own runtime supervision model
- editor-dependent execution semantics inside the control plane

## Risks

- overfitting Hermes Fabric to VS Code and making the model worse for non-VS-Code repos
...
Hermes Fabric should adopt VS Code tasks as an optional workspace command source, not as the canonical runtime model.
...
- Existing Hermes Fabric workspace plan: `doc/plans/2026-03-10-workspace-strategy-and-git-worktrees.md`
