...
- New config/env settings:
  - `HERMES_FABRIC_AGENT_JWT_SECRET`
  - `HERMES_FABRIC_AGENT_JWT_TTL_SECONDS` (default: `172800`)
  - `HERMES_FABRIC_AGENT_JWT_ISSUER` (default: `fabric`)
  - `HERMES_FABRIC_AGENT_JWT_AUDIENCE` (default: `fabric-api`)
...
1. In:
   ...
   inject `HERMES_FABRIC_API_KEY` from context token.

- Preserve existing behavior for explicit user-defined env vars in `adapterConfig.env`:
  - if user already sets `HERMES_FABRIC_API_KEY`, do not overwrite it.
- Continue injecting:
...
- Local adapters authenticate without manual `HERMES_FABRIC_API_KEY` config.
