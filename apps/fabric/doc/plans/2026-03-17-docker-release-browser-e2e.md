...
Today release smoke testing for published Hermes Fabric packages is manual and shell-driven:
...
- Docker
- `npx hermes-fabric@canary`
- `npx hermes-fabric@latest`
...
- runs `hermes-fabric@${HERMES_FABRIC_VERSION}` inside Docker
- waits for health
...
- email: `smoke-admin@fabric.local`
- password: `fabric-smoke-password`
...
- the suite passes locally against both:
  - `HERMES_FABRIC_VERSION=canary`
  - `HERMES_FABRIC_VERSION=latest`
...
1. A published `hermes-fabric@canary` Docker install can be smoke-tested by Playwright in CI.
2. A published `hermes-fabric@latest` Docker install can be smoke-tested by Playwright in CI.
