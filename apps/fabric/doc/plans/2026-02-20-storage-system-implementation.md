...
Add a single storage subsystem for Hermes Fabric that supports:
...
- API serves attachment bytes through authenticated Hermes Fabric endpoints.
...
- Default local path is under instance root: `~/.hermes-fabric/instances/<instanceId>/data/storage`.
...
- `hermes-fabric onboard` writes a valid `storage` config block by default.
- `hermes-fabric configure --section storage` can switch between local and s3 modes.
...
- `hermes-fabric doctor` reports actionable storage status.
