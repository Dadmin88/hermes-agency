...
- local mode that still feels instant (`npx hermes-fabric run` and go)
...
2. CLI command `pnpm hermes-fabric auth bootstrap-ceo` creates a one-time CEO onboarding invite URL for that instance.
3. `pnpm hermes-fabric onboard` runs this bootstrap check and prints the invite URL automatically when `bootstrap_pending`.
4. Visiting the app while `bootstrap_pending` shows a blocking setup page with the exact CLI command to run (`pnpm hermes-fabric onboard`).
...
- expose key instance settings in API + CLI (`hermes-fabric configure` / `hermes-fabric onboard`)
...
7. `pnpm hermes-fabric onboard` outputs a CEO onboarding invite URL when bootstrap is pending.
