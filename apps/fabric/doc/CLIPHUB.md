...
ClipHub is the public registry where people share, discover, and download Hermes Fabric company configurations. A company template is a portable artifact containing an entire org — agents, reporting structure, adapter configs, role definitions, seed tasks — ready to spin up with one command.

---

## What It Is

ClipHub is to Hermes Fabric what a package registry is to a programming language. Hermes Fabric already supports exportable org configs (see [SPEC.md](./SPEC.md) §2). ClipHub is the public directory where those exports live.

A user builds a working company in Hermes Fabric — a dev shop, a marketing agency, a research lab, a content studio — exports the template, and publishes it to ClipHub. Anyone can browse, search, download, and spin up that company on their own Hermes Fabric instance.
...
A ClipHub package is a **company template export** — the portable artifact format defined in the Hermes Fabric spec. It contains:
...
**Install (fresh start):**
```
fabric install cliphub:<publisher>/<company-slug>
```
Downloads the template and creates a new company in your local Hermes Fabric instance. You add your own API keys, set budgets, customize agents, and hit go.
...
From within Hermes Fabric, export your company as a template, then publish:

```
fabric export --template my-company
fabric publish cliphub my-company
```
...
ClipHub is a **separate service** from Hermes Fabric itself. Hermes Fabric is self-hosted; ClipHub is a hosted registry that Hermes Fabric instances talk to.
...
| **Hermes Fabric CLI** | `hermes-fabric install`, `hermes-fabric publish`, `hermes-fabric cliphub sync` — built into Hermes Fabric |
| **Hermes Fabric UI** | "Browse ClipHub" panel in the Hermes Fabric web UI for discovering templates without leaving the app |

### Tech Stack

| Layer | Technology |
|---|---|
| Frontend | React + Vite (consistent with Hermes Fabric) |
| Backend | TypeScript + Hono (consistent with Hermes Fabric) |
| Database | PostgreSQL |
...
4. Run `hermes-fabric install cliphub:acme/lean-saas-shop`
5. Hermes Fabric creates the company locally with all agents pre-configured
6. Set your API keys, adjust budgets, add your initial tasks
7. Hit go

### "I built something great and want to share it"

1. Build and iterate on a company in Hermes Fabric until it works well
2. Export: `hermes-fabric export --template my-agency`
3. Publish: `hermes-fabric publish cliphub my-agency`
...
3. Install just that agent: `hermes-fabric install cliphub:acme/senior-python-eng --agent`
4. Assign it to a manager in your existing company
5. Done

---

## Relationship to Hermes Fabric

ClipHub is **not required** to use Hermes Fabric. You can build companies entirely from scratch without ever touching ClipHub. But ClipHub dramatically lowers the barrier to entry:
...
ClipHub is to Hermes Fabric what a package registry is to a language runtime: optional, but transformative.
...
- [ ] `hermes-fabric install cliphub:<publisher>/<slug>` CLI command
...
- [ ] "Browse ClipHub" panel in Hermes Fabric web UI
- [ ] `hermes-fabric cliphub sync` for bulk publishing
...
- Running companies on ClipHub (it's a registry, not a runtime — consistent with Hermes Fabric's own philosophy)
