...
class PoolManager:
    ...
    def _ensure_profile(self, name):
        ...
        model_name = model.get('model', 'gpt-5.5')
        provider = model.get('provider', 'openai')
        try:
            subprocess.run(
                [HERMES_BIN, '-p', name, 'config', 'set', 'model.default', model_name],
                check=True, capture_output=True, text=True, timeout=30,
            )
            subprocess.run(
                [HERMES_BIN, '-p', name, 'config', 'set', 'model.provider', provider],
                check=True, capture_output=True, text=True, timeout=30,
            )
        except subprocess.CalledProcessError as e:
            print(f"[PoolManager] Config set warning for {name}: {e.stderr or e}")
        except Exception as e:
            print(f"[PoolManager] Config set warning for {name}: {e}")
        return profile_dir
    ...
    def _resolve_started_peer_id(self, name, initial_output='', timeout=10):
        ...
        print(f"[PoolManager] final agency status output for {name}: {last_status_output.strip()[:1000]}")
    ...
    def wake(self, name, persistent=False):
        ...
        with self.lock:
            ...
            if cli_started:
                ...
                print(f"[PoolManager] agency start CLI unavailable/failed for {name}; falling back to runner")
            proc = None
    ...
    def sleep(self, name):
        with self.lock:
            ...
            if proc is not None and proc.poll() is None:
                ...
                if stop_output:
                    print(f"[PoolManager] agency stop output for {name}: {stop_output.strip()[:1000]}")
                if not stopped:
                    print(f"[PoolManager] CLI stop unavailable/failed for {name}; removing from active registry only")
            del self.active[name]
