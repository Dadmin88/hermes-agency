...
JSON restoration is a hostile-input boundary: it requires an explicit, first, and unique `WORKFLOW_CREATED` genesis event with an exact creation-input payload, and strictly parses raw envelopes, revisions, gates, verdicts, operator decisions, artifact identities, and every event payload before any model constructor can normalize values. Each event type has an allowlisted payload shape; malformed values, containers, missing or extra keys, and coherent event/snapshot/digest rewrites carrying malformed data fail deterministically with `SerializationError`. Artifact identities must use the exact canonical schema and types (including real mappings for hashes and byte sizes and a real boolean freeze state). It rebuilds the initial run and fixed architecture-governance template only from that event, then replays later events and requires exact equality with the stored materialized state. Snapshot-only changes to topology, gate policy, revision identity, author ownership, or artifact hashes therefore fail closed. Event digests provide deterministic identity and idempotency checks, not storage authentication: deterministic validation cannot detect a wholly valid rewritten unauthenticated ledger/root until a trusted signed root or authenticated persistence layer is added.
