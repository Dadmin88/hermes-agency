...
class AgentCard:
    ...
    def to_dict(self) -> dict[str, Any]:
        ...
        if has_p2p:
            ...
            # Preserve AgentAnycast compatibility while also emitting the
            # Keryx extension name so Keryx-native registry/card consumers can
            # read the same transport metadata without special-casing.
            d["agentanycast"] = dict(p2p)
            d["keryx"] = dict(p2p)
        return d
    ...
    def from_dict(cls, data: dict[str, Any]) -> AgentCard:
        ...
        # Accept both the legacy AgentAnycast extension and the native Keryx
        # extension.  If both exist, Keryx-specific values override the shared
        # AgentAnycast metadata.
        p2p: dict[str, Any] = {}
        agentanycast_ext = data.get("agentanycast", {})
        keryx_ext = data.get("keryx", {})
        for extension_name, extension in (
            ("agentanycast", agentanycast_ext),
            ("keryx", keryx_ext),
        ):
            if extension is None:
                continue
            if not isinstance(extension, dict):
                raise ValueError(f"AgentCard {extension_name} extension must be a dictionary")
            p2p.update(extension)
